# 《發現之前》遊戲 runtime

本資料夾已從早期斜面灰盒演進為系列正式 runtime；名稱保留只為相容既有路徑。

## 玩家入口

- `stage.html`：唯一正式入口。首頁以「目前旅程＋章節目錄」承載資料驅動章節列表；目前可選第一章《重物的渴望》、第二章《第一寸的弧線》、第三章《船艙裡的靜止》或第四章《月亮的無盡墜落》。
- 線上版：<https://before-discovery.vercel.app/>

`chapter.html`、`chapter2.html`、`index.html` 是內部 QA／開發對照殼，不再對玩家公開推薦。

## 目前完成度（2026-07-23）

- 第一章：完整候選版。
- 第二章：M1–M3、彈射工坊、完整辯論、敵方數據卡、跨章書信碼、P0/P1 美術均已接入正式舞台。
- 第三章：航船共同運動、雙紙帶、公開驗證、專屬美術與音樂均已接入。
- 第四章：14 場 runtime、逐拍軌道、60／60 跨尺度、封存行星預測、雙模型反驗、行動制校樣、獨立存檔、七組場景與兩齡 Newton／Halley 角色均已接入；十一首專屬音樂提示詞已交付，正式音檔待生成與實聽。

## 測試

在本資料夾執行：

```bash
npm test
```

現行基線：核心 **107 通過、0 失敗**，另跑第四章美術／音樂交接契約。涵蓋第一章 216 判定點、四章敘事可達性、前三章既有回歸，以及第四章軌道後果、跨尺度 fixture、封存預測、模型反驗、校樣窗口、全章走查、存檔淨化、舞台與正式資產契約。

真人瀏覽器與手機簽核表：`docs/browser-checklist.md`。

## 結構

- `src/`：純函式引擎、敘事狀態機、章節 UI、舞台表現層。
- `src/stage/*.part.js`：舞台來源；`stage-ui.js` 為生成物。
- `data/`：四章劇情、辯論、史實、fixture 與美術 manifest（JS 執行載體＋JSON 規範鏡像）。
- `tests/`：零外部依賴的 Node 回歸測試。
- `docs/`：瀏覽器清單、美術需求與開發文件。
- `tools/`：stage／assets 生成與一致性工具。
- `spike_workshop/`（根目錄）：工坊研究切片；正式實作以本資料夾 `engine2.js` 為準。
