 ...
 "scenes": [
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "type": "line",
     "speaker": "杜夏特萊",
     "text": "（順著他的視線）拆了才好逐頁對照。旁邊那些，是我自己的清樣。",
     "next": "x6"
    },
    {
     "id": "x6",
     "type": "line",
     "speaker": "杜夏特萊",
     "text": "我正在校訂自己的《物理學原理》（Institutions de physique）清樣。印刷鋪明早來收；我想在談活力的那幾頁留一個欄位。那筆短少今晚未必能算平，但我得弄清楚，這些紙到底准許我寫到哪裡。",
     "next": "c_intro"
    },
    {
     "id": "c_intro",
   ...
   "nodes": [
    {
     ...
     "type": "line",
     "speaker": "杜佩院士",
     "text": "運動之量，就是質量乘速度。方向也要算。撞前有多少，撞後就有多少。至於萊布尼茲那個還要把速度自乘的『活力』——多出來的玄想罷了。",
     "next": "x2"
    },
    {
     "id": "x2",
    ...
    },
    {
     ...
     "speaker": "system",
     "receiptKind": "source",
     "text": "取得 S7：黏土報告。黏土坑提供一把可量的尺；它還不能說明短少的完整去向。",
     "effects": [
      {
       "evidence": "S7"
      }
  ...
  },
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "type": "line",
     "speaker": "杜夏特萊",
     "text": "先別比哪個數字大。你要問的是：同一場碰撞前後，每一本帳記的數有沒有變。這句話由你自己寫。",
     "next": "q1"
    },
    {
     "id": "bad2",
  ...
  },
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "system": "collision",
     "phase": "momentum",
     "hint": "這一輪要完成：彈開與黏住兩種碰法，各用同一種速度做三回；六張紀錄都留下後，說出這批紀錄支持哪一句話。",
     "until": {
      "collision": "j1"
     },
     "next": "x6"
  ...
  },
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "type": "line",
     "speaker": "杜夏特萊",
     "text": "對。如果真是同一件事的兩種寫法，它們在同一場撞裡，就該一起對平、一起短少。",
     "next": "reuse_2"
    },
    {
     "id": "reuse_2",
    ...
    },
    {
     ...
     "system": "collision",
     "phase": "vis-viva",
     "hint": "這一輪要完成：不要再撞。挑回第一輪那同一批紀錄，用第二本帳重算一次；再補一筆兩球一輕一重（四比八）的油灰紀錄。",
     "until": {
      "collision": "followup"
     },
     "next": "x8"
  ...
  },
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "system": "collision",
     "phase": "clay",
     "hint": "這一輪要完成：三種落下高度各留一筆坑深紀錄；留完後，說出這盤油灰支持哪一句話。",
     "until": {
      "collision": "j3"
     },
     "next": "c_scale"
   ...
   "nodes": [
    {
     ...
     "type": "system",
     "speaker": "system",
     "text": "莊園主人請眾人入席。動量帳、活力帳與黏土盤並排留在桌上。論證對位從 5/5 開始：證據若咬不住主張，就會退回；歸零先複盤，帳與已破支柱都保留。",
     "effects": [
      {
       "debate": "init"
      }
  ...
  },
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "type": "line",
     "speaker": "stage",
     "text": "人散得很慢。杜佩院士最後一個起身，走到桌邊，在兩本帳之間重新列式。他把坑深逐列抄到活力帳短少的數字旁，試著找出固定的換法；算了半頁，仍沒有一張紙告訴他該怎麼換。",
     "next": "x2"
    },
    {
     "id": "x2",
  ...
  },
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "type": "line",
     "speaker": "杜夏特萊",
     "text": "（看著清樣）明早它就送進印刷鋪。接下來每一封質問，我都得為這個『未決』回答；但今晚沒有量到的，我不替它補。",
     "next": "x5"
    },
    {
     "id": "x5",
  ...
  },
  {
   ...
   "nodes": [
    ...
    },
    {
     ...
     "type": "line",
     "speaker": "旅人・心聲",
     "text": "空框。劍橋那個問號曾在抽屜裡待了二十年。這個要多久？",
     "osPurpose": "cross_chapter_memory",
     "next": "review"
    },
    {
    ...
    },
    {
     ...
     "type": "line",
     "speaker": "stage",
     "text": "旅人翻回筆記最前面。那行褐掉的字還在：「這個世界只為證據付費。」",
     "next": "x6"
    },
    {
     "id": "x6",
    ...
    },
    {
     ...
     "options": [
      {
       "id": "write-question",
       "text": "油灰上的坑只留下了一個可量的印子。那一截短少的，完整去向在哪裡？",
       "next": "x7"
      }
     ]
    },
    {
     ...
     "type": "line",
     "speaker": "stage",
     "text": "那一截短少的，沒有消失——油灰上的坑就是它留下的一個印子。可是那把尺只量到油灰，沒有量到那兩顆球。剩下的部分，我還沒有辦法量。它去了哪裡？",
     "next": "x8"
    },
    {
     "id": "x8",
