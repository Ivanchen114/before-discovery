...
(function () {
  ...
  var SERIES_CHAPTERS = REGISTRY ? REGISTRY.chapters : [];
  var CHAPTER_META = REGISTRY && REGISTRY.byId
    ? REGISTRY.byId(N.CHAPTER_ID || SCENES.chapter || "ch1") : null;
  if (!CHAPTER_META || !CHAPTER_META.runtime)
    throw new Error("章節介面找不到 runtime metadata:" + String(SCENES.chapter || "(空白)"));
  var CHAPTER_ID = CHAPTER_META.id;
  var KEY = CHAPTER_META.runtime.saveKey;
  ...
  var evidenceCueIndex = {};
  (SCENES.scenes || []).forEach(function (scene) {
    (scene.nodes || []).forEach(function (node) {
      if (!node.evidenceCue) return;
      evidenceCueIndex[String(node.evidenceCue).toUpperCase()] = {
        sceneId: scene.id,
        nodeId: node.id,
        sourceNodes: Array.isArray(node.evidenceCueFrom) ? node.evidenceCueFrom.slice() : []
      };
    });
  });
  ...
  function sanitizeLoaded(s) {
    ...
    return window.GB.Sanitize.sanitizeByChapter(CHAPTER_ID, s, {
      patterns: PATTERNS,
      scenes: SCENES,
      engine: window.GB.Engine
    });
  }
  ...
  function evidenceDispositionForState(s, code) {
    var upper = String(code || "").toUpperCase();
    var lab = s && s.lab;
    if (CHAPTER_ID !== "ch7" || !lab || !lab.integrity) return { status:"available" };
    var active = lab.integrity.activeWithholding;
    if (!active) return { status:"available" };
    var traceByCode = {
      GRID_BASELINE:"baseline", GRID_BIMETAL:"bimetal",
      GRID_SAME_METAL:"sameMetal", GRID_NO_METAL:"noMetal",
      GRID_ELECTROMETER:"electrometer", GRID_PILE:"pile"
    };
    var key = traceByCode[upper];
    var withheld = upper === "GRID_STATE" ||
      (active.kind === "trace" && active.id === key) ||
      (active.kind === "claim-M-papers" && key === "noMetal");
    return withheld ? {
      status:"withheld",
      label:"原紙已離桌，待修復",
      reason:"這張原紙目前暫扣。修復後會恢復內容；事故與修復史仍留在研究紀錄。"
    } : { status:"available" };
  }
  function evidenceRecordById(lab, id) {
    return (lab && lab.records || []).find(function (row) { return row && row.id === id; }) || null;
  }
  function evidencePathProjection(path) {
    if (!Array.isArray(path) || !path.length) return [];
    var step = Math.max(1, Math.ceil(path.length / 72));
    return path.filter(function (_, index) { return index % step === 0 || index === path.length - 1; })
      .map(function (point) {
        return { x:Number(point.x) || 0, y:Number(point.y) || 0 };
      });
  }
  /* GB-ADR-057：只把本局可見、可驗的最小資料投影給舞台；固定美術不得
     成為第二份答案資料庫。這裡不寫 state，也不補不存在的原紙。 */
  function evidenceOverlayForState(s, code) {
    var upper = String(code || "").toUpperCase();
    var lab = s && s.lab || {};
    var disposition = evidenceDispositionForState(s, upper);
    if (disposition.status === "withheld") return {
      kind:"withheld", title:"內容暫扣", lines:[disposition.reason]
    };
    if (CHAPTER_ID === "ch4" && upper === "K1") {
      ...
      var selected = (lab.orbitLab && lab.orbitLab.paperTrials || []).filter(function (run) {
        return ids.indexOf("trial:" + run.id) >= 0;
      });
      if (!selected.length) selected = (lab.orbitLab && lab.orbitLab.paperTrials || []).slice(-3);
      return {
        kind:"orbit-trials", title:"本局選入斷言的試跑紙",
    ...
    if (CHAPTER_ID === "ch4" && upper === "K3") {
      var planetNames = { mars:"火星", jupiter:"木星" };
    ...
    if (CHAPTER_ID === "ch2" && ["F1","F2","F4","F5"].indexOf(upper) >= 0) {
      var sourceId = lab.evidence && lab.evidence.f2 && lab.evidence.f2.lawSource;
      var series = (lab.series || []).filter(function (row) { return !sourceId || row.id === sourceId; });
      var lines2 = series.slice(-2).map(function (row) {
        var reads = Object.keys(row.readings || {}).sort(function (a,b) { return Number(a) - Number(b); })
          .map(function (height) { return height + " 格→" + row.readings[height]; });
        return "第 " + row.id + " 輪｜" + reads.join("、") +
          (row.prediction == null ? "" : "｜第五筆先押 " + row.prediction);
      });
      if (!lines2.length && upper === "F1") lines2.push("船桅落球仍是待驗預測；尚未取得船上原紙。");
      return { kind:"lines", title:"本局裝置與原紙", lines:lines2 };
    }
    if (CHAPTER_ID === "ch3" && /^G[1-5]$/.test(upper)) {
      var dossier = lab.caseFile && lab.caseFile.dossier || {};
      var records = dossier.records || [];
      var blind = dossier.blind && dossier.blind.records || [];
      var lines3 = upper === "G2" ? blind.slice(-6).map(function (row) {
        return row.id + "｜" + (row.stageLabel || row.stage) + "｜" +
          (row.instrument || "觀察物未留存") + "｜" + (row.water || row.ball || "結果未留存");
      }) : records.slice(-6).map(function (row) {
        return row.id + "｜" + (row.stageLabel || row.stage || "船況未留存") + "｜" +
          (row.classification || "尚待判讀");
      });
      return { kind:"lines", title:"本局留下的船紙與岸紙", lines:lines3 };
    }
    if (CHAPTER_ID === "ch5" && /^J[1-4]$/.test(upper)) {
      var collisionHeadNames = { steel:"鋼頭", putty:"油灰頭" };
      var collisionSpeedNames = { low:"慢速", mid:"中速", high:"快速" };
      var collisionMassNames = { "4/4":"兩球各 4 磅", "4/8":"4 磅撞 8 磅" };
      var clayMassNames = { light:"輕球", heavy:"重球" };
      var collision = (lab.collisionRuns || []).map(function (row) {
        return "碰撞 #" + row.id + "｜" + (collisionHeadNames[row.head] || row.head) + "｜" +
          (collisionMassNames[row.masses] || row.masses) + "｜" +
          (collisionSpeedNames[row.speedBand] || row.speedBand) +
          "｜p " + row.momentum.before + "→" + row.momentum.after +
          "；v²帳 " + row.visViva.before + "→" + row.visViva.after;
      });
      var clay = (lab.clayRuns || []).map(function (row) {
        return "黏土 #" + row.id + "｜" + (clayMassNames[row.ballMass] || row.ballMass) +
          "｜高度 " + row.height +
          "｜速度 " + row.speed + "｜坑深 " + row.depth;
      });
      return { kind:"lines", title:"本局碰撞與黏土原紙",
        lines:upper === "J3" ? clay : (upper === "J4" ? collision.concat(clay) : collision) };
    }
    if (CHAPTER_ID === "ch6" && /^T[1-5]$/.test(upper)) {
      var relevant = (lab.records || []).slice(-8).map(function (row) {
        var values = row.curve || row.chipCurve || [];
        return "原紙 #" + row.id + "｜" + row.kind +
          (values.length ? "｜" + values.join("→") : "") +
          (row.temperature == null ? "" : "｜" + row.temperature + "°");
      });
      return { kind:"lines", title:"本局溫度原紙", lines:relevant };
    }
    if (CHAPTER_ID === "ch7" && /^GRID_/.test(upper)) {
      var keys = {
        GRID_BASELINE:"baseline", GRID_BIMETAL:"bimetal", GRID_SAME_METAL:"sameMetal",
        GRID_NO_METAL:"noMetal", GRID_ELECTROMETER:"electrometer", GRID_PILE:"pile"
      };
      var all = upper === "GRID_STATE"
        ? ["baseline","bimetal","sameMetal","noMetal","electrometer","pile"]
        : [keys[upper]];
      var names7 = { baseline:"外部刺激基準", bimetal:"雙金屬", sameMetal:"同材質",
        noMetal:"無金屬", electrometer:"電量器", pile:"伏打堆" };
      var lines7 = all.filter(Boolean).map(function (key) {
        var row = evidenceRecordById(lab, lab.matrix && lab.matrix.traces && lab.matrix.traces[key]);
        return row ? ((names7[key] || key) + "｜" + (row.configNote || "配置未留存") +
          "｜觀測：" + (row.observation || "未留存")) : ((names7[key] || key) + "｜尚未取得原紙");
      });
      return { kind:"lines", title:upper === "GRID_STATE" ? "本局六格合帳" : "本局觀測紙", lines:lines7 };
    }
    return null;
  }
  ...
  function collectNewEvidence(before, after) {
    ...
    var sourceNode = before && before.cursor ? before.cursor.node : null;
    Object.keys(now).forEach(function (code) {
      if (now[code] && !old[code]) {
    ...
    /* 實驗取得證據時不一定有取得台詞：等本次事件堆疊完成後才補發。
       有指定 scene 掛點的證據留在佇列；其餘證據才由 timer 補發。 */
    if (pendingEvidence.some(function (item) { return !item.releaseAt; }) && !pendingEvidenceTimer) {
  ...
  function takePendingEvidence(sceneId, nodeId) {
    var items = [];
    var keep = [];
    pendingEvidence.forEach(function (item) {
      if (!item.releaseAt) items.push(item);
      else if (sceneId && nodeId && item.releaseAt.sceneId === sceneId && item.releaseAt.nodeId === nodeId)
        items.push(item);
      else keep.push(item);
    });
    pendingEvidence = keep;

    /* 讀檔後記憶體佇列已消失，但若授證行尚未播放，仍依狀態補發；
       transcript 重建時 addLine 會因 replaying 而跳過本函式。 */
    if (sceneId && nodeId && state && state.evidence) {
      Object.keys(evidenceCueIndex).forEach(function (code) {
        var target = evidenceCueIndex[code];
        if (target.sceneId !== sceneId || target.nodeId !== nodeId) return;
        var stateCode = Object.keys(state.evidence).find(function (key) {
          return key.toUpperCase() === code;
        });
        if (!stateCode || !state.evidence[stateCode] || items.some(function (item) {
          return item.code.toUpperCase() === code;
        })) return;
        items.push({
          code: stateCode,
          name: (SCENES.evidenceNames || {})[code] || "新證據",
          sceneId: sceneId,
          releaseAt: target
        });
      });
  ...
  function addLine(speaker, text, cls, sceneId, nodeId) {
    ...
    emit("bd:line", {
      ...
      evidence: replaying ? [] : takePendingEvidence(
        sceneId || (state && state.cursor ? state.cursor.scene : null), nodeId || null),
      replay: replaying
    });
  }
  ...
  function rebuildLog() {
    ...
    state.transcript.forEach(function (e) {
      ...
      addLine(e.speaker, e.text, classFor(e.speaker), e.scene, e.node);
    });
    replaying = false;
  }
  ...
  function renderStatus() {
    ...
    if (CHAPTER_ID === "ch2") {
      ...
      var grid7 = state.lab.evidence || {};
      var gridKeys7 = ["baseline", "bimetal", "sameMetal", "noMetal", "electrometer", "pile"];
      var acquired7 = gridKeys7.filter(function (key) { return grid7[key]; }).length;
      /* 進入工作台到第一次操作之間，引擎仍保留上一個 phase。HUD 必須以眼前
         可操作節點為準，否則玩家站在堆前還會被告知「等信」。 */
      var targetPhase7 = state.cursor && state.cursor.node === "e_pile" ? "pile"
        : state.cursor && state.cursor.node === "e_board" ? "board"
          : state.lab.phase;
      var target7 = {
        qualification: "完成四種蛙腿接法",
        "exclusive-claim": "在原紙邊寫下主張",
        "next-config": "選一格真能分出說法的配置",
        electrometer: "親手完成無蛙的針格",
        "mid-verdict": "先判讀桌上五格",
        "await-pile": "等一封還沒有答完的信",
        pile: "依圖說排層並親手同觸兩端",
        board: "把六張原紙歸回同一頁",
        "final-verdict": "只寫六格撐得起的結論",
        named: "在公開原句上補上邊界",
        scoped: "六格已合帳"
      }[targetPhase7] || "完成眼前這一格";
      $("e3Val").textContent = "實驗紙：" + acquired7 + "/6" +
        (state.lab.matrix && state.lab.matrix.boardComplete ? "｜合帳●" : "｜合帳○");
      $("e3Val").title = "現在只做：" + target7 + "。每張紙只記實際完成的配置與觀察。";
    } else {
    ...
    if (CHAPTER_ID === "ch3") {
      ...
      var active7 = state.lab.integrity && state.lab.integrity.activeWithholding;
      var claim7 = state.lab.commitment && state.lab.commitment.exclusiveClaim;
      var final7 = state.lab.verdicts && state.lab.verdicts.final;
      $("perVal").textContent = active7 ? "原紙誠信：待修復"
        : final7 ? "主張邊界：已限縮"
          : claim7 === "A" ? "署名主張：已傳抄"
            : claim7 === "M" ? "排他主張：原紙已反駁"
              : claim7 === "not-yet" ? "主張：暫不排他" : "";
      $("perVal").title = active7
        ? "有一筆原紙被收離桌面；先放回去，再繼續判讀。"
        : "這裡記的是你公開承擔到哪一句，不是答對幾題。";
    } else {
      $("perVal").textContent = state.debate ? ("論證對位｜" + state.debate.persuasion + "/5") : "";
  ...
  function renderLabTables() {
    ...
    order.forEach(function (k) {
      ...
      list.forEach(function (r, i) {
        ...
        [("#" + r.id), cfgLabel(r.config)].concat(r.readings.slice(0, 4).map(fmt))
  ...
  function lab1AssertionCorrection(type, assertion) {
    if (!assertion || assertion.ok) return;
    var playerText = type === "b"
      ? "我用這兩筆主張：只換球重，規律沒有改變。"
      : "我用這兩筆主張：只換傾角，規律的形式沒有改變。";
    var reply;
    if (assertion.reason === "兩筆主張皆須成立")
      reply = "先等等。你挑的紙裡，有一張連自己押的第五段都還沒接上。它現在還不能替下一層比較作證。";
    else if (assertion.reason === "球之相異須為銅大/銅小")
      reply = "這兩張確實只換了球，可你要問的是重量，就得讓同樣是銅球的一重一輕。現在這一對還沒把重量單獨挑出來。";
    else if (assertion.reason === "變因不符") {
      var changed = assertion.diff && assertion.diff.length ? assertion.diff.join("、") : "不只一項條件";
      reply = "你看這兩張紙的條件欄。跟著一起變的是「" + changed + "」。一次動了不只你要問的那一項，這句話就還接不上。";
    } else return;
    sayWorkbenchCorrection(playerText, "伽利略", "把兩張實驗紙拉到同一條刻線上", reply);
  }
  ...
  function cat2SayFailure(action, args, code, result) {
    args = args || {};
    if (action === "assertLaw" && code === "concept-mismatch") {
      var concept = (SCENES.lab2LawConcepts || []).filter(function (item) {
        return item.id === args.conceptId;
      })[0];
      sayWorkbenchCorrection(
        "我用第 " + args.seriesId + " 組紙，主張「" + (concept ? concept.label : "這個關係") + "」。",
        "伽利略", "用指甲依序點過四格與十六格的兩欄",
        "這裡的下落高度放大了四倍，沙痕卻只從約兩尺走到約四尺。你剛說的關係若成立，這兩欄不該留下現在這個倍數。"
      );
      return;
    }
    if (action === "assertLaw" && code === "law-source-ball") {
      sayWorkbenchCorrection(
        "我先引用第 " + args.seriesId + " 組紙來寫第一句規律。",
        "伽利略", "把紙角的球種欄轉向旅人",
        "這張寫的是木球。木球要留下來回答換重量後有沒有改變；第一張基準得先用你剛才那組銅球，兩步才不會纏在一起。"
      );
      return;
    }
    if (action !== "compareBalls" || code !== "compare-mismatch") return;
    var diffs = result && result.diffs || [];
    var reply = diffs.indexOf("球種須一銅一木") >= 0
      ? "你選的兩張球種欄相同。重做同一顆球能看穩不穩，卻還沒有問到重量換掉後會怎樣。"
      : (diffs.indexOf("裝置指紋(零件/校準)不同") >= 0 || diffs.indexOf("裝置曾拆裝(revision 不同)") >= 0
        ? "兩張紙的裝置與校準欄對不上。球換了，器材也跟著換；這一對不能把差別只算在重量頭上。"
        : "這兩張裡至少有一張沒有留下完整、穩定的四個高度。先別拿破掉的比較，替重量下結論。");
    sayWorkbenchCorrection(
      "我把第 " + args.a + " 組和第 " + args.b + " 組並排，主張兩組只換了球。",
      "伽利略", "把兩張紙的條件欄疊在一起", reply
    );
  }
  function doLab2(action, args, okText) {
    ...
    var r = N.labAction(state, action, args);
    if (r.error) {
      cat2Msg = cat2ErrorText(r.error, r.result);
      cat2SayFailure(action, args, r.error, r.result);
    }
    else {
      setState(r.state);
      ...
      if (r.result && r.result.ok === false)
        cat2SayFailure(action, args, action === "assertLaw" ? r.result.reason : "compare-mismatch", r.result);
  ...
  var ship3CabinWindDialogueOpen = false; /* 研究策略只在人物對話中選；讀檔也不得退回工作台三選。 */
  ...
  function sayWorkbenchCorrection(playerText, speaker, action, reply) {
    if (!reply) return;
    var lines = [];
    if (playerText) lines.push({ speaker: "旅人(你)", text: playerText });
    lines.push({ speaker: speaker, action: action || "", text: reply });
    sayIntoDialogue(lines, "", state && state.cursor ? state.cursor.scene : null);
  }
  ...
  var SHIP3_CABIN_WIND_OPTIONS = [
    {
      id: "wait-calm", label: "不然，等一個沒風的日子再試？",
      feedback: "沒有量風，就不能確認兩趟的甲板條件相同。",
      reply: [
        { speaker: "維達爾船長", text: "你可以在馬賽等。我也可以等——一天照算錢。" },
        { speaker: "伽桑狄", text: "可我們沒有量風。就算大家都說今天沒風，紙上也不能把兩趟寫成同一個條件。" }
      ]
    },
    {
      id: "close-cabin", correct: true, label: "那就進船艙吧，把外頭的風隔開。",
      feedback: "承諾已寫下：隔開甲板風，但結論只限關艙後的比較。",
      reply: [
        { speaker: "伽桑狄", text: "對。先把窗關上。不過這樣只能問：沒有甲板風時，停船和平駛的艙內痕跡能不能分開。" },
        { speaker: "旅人(你)", text: "嗯。我們不重做桅頂落石，只比較關艙後留下的痕跡。" },
        { speaker: "伽桑狄", action: "推開艙門，指了指吊壺與石球", text: "進去選一樣：讓水滴落進碗，或把石球直直拋起。兩種船況都用同一樣。" },
        { speaker: "艾蒂安", text: "我留在岸上。等我確認船走穩，再升白旗。" }
      ]
    },
    {
      id: "windbreak", label: "要不要先在甲板立一面擋風板？",
      feedback: "擋風板仍留在甲板風裡，殘留風也沒有被量清楚。",
      reply: [
        { speaker: "馬蒂厄", text: "能擋掉一部分。可剩下多少風，我們還是不知道。" },
        { speaker: "伽桑狄", text: "那張紙最多只能寫『擋過風』，不能寫成『風已經排除』。" }
      ]
    }
  ];
  ...
  function ship3FocusDossierRunButton() {
    window.requestAnimationFrame(function () {
      var work = document.querySelector(".shipDossierWork");
      var button = work &&
        work.querySelector('[data-ship-focus="run-deck-record"]:not(:disabled)');
      if (!work || !button) return;
      var workRect = work.getBoundingClientRect();
      var buttonRect = button.getBoundingClientRect();
      if (buttonRect.top < workRect.top + 10)
        work.scrollTop -= workRect.top + 10 - buttonRect.top;
      else if (buttonRect.bottom > workRect.bottom - 10)
        work.scrollTop += buttonRect.bottom - (workRect.bottom - 10);
      ship3DossierScrollTop = work.scrollTop;
      try { button.focus({ preventScroll: true }); } catch (e) { button.focus(); }
    });
  }
  function ship3PresentCabinWindDialogue(fromReload) {
    if (ship3CabinWindDialogueOpen) return;
    ship3CabinWindDialogueOpen = true;
    ship3OpenIntermission("ch3-cabin-wind", "", SHIP3_CABIN_WIND_OPTIONS.map(function (option) {
      return { id: option.id, label: option.label };
    }));
    /* 正常路徑已在斷言成立後播過這段；只有讀檔直接落在船艙任務時補回因果橋。 */
    if (fromReload) ship3SayMissionBridge("speed", "cabin");
  }
  ...
  document.addEventListener("bd:workbench-intermission-choice", function (event) {
    ...
    sayIntoDialogue([
      { speaker: "旅人(你)", text: option.label }
    ].concat(option.reply || []), "", state.cursor ? state.cursor.scene : null);
    if (result.result && result.result.ok) {
      ship3OpenIntermission("ch3-after-wind-choice", "▸ 進入船艙實驗");
    }
  });
  document.addEventListener("bd:start", function () {
    ship3CabinWindDialogueOpen = false;
  });
  ...
  function ship3SayDossierCorrection(args, result) {
    var d = state && state.lab && state.lab.caseFile && state.lab.caseFile.dossier;
    if (!d || !args || !args.assertionId || !result) return;
    var assertionId = args.assertionId;
    var option = ship3DossierScopeOptions(assertionId, d).filter(function (row) {
      return row[0] === args.choice;
    })[0];
    var selected = d.claimSelections && d.claimSelections[assertionId] || [];
    var groupNames = ship3DossierSourceGroups(d).filter(function (group) {
      return group.sourceIds.some(function (id) { return selected.indexOf(id) >= 0; });
    }).map(function (group) { return group.title; });
    var paperQuote = groupNames.length ? "「" + groupNames.join("」、「") + "」" : "眼前這批原紙";
    var playerText = "我用" + paperQuote + "提交這句：" + (option ? option[1] : ship3DossierAssertionText(assertionId, d));
    var scopeReplies = {
      S1: {
        "all-hand-aft": ["馬蒂厄", "把三張徒手紙攤成扇形", "你看，三張偏的方向並不一樣，手勢欄也各自不同。它們能質疑徒手放法，還不能說徒手一定把石頭送向船尾。"],
        "latch-solves-all": ["馬蒂厄", "按住門閂那組紙", "門閂只把我的手從放手裡拿掉。船怎麼走、石頭多高、誰在哪裡看，這張紙一項也沒有替我們免掉。"]
      },
      A1: {
        "all-moving": ["艾蒂安", "指著岸紙上等距的船位記號", "我這三張記的是船已走穩。起步和收槳的船位不是這樣排；這三張沒有替所有前進船況作答。"],
        "earth-moves": ["伽桑狄", "把斷言末句遮住", "原紙上有這艘船、這顆石頭和三次落點，沒有地球的運動。你能引用紙上量過的事，不能把沒量的反對也一起劃掉。"]
      },
      A2: {
        "all-motion-hidden": ["伽桑狄", "在六張紙上分別圈出停泊與平駛", "這六張只做了停泊和平駛。加速、減速沒有躲在紙裡；把它們一併寫進結論，範圍就越過原紙了。"],
        "wind-never-matters": ["伽桑狄", "把艙門那一欄推到最前面", "這批紙是在隔開甲板風後做的。它能說艙內結果，卻沒有量甲板那一趟受了多少風，不能替另一個場景判案。"]
      },
      A3: {
        "old-was-accelerating": ["維達爾船長", "用指節敲了敲八年前那張紙的空欄", "舊紙寫了後偏，沒寫逐拍船位。今天的起步紙再像，也不能替八年前補上一欄當時沒有的船速。"],
        "aft-reveals-all": ["維達爾船長", "把今天三種船況並排", "今天後偏跟著變快出現，只證明這一組比較。風、手推或別的船況都沒進這三張紙，看到後偏還不能只剩一個解釋。"]
      },
      S4: {
        "infer-old": ["維達爾船長", "把舊紙壓在三組新紙旁", "新紙知道自己的船況，舊紙不知道。樣子落在三種之中，不會替那張空白的船速欄生出答案。"],
        "universal-three-dots": ["伽桑狄", "在三組紙的條件欄下畫線", "這三組只改了船的狀態，沒有做風浪。後、中、前是今天三種條件的結果，不是所有擾動的總表。"]
      }
    };
    var scoped = scopeReplies[assertionId] && scopeReplies[assertionId][args.choice];
    if (result.reason === "dossier-scope-overread" && scoped) {
      sayWorkbenchCorrection(playerText, scoped[0], scoped[1], scoped[2]);
      return;
    }
    var sourceReplies = {
      "dossier-too-few-records": ["馬蒂厄", "逐張數過所選原紙", "這句要靠重做站穩。你現在勾到的同條件原紙還不到三張；少的不是措辭，是一次獨立重現。"],
      "dossier-dirty-release": ["馬蒂厄", "翻出每張紙的放手欄", "這幾張有徒手，也有門閂。放手方法跟著變，落點差異就不能全算在你要問的船況上。"],
      "dossier-speed-paper-missing": ["艾蒂安", "指著沒有逐拍船位的岸紙", "這張只記落點，沒有同拍船位。少了這一欄，我不能替你證明放手時船已走穩，或仍在變快。"],
      "dossier-variable-mismatch": ["伽桑狄", "把所選原紙的條件欄排成一列", "這幾張不只改了你要比較的那一項。先看船、石頭、高度和放手方法；有哪一欄沒有對齊，結論就不能只歸給一個原因。"],
      "dossier-comparison-missing": ["維達爾船長", "把兩側原紙各數了一次", "你的句子在比較兩種船況，可桌上只有一邊湊齊三張。另一邊沒進卷宗，這個比較就只有半句。"],
      "dossier-source-mismatch": ["伽桑狄", "把斷言放在所選原紙上方", "這句問的問題，和你勾的紙不是同一件事。先逐字看它談的是放手、船況還是觀察位置，再找真正量過那一項的紙。"]
    };
    var sourceReply = sourceReplies[result.reason];
    if (sourceReply)
      sayWorkbenchCorrection(playerText, sourceReply[0], sourceReply[1], sourceReply[2]);
  }
  ...
  /* notebook-renderer:ch1 — stable boundary for behavior contracts. */
  document.addEventListener("bd:notebook-snapshot", function (ev) {
    if (CHAPTER_ID !== "ch1" || !state || !ev.detail || !ev.detail.target) return;
    var lab = state.lab || {};
    ...
    var comparisons = lab.inference && lab.inference.comparisons || [];
    var target = ev.detail.target;
    target.innerHTML = "";
    ev.detail.handled = true;
    ship3El("p", "第一章斜面實驗紀錄", target, "shipNotebookSnapshotTitle");
    if (!runs.length && !claims.length && !assertions.length) {
      ship3El("p", "尚無實驗紀錄。", target, "hint");
      return;
    }
    var revealedRunIds = {};
    claims.forEach(function (claim) {
      (claim.runIds || []).forEach(function (id) { revealedRunIds[String(id)] = true; });
    });
    runs.forEach(function (run) {
      var card = ship3El("section", null, target, "shipNotebookSnapshotGroup");
      ship3El("b", "實驗紀錄 #" + run.id + "｜" + cfgLabel(run.config || {}), card);
      var visibleReadings = (run.readings || []).slice(0, 4).map(fmt);
      var fifth = revealedRunIds[String(run.id)] && (run.readings || []).length > 4
        ? fmt(run.readings[4]) : "封存中";
      ship3El("p", "前四段增量：" + visibleReadings.join("／") +
        "｜第五段：" + fifth, card);
    });
    if (claims.length) {
      ship3El("p", "第五段押記與判讀", target, "shipNotebookSnapshotTitle");
      claims.forEach(function (claim) {
        var card = ship3El("section", null, target, "shipNotebookSnapshotGroup");
        ship3El("b", "主張 #" + claim.id + "｜引用紀錄 " + (claim.runIds || []).join("、"), card);
        ship3El("p", "先押 " + fmt(claim.prediction) + "｜翻面 " +
          fmt(claim.observedFifth) + "｜" + (claim.ok ? "規律接上" : "與實測未接上"), card);
      });
    }
    if (assertions.length) {
      ship3El("p", "跨實驗斷言", target, "shipNotebookSnapshotTitle");
      assertions.forEach(function (assertion) {
        var label = assertion.type === "b" ? "只換球重" :
          (assertion.type === "c" ? "只換傾角" : assertion.type);
        var card = ship3El("section", null, target, "shipNotebookSnapshotGroup");
        ship3El("b", label + "｜主張 " + (assertion.claimIds || []).join("、"), card);
        ship3El("p", assertion.ok ? "斷言成立" : ("未成立：" + (assertion.reason || "資料不足")), card);
      });
    }
    if (comparisons.length) {
      ship3El("p", "比較草紙", target, "shipNotebookSnapshotTitle");
      comparisons.forEach(function (comparison, index) {
        var card = ship3El("section", null, target, "shipNotebookSnapshotGroup");
        ship3El("b", "比較 #" + (index + 1) + "｜紀錄 " +
          (comparison.runIds || []).join("、"), card);
        ship3El("p", "兩輪相異變因：" + ((comparison.diff || []).join("、") || "無"), card);
      });
    }
  });
  /* notebook-renderer:ch2 — stable boundary for behavior contracts. */
  document.addEventListener("bd:notebook-snapshot", function (ev) {
    if (CHAPTER_ID !== "ch2" || !state || !state.lab ||
        !ev.detail || !ev.detail.target) return;
    var lab = state.lab;
    var target = ev.detail.target;
    var series = lab.series || [];
    var log = lab.assemblyLog || [];
    var f2 = lab.evidence && lab.evidence.f2 || {};
    var hasWork = !!(series.length || log.length || lab.revision ||
      lab.calib && (lab.calib.releaseZero || lab.calib.rangeScale) ||
      f2.law || f2.ball);
    var ballNames = { copper:"銅球", wood:"木球" };
    var statusNames = { open:"進行中", complete:"已完成", abandoned:"已保留並中止" };
    var partNames = {
      shortGroove:"短斜槽", latchRelease:"門閂釋放", handRelease:"手放",
      polishedEdge:"打磨桌沿", roughEdge:"毛邊桌沿", rakedSand:"耙平沙盤",
      eyeBoard:"目測板", fineSandPlumb:"細沙加鉛垂規", liftSandbed:"升降沙盤"
    };
    var slotNames = {
      launcher:"發射槽", release:"釋放方式", edge:"桌沿", rangeBed:"落點量尺", heightRig:"高度架"
    };
    var profileNames = {
      clean:"條件乾淨", directionScatter:"方向散開", speedDrift:"初速漂移",
      coarseRead:"讀值太粗", notRunnable:"裝置未完成"
    };
    var rejectNames = {
      "non-scalar":"讀值不是可比較的單一數字", "bad-prediction":"封存預測無效"
    };
    var conceptNames = {
      sqrtScale:"射程隨落下高度的平方根改變",
      linearScale:"射程隨落下高度等比例改變",
      constantRange:"射程不受落下高度影響"
    };
    target.innerHTML = "";
    ev.detail.handled = true;
    ship3El("p", "第二章拋體研究紀錄", target, "shipNotebookSnapshotTitle");
    ship3El("p",
      "裝置調整、原始讀值、封存預測與玩家判讀都會留下；只有通過取得閘門的內容，才會在上方成為正式證據卡。",
      target, "shipNote");
    if (!hasWork) {
      ship3El("p", "尚無研究紀錄。", target, "hint");
      return;
    }
    if (log.length || lab.revision || lab.calib && (lab.calib.releaseZero || lab.calib.rangeScale)) {
      var apparatus = ship3El("section", null, target, "shipNotebookSnapshotGroup");
      ship3El("b", "裝置與校準｜版本 " + (lab.revision || 0), apparatus);
      var apparatusLines = [];
      if (lab.calib && lab.calib.releaseZero) apparatusLines.push("放球零點已校準。");
      if (lab.calib && lab.calib.rangeScale) apparatusLines.push("射程尺度已校準。");
      if (log.length) apparatusLines.push("已留下 " + log.length + " 次裝配、換件或校準動作。");
      ship3El("p", apparatusLines.join(" ") || "裝置仍在組裝。", apparatus);
      if (log.length) {
        var history = ship3El("details", null, apparatus);
        ship3El("summary", "查看裝配履歷（" + log.length + " 筆）", history);
        var historyList = ship3El("ol", null, history);
        log.forEach(function (entry) {
          var text = "版本 " + (entry.rev == null ? "—" : entry.rev) + "｜";
          if (entry.t === "calibrate") {
            text += (entry.kind === "releaseZero" ? "校準放球零點" : "校準射程尺度") +
              (entry.day == null ? "" : "｜第 " + entry.day + " 天");
          } else {
            text += (entry.t === "replace" ? "換件" : "裝上") + "｜" +
              (slotNames[entry.slot] || entry.slot) + "：" + (partNames[entry.part] || entry.part);
          }
          ship3El("li", text, historyList);
        });
      }
    }
    series.forEach(function (row) {
      var card = ship3El("section", null, target, "shipNotebookSnapshotGroup");
      ship3El("b", "第 " + row.id + " 輪｜" + (ballNames[row.ball] || row.ball) +
        "｜" + (statusNames[row.status] || row.status), card);
      var readingParts = [4, 9, 16].filter(function (height) {
        return row.readings && typeof row.readings[height] === "number";
      }).map(function (height) {
        return height + " 格→" + fmt(row.readings[height]);
      });
      ship3El("p", readingParts.length
        ? ("前三筆原始讀值：" + readingParts.join("／"))
        : "尚未留下射程讀值。", card);
      var setup = ship3El("details", null, card);
      ship3El("summary", "查看本輪裝置條件", setup);
      var setupList = ship3El("ul", null, setup);
      ship3El("li", "裝置版本：" + (row.apparatusRevision == null ? "未留存" : row.apparatusRevision), setupList);
      ship3El("li", "裝置指紋：" + (row.fingerprint || "未留存"), setupList);
      ship3El("li", "讀值輪型：" + (profileNames[row.profile] || row.profile || "未留存"), setupList);
      if (row.shapeError != null)
        ship3El("li", "前三筆形狀誤差：" + (Number(row.shapeError) * 100).toFixed(1) + "%", setupList);
      if (row.predictionError != null)
        ship3El("li", "第五筆預測誤差：" + (Number(row.predictionError) * 100).toFixed(1) + "%", setupList);
      if (row.rejectReason)
        ship3El("li", "退件原因：" + (rejectNames[row.rejectReason] || row.rejectReason), setupList);
      if (typeof row.prediction === "number") {
        var opened = row.readings && typeof row.readings[25] === "number";
        ship3El("p", "第五筆封存預測：" + fmt(row.prediction) + "｜" +
          (opened
            ? ("翻面實測 " + fmt(row.readings[25]) + "；" +
              (row.accepted ? "資料可引用" : "資料未通過門檻"))
            : "25 格尚未施作，實測仍封存"), card);
      }
      if (row.status === "abandoned")
        ship3El("p", "這輪沒有刪除；中止狀態與既有讀值一併保留。", card, "shipNote");
    });
    if (f2.law || f2.ball) {
      var judgment = ship3El("section", null, target, "shipNotebookSnapshotClaims");
      ship3El("b", "玩家判讀", judgment);
      var list = ship3El("ul", null, judgment);
      if (f2.law) {
        var lawSourceExists = series.some(function (row) {
          return String(row.id) === String(f2.lawSource);
        });
        ship3El("li", "規律判讀：" +
          (conceptNames[f2.lawConcept] || f2.lawConcept || "已成立") +
          "｜引用第 " + f2.lawSource + " 輪" +
          (lawSourceExists ? "" : "（本裝置未留存該輪紀錄）"), list);
      }
      if (f2.ball) ship3El("li", "換球比較已成立；銅球與木球使用同一裝置條件。", list);
    }
  });
  /* notebook-renderer:ch3 — stable boundary for behavior contracts. */
  document.addEventListener("bd:notebook-snapshot", function (ev) {
    ...
    if (!d) {
      ship3El("p", "本存檔未留存第三章卷宗紀錄。", target, "hint");
      return;
    }
    ...
    var commitments = d.designCommitments || {};
    if ((commitments.cabinWindAttempts || []).length ||
        (commitments.cabinInstrumentAttempts || []).length) {
      var design = ship3El("section", null, target, "shipNotebookSnapshotGroup");
      ship3El("b", "船艙對照設計", design);
      var designList = ship3El("ul", null, design);
      (commitments.cabinWindAttempts || []).forEach(function (attempt) {
        ship3El("li", "隔開甲板風：" + attempt.choice + "｜" +
          (attempt.ok ? "採用" : "留下缺口後改選"), designList);
      });
      (commitments.cabinInstrumentAttempts || []).forEach(function (choice) {
        ship3El("li", "船艙觀察物：" +
          ({ drip:"滴水", toss:"直拋石球", combined:"滴水與石球" }[choice] || choice), designList);
      });
    }
    if ((d.sourceAttempts || []).length || (d.scopeAttempts || []).length) {
      var attempts = ship3El("section", null, target, "shipNotebookSnapshotGroup");
      ship3El("b", "斷言嘗試與修正", attempts);
      ship3El("p", "選紙判讀 " + (d.sourceAttempts || []).length + " 次；範圍判讀 " +
        (d.scopeAttempts || []).length + " 次。失敗與縮限不會從卷宗刪除。", attempts);
      var attemptDetails = ship3El("details", null, attempts);
      ship3El("summary", "查看每次選紙與範圍判讀", attemptDetails);
      var attemptList = ship3El("ol", null, attemptDetails);
      (d.sourceAttempts || []).forEach(function (attempt) {
        ship3El("li", "選紙｜" + (attempt.assertion || "未記名斷言") + "｜引用 " +
          ((attempt.sources || []).join("、") || "未選原紙") + "｜" +
          (attempt.ok ? "可進入範圍判讀" : "未通過：" + (attempt.reason || "來源不合")), attemptList);
      });
      (d.scopeAttempts || []).forEach(function (attempt) {
        ship3El("li", "範圍｜" + (attempt.assertion || "未記名斷言") + "｜選「" +
          (attempt.choice || "未留存") + "」｜" +
          (attempt.ok ? "範圍成立" : "過度延伸，已保留後重選"), attemptList);
      });
    }
  ...
  /* notebook-renderer:ch4 — stable boundary for behavior contracts. */
  document.addEventListener("bd:notebook-snapshot", function (ev) {
    if (CHAPTER_ID !== "ch4" || !state || !state.lab ||
        !ev.detail || !ev.detail.target) return;
    var lab = state.lab;
    var target = ev.detail.target;
    var claims = lab.claims || {};
    var source = lab.sourceLab || {};
    var orbit = lab.orbitLab || {};
    var scale = lab.scaleLab || {};
    var planets = lab.planetLab || {};
    var models = lab.modelLab || {};
    var proof = lab.proof || {};
    var press = proof.press || {};
    var hasWork = !!(
      (source.attempts || []).length ||
      (source.tangentPrediction && source.tangentPrediction.sealed) ||
      (orbit.paperTrials || []).length || (orbit.ruleRuns || []).length ||
      (orbit.manualAttempts || []).length || (orbit.manualBeats || []).length ||
      (scale.predictionAttempts || []).length || (scale.conversionAttempts || []).length ||
      (scale.ratioAttempts || []).length || (scale.relationAttempts || []).length ||
      (planets.predictions || []).length || (planets.comparisonAttempts || []).length ||
      (models.rowOrder || []).length || (models.stampAttempts || []).length ||
      (models.loans || []).length || (models.comparisonAttempts || []).length ||
      (proof.slotAttempts || []).length || (proof.attributionAttempts || []).length ||
      (proof.boundaryAttempts || []).length || (press.proofs || []).length ||
      ["k1", "k2", "k3", "k4", "k5"].some(function (id) {
        return claims[id] && claims[id].length;
      })
    );
    function addCard(title, lines) {
      ...
      var list = ship3El("ul", null, card);
      lines.forEach(function (line) { ship3El("li", line, list); });
    }
    function addHistory(title, rows, format) {
      if (!rows || !rows.length) return;
      var details = ship3El("details", null, target, "shipNotebookSnapshotGroup shipNotebookHistory");
      ship3El("summary", title + "（" + rows.length + " 筆）", details);
      var list = ship3El("ol", null, details);
      rows.forEach(function (row, index) {
        ship3El("li", format(row || {}, index), list);
      });
    }
    ...
    if ((orbit.manualBeats || []).length)
      k1Lines.push("舊版續畫紙：已留下 " + orbit.manualBeats.length + " 拍；錯線也保留。");
    var k1Claim = claimLine("k1"); if (k1Claim) k1Lines.push(k1Claim);
    ...
    addHistory("查看 K1 舊版三拍續畫", orbit.manualAttempts || [], function (row, index) {
      return "版本 " + (index + 1) + "｜" + ((row.beats || []).length) + " 拍｜" +
        (row.complete ? "三拍都符合當時規則" : "含偏離規則的舊線") +
        (row.resetAt == null ? "" : "｜重畫次序 " + row.resetAt);
    });
    ...
    addHistory("查看 K2 倍率與關係判讀", (scale.ratioAttempts || []).concat(scale.relationAttempts || []), function (row, index) {
      return "第 " + (index + 1) + " 次｜選「" + (row.choice == null ? "未留存" : row.choice) + "」｜" +
        (row.ok ? "成立" : "未成立，保留後重判") + (row.at == null ? "" : "｜次序 " + row.at);
    });

    var planetNames = { mars:"火星", jupiter:"木星" };
    var k3Lines = [];
    (planets.predictions || []).filter(function (row) {
      return row && row.superseded !== true;
    }).forEach(function (row) {
    ...
    var loanNames = { loan:"留下借條", "no-loan":"不借" };
    var k4Lines = [];
    (models.rowOrder || []).forEach(function (caseId) {
      var row = models.rowStage && models.rowStage[caseId] || {};
      var force = row.forceStamp ? stampNames[row.forceStamp] : "待判讀";
      var vortex = row.vortexStamp ? stampNames[row.vortexStamp] : "待判讀";
      var loan = models.loanDecisions && models.loanDecisions[caseId];
      k4Lines.push((caseNames[caseId] || caseId) + "：拉力帳「" + force +
        "」、渦旋帳「" + vortex + "」" +
        (loan ? "；" + loanNames[loan] : "") + "。");
    });
    ...
    addHistory("查看 K4 蓋章履歷", models.stampAttempts || [], function (row, index) {
      return "第 " + (row.id || index + 1) + " 次｜" + (caseNames[row.caseId] || row.caseId) +
        "｜" + (row.model === "inverseSquare" ? "拉力帳" : "渦旋帳") +
        "蓋「" + (stampNames[row.stamp] || row.stamp || "未留存") + "」｜" +
        (row.ok ? "與固定判準相符" : "與固定判準不符") + (row.at == null ? "" : "｜次序 " + row.at);
    });
    addHistory("查看 K4 兩本帳比較履歷", models.comparisonAttempts || [], function (row, index) {
      return "第 " + (row.id || index + 1) + " 次｜主張「" + (row.claim || "未留存") + "」｜" +
        (row.ok ? "比較成立" : "比較未成立") + (row.at == null ? "" : "｜次序 " + row.at);
    });
    ...
    if ((press.proofs || []).length) {
      ...
      k5Lines.push("校樣：已送出 " + press.proofs.length + " 份；完整版本 " + completeProofs + " 份。");
    }
    var k5Claim = claimLine("k5"); if (k5Claim) k5Lines.push(k5Claim);
    addCard("K5 前的證明鏈與校樣", k5Lines);
    addHistory("查看 K5 證明鏈放紙履歷", proof.slotAttempts || [], function (row, index) {
      return "第 " + (row.id || index + 1) + " 次｜" + (row.slot || "未留存欄位") +
        " 放入 " + (row.evidenceId || "未留存來源") + "｜" + (row.ok ? "位置成立" : "位置不合") +
        (row.at == null ? "" : "｜次序 " + row.at);
    });
    addHistory("查看 K5 署名與範圍履歷",
      (proof.attributionAttempts || []).concat(proof.hookeScopeAttempts || [], proof.boundaryAttempts || []),
      function (row, index) {
        var choice = row.person ? ((row.contribution || "貢獻") + "→" + row.person) : row.choice;
        return "第 " + (row.id || index + 1) + " 次｜選「" + (choice || "未留存") + "」｜" +
          (row.ok ? "成立" : "已保留並修正") + (row.at == null ? "" : "｜次序 " + row.at);
      });
    addHistory("查看舊版與作廢校樣", (press.proofs || []).filter(function (row) {
      return row && (row.superseded === true || row.complete !== true);
    }), function (row, index) {
      return "校樣版本 " + (index + 1) + "｜" + (row.complete ? "當時完整" : "未完整") +
        (row.superseded ? "｜已作廢，由後一版取代" : "｜仍保留為失敗版本") +
        (row.submittedAt == null ? "" : "｜送印次序 " + row.submittedAt);
    });
  });
  /* notebook-renderer:ch5 — stable boundary for behavior contracts. */
  document.addEventListener("bd:notebook-snapshot", function (ev) {
    if (CHAPTER_ID !== "ch5" || !state || !state.lab ||
        !ev.detail || !ev.detail.target) return;
    var lab = state.lab;
    var target = ev.detail.target;
    var collisions = lab.collisionRuns || [];
    var clay = lab.clayRuns || [];
    var prediction = lab.clayPrediction;
    var assertions = lab.assertions || {};
    var judgments = lab.judgments || {};
    var headNames = { steel:"鋼頭", putty:"油灰頭" };
    var judgmentNames = {
      "both-close":"兩種碰撞的第一本帳都接近守恆",
      "steel-only":"只有鋼頭碰撞接近守恆",
      "putty-breaks":"油灰頭碰撞讓第一本帳對不上",
      "steel-close-putty-short":"鋼頭接近守恆，油灰頭出現短少",
      "both-short":"兩種碰撞都出現短少",
      "always-half":"油灰碰撞總會少一半",
      "changes-with-mass":"短少比例會隨質量配置改變",
      "same-amount":"改變質量後仍少相同數量",
      "speed-squared":"坑深較接近速度平方",
      "speed-linear":"坑深較接近速度",
      "height-independent":"坑深與落下高度無關"
    };
    var clayMassNames = { light:"輕球", heavy:"重球" };
    function addCard(title, lines) {
      if (!lines.length) return;
    ...
    function n(value) { return Number(value).toFixed(1).replace(/\.0$/, ""); }
    target.innerHTML = "";
    ev.detail.handled = true;
    ship3El("p", "第五章碰撞與黏土研究紀錄", target, "shipNotebookSnapshotTitle");
    ship3El("p",
      "原始碰撞數據、封存預測與玩家判讀分開保存。第二本帳在取得 J1 後才展開，不能從筆記提前看見。",
      target, "shipNote");
    if (!collisions.length && !clay.length && !prediction) {
      ship3El("p", "尚無研究紀錄。", target, "hint");
      return;
    }
    var collisionLines = collisions.map(function (row) {
      var line = "原紙 #" + row.id + "｜" + (headNames[row.head] || row.head) +
        "｜質量 " + row.mA + "/" + row.mB + "｜速度 A " + n(row.before.a) +
        "→" + n(row.after.a) + "、B " + n(row.before.b) + "→" + n(row.after.b) +
        "｜第一本帳 " + n(row.momentum.before) + "→" + n(row.momentum.after);
      if (lab.evidence && lab.evidence.j1)
        line += "｜第二本帳 " + n(row.visViva.before) + "→" + n(row.visViva.after) +
          "（短少 " + n(row.visViva.deficit) + "）";
      if (row.followupPrediction)
        line += "｜追一筆押記「" + (judgmentNames[row.followupPrediction] || row.followupPrediction) +
          "」；" + (row.predictionMatched ? "與結果相符" : "與結果不符");
      return line;
    });
    addCard("碰撞原紙", collisionLines);
    var collisionJudgments = ["j1", "j2", "followup"].filter(function (key) {
      return judgments[key];
    }).map(function (key) {
      var source = assertions[key] && assertions[key].sources;
      return key.toUpperCase() + "：" + (judgmentNames[judgments[key]] || judgments[key]) +
        (source && source.length ? "｜引用原紙 " + source.join("、") : "");
    });
    addCard("碰撞判讀", collisionJudgments);
    var clayLines = clay.map(function (row) {
      return "原紙 #" + row.id + "｜球重 " + (clayMassNames[row.ballMass] || row.ballMass || "未留存") +
        "｜落下高度 " + row.height + " 格｜速度 " + row.speed +
        "｜坑深 " + n(row.depth) + " " + row.unitDepth;
    });
    if (prediction) {
      clayLines.push("第三筆封存預測：" + n(prediction.min) + "～" + n(prediction.max) + " 分｜" +
        (prediction.revealed
          ? ("翻面實測 " + n(prediction.actualDepth) + " 分；" +
            (prediction.matched ? "落在封存範圍內" : "落在封存範圍外"))
          : "第三球尚未落下，實測仍封存"));
    }
    addCard("黏土原紙與第三筆預測", clayLines);
    if (judgments.j3)
      addCard("黏土判讀", ["J3：" + (judgmentNames[judgments.j3] || judgments.j3) +
        (assertions.j3 && assertions.j3.sources && assertions.j3.sources.length
          ? "｜引用原紙 " + assertions.j3.sources.join("、") : "")]);
  });
  /* notebook-renderer:ch6 — stable boundary for behavior contracts. */
  document.addEventListener("bd:notebook-snapshot", function (ev) {
    if (CHAPTER_ID !== "ch6" || !state || !state.lab ||
        !ev.detail || !ev.detail.target) return;
    var lab = state.lab;
    var target = ev.detail.target;
    var records = lab.records || [];
    var ledger = lab.sourceLedger || {};
    var finite = lab.finiteSources || {};
    var audit = lab.auditBoard || {};
    var joint = lab.jointPage || {};
    var sourceNames = { chips:"碎屑", cannon:"炮身", air:"空氣", water:"水箱" };
    var bandNames = { soon:"很快用盡", "within-shift":"一班內用盡", "no-endpoint":"不押終點" };
    var verdictNames = { fulfilled:"已兌現", "not-fulfilled":"未兌現", insufficient:"資料不足" };
    var conditionNames = {
      "rotation-only":"只轉動", "pressure-only":"只加壓", "contact-motion":"接觸並轉動",
      open:"進氣口開啟", sealed:"進氣口密合"
    };
    var modelPredictionNames = {
      "finite-sources":"若熱質有限，持續取熱應逐漸露出耗盡終點",
      "no-visible-cost":"若熱只從既有運動轉出，本段不必先押可見耗盡終點",
      "continued-motion":"只要接觸運動持續，升溫可以繼續",
      "single-impact":"若只靠一次撞擊，停止撞擊後不應持續升溫"
    };
    var airPredictionNames = {
      "slower-when-sealed":"密合進氣口後升溫會變慢",
      "faster-when-sealed":"密合進氣口後升溫會變快"
    };
    var conditionValueNames = {
      steady:"穩定", changed:"中途改變", fixed:"固定", "sixteen-beats":"每十六拍讀一次",
      missed:"漏讀", none:"無", true:"是", false:"否"
    };
    function addCard(title, lines) {
      if (!lines.length) return;
      var card = ship3El("section", null, target, "shipNotebookSnapshotGroup");
    ...
    function addDetails(title, lines) {
      if (!lines.length) return;
      var details = ship3El("details", null, target, "shipNotebookSnapshotGroup shipNotebookHistory");
      ship3El("summary", title + "（" + lines.length + " 筆）", details);
      var list = ship3El("ul", null, details);
      lines.forEach(function (line) { ship3El("li", line, list); });
    }
    function curve(row) {
      var values = row.curve || [];
      return values.map(function (value) { return value == null ? "缺值" : value; }).join("→");
    }
    target.innerHTML = "";
    ev.detail.handled = true;
    ship3El("p", "第六章熱研究紀錄", target, "shipNotebookSnapshotTitle");
    ship3El("p",
      "來源假說、封存預測、原始溫度線與判讀分層保存；尚未揭曉的結果不會因打開筆記而提前出現。",
      target, "shipNote");
    var hasLedger = !!ledger.sealed || Object.keys(ledger.placements || {}).some(function (key) {
      return !!ledger.placements[key];
    });
    var air = lab.airBench || {};
    var hasAirWork = !!(air.sealed || air.judged || (air.attempts || []).length);
    if (!hasLedger && !records.length && !hasAirWork && !finite.sealed && !audit.complete && !joint.complete) {
      ship3El("p", "尚無研究紀錄。", target, "hint");
      return;
    }
    var ledgerLines = [];
    Object.keys(ledger.placements || {}).forEach(function (source) {
      if (!ledger.placements[source]) return;
      ledgerLines.push((sourceNames[source] || source) + "：熱質位置「" + ledger.placements[source] +
        "」；若成立，應看到「" + ledger.consequences[source] + "」。");
    });
    if (ledger.sealed) {
      ledgerLines.push("熱質模型押記：" +
        (modelPredictionNames[ledger.modelPredictions && ledger.modelPredictions.caloric] || "未留存"));
      ledgerLines.push("運動模型押記：" +
        (modelPredictionNames[ledger.modelPredictions && ledger.modelPredictions.motion] || "未留存"));
      ledgerLines.push("兩套模型預測已封存，後續不能改寫。" );
    }
    addCard("來源追債簿", ledgerLines);
    var recordLines = records.map(function (row) {
      if (row.kind === "chip-comparison")
        return "原紙 #" + row.id + "｜碎屑／整片比較｜碎屑水溫 " + row.chipCurve.join("→") +
          "、整片水溫 " + row.plateCurve.join("→") + "｜" + (row.clean ? "條件可比較" : "條件不齊：" + row.dirtyReasons.join("、"));
      if (row.kind === "friction-condition")
        return "原紙 #" + row.id + "｜" + (conditionNames[row.condition] || row.condition) +
          "｜溫度 " + curve(row);
      if (row.kind === "dry-strip")
        return "原紙 #" + row.id + "｜乾鑽溫度 " + curve(row) + "｜" +
          (row.clean ? "條件固定" : "條件中途改變");
      if (row.kind === "air-comparison")
        return "原紙 #" + row.id + "｜" + (conditionNames[row.condition] || row.condition) +
          "｜溫度 " + curve(row) + "｜" + (row.clean ? "可比較" : "不可比較");
      if (row.kind === "water-box-preparation")
        return "原紙 #" + row.id + "｜水箱起始條件已讀取、等溫、密合並檢漏。";
      if (row.kind === "continuous-segment")
        return "原紙 #" + row.id + "｜連續第 " + row.segment + " 段｜" + row.minutes +
          " 分鐘｜" + row.temperature + "°｜馬速 " + row.horsePace + "、壓力 " + row.pressure +
          "、漏水 " + row.leak;
      return "原紙 #" + row.id + "｜" + row.kind;
    });
    addCard("原始讀值", recordLines);
    var conditionLines = records.map(function (row) {
      var conditions = row.conditions || {};
      if (row.kind === "chip-comparison") {
        return "原紙 #" + row.id + "｜碎屑 " + conditions.chipMass + " 份、整片 " + conditions.plateMass +
          " 份；起始溫度 " + conditions.chipTemp + "/" + conditions.plateTemp +
          "°；兩杯水 " + conditions.waterA + "/" + conditions.waterB + " 份，起始 " +
          conditions.waterTempA + "/" + conditions.waterTempB + "°。";
      }
      if (row.kind === "dry-strip") {
        return "原紙 #" + row.id + "｜馬速「" + (conditionValueNames[conditions.horsePace] || conditions.horsePace) +
          "」、壓力「" + (conditionValueNames[conditions.pressure] || conditions.pressure) +
          "」、讀值節拍「" + (conditionValueNames[conditions.sampling] || conditions.sampling) + "」。";
      }
      if (row.kind === "water-box-preparation") {
        return "原紙 #" + row.id + "｜水／炮身起始 " + conditions.water + "/" + conditions.cannon +
          "°；等溫「" + (conditions.equilibrated ? "是" : "否") + "」、密合「" +
          (conditions.sealed ? "是" : "否") + "」、檢漏「" + (conditions.leakChecked ? "是" : "否") +
          "」、固定取樣「" + (conditions.sampling ? "是" : "否") + "」。";
      }
      if (row.kind === "friction-condition" || row.kind === "air-comparison" || row.kind === "continuous-segment") {
        return "原紙 #" + row.id + "｜條件「" + (conditionNames[row.condition] || row.condition ||
          ("馬速 " + row.horsePace + "、壓力 " + row.pressure + "、漏水 " + row.leak)) + "」。";
      }
      return "";
    }).filter(Boolean);
    addDetails("查看每張原紙的固定條件", conditionLines);
    var benchLines = [];
    if ((lab.chipBench && lab.chipBench.attempts || []).length)
      benchLines.push("碎屑／整片比較：" + (lab.chipBench.judged ? "已判讀" : "已有原紙，尚待判讀"));
    if ((lab.frictionBench && lab.frictionBench.recordIds || []).length)
      benchLines.push("三種摩擦條件：" + (lab.frictionBench.judged ? "已判讀" : "已有原紙，尚待判讀"));
    if ((lab.dryBench && lab.dryBench.attempts || []).length)
      benchLines.push("乾鑽紙帶：" + (lab.dryBench.judged ? "已判讀" : "已有原紙，尚待判讀"));
    if (hasAirWork)
      benchLines.push("空氣對照：" + (air.judged ? "已判讀" : (air.sealed ? "預測已封存，尚待完成對照" : "進行中")));
    if (lab.waterBench && ((lab.waterBench.attempts || []).length || lab.waterBench.ready))
      benchLines.push("水箱：" + (lab.waterBench.ready ? "條件已完成並留紙" : "條件尚未齊備"));
    addCard("各工作台判讀狀態", benchLines);
    var predictionLines = [];
    if (air.sealed) predictionLines.push("密合進氣口預測：" +
      (airPredictionNames[air.prediction] || air.prediction || "未留存") + "｜" +
      (air.judged ? ("揭曉：" + (verdictNames[air.predictionOutcome] || air.predictionOutcome)) : "尚未判讀，結果仍封存"));
    Object.keys(finite.bands || {}).forEach(function (source) {
      if (!finite.bands[source]) return;
      var verdict = finite.verdicts && finite.verdicts[source];
      predictionLines.push((sourceNames[source] || source) + "耗盡押記：" +
        (bandNames[finite.bands[source]] || finite.bands[source]) + "｜" +
        (verdict ? ("揭曉：" + (verdictNames[verdict] || verdict)) : "判讀尚未完成"));
    });
    addCard("封存預測與揭曉", predictionLines);
    var auditLines = [];
    Object.keys(audit.placements || {}).forEach(function (slot) {
      auditLines.push(slot + "欄引用 " + audit.placements[slot]);
    });
    if (audit.latentDisposition) auditLines.push("潛在運動去向：仍未解決。");
    if (audit.complete) auditLines.push("稽核板已完成。" );
    addCard("證據稽核板", auditLines);
    var jointLines = [];
    Object.keys(joint.columns || {}).forEach(function (column) {
      if (joint.columns[column]) jointLines.push(column + "：" + joint.columns[column]);
    });
    if (joint.scopeDebt) jointLines.push("範圍債：仍未解決。" );
    if (joint.rateDebt) jointLines.push("換算債：尚未量得。" );
    if (joint.complete) jointLines.push("聯名頁已由四方署名。" );
    addCard("聯名帳頁", jointLines);
  });
  /* notebook-renderer:ch7 — stable boundary for behavior contracts. */
  document.addEventListener("bd:notebook-snapshot", function (ev) {
    if (CHAPTER_ID !== "ch7" || !state || !state.lab ||
        !ev.detail || !ev.detail.target) return;
    var lab = state.lab;
    var target = ev.detail.target;
    var engine = window.GB && window.GB.Engine7;
    var records = lab.records || [];
    var matrix = lab.matrix || {};
    var traces = matrix.traces || {};
    var commitment = lab.commitment || {};
    var verdicts = lab.verdicts || {};
    var integrity = lab.integrity || {};
    var active = integrity.activeWithholding;
    var matrixKeys = engine && engine.MATRIX_KEYS ||
      ["baseline", "bimetal", "sameMetal", "noMetal", "electrometer", "pile"];
    var configNames = {
      baseline:"外部刺激基準", bimetal:"黃銅與鐵的雙金屬接法",
      sameMetal:"同材質金屬接法", noMetal:"無金屬直接接觸",
      electrometer:"無蛙電量器", pile:"無動物組織的伏打堆"
    };
    var choiceNames = {
      "take-the-box":"接下整箱複驗", "sample-only":"只看一個漂亮樣本",
      M:"金屬獨有來源", A:"動物組織獨有來源", "not-yet":"先不押排他來源",
      "next-no-metal-contraction":"下一格只重做無金屬收縮",
      "next-tissue-free-charge":"下一格改做無組織電量器",
      "next-repeat-known":"下一格只重複已知配置",
      "mid-m-fell-a-open":"M 已倒下，A 仍待檢驗",
      "m-a-both-fell-record-configs-separately":"M 與 A 都失敗；不同配置分開記"
    };
    var materialNames = { copper:"銅片", zinc:"鋅片", brine:"浸鹽水布" };
    function addCard(title, lines) {
      if (!lines.length) return;
      var card = ship3El("section", null, target, "shipNotebookSnapshotGroup");
      ship3El("b", title, card);
    ...
    function recordById(id) {
      return records.find(function (row) { return row && row.id === id; }) || null;
    }
    function traceVisible(key) {
      if (active && active.kind === "claim-M-papers" && key === "noMetal") return false;
      return engine && typeof engine.visibleTrace === "function"
        ? engine.visibleTrace(lab, key)
        : !!traces[key] && !(active && active.kind === "trace" && active.id === key);
    }
    function archivalVisible() {
      return engine && typeof engine.visibleArchival1794 === "function"
        ? engine.visibleArchival1794(lab)
        : !!matrix.archival1794 && !(active &&
    ...
    ship3El("p",
      "每次接法、觀測、公開主張與修正都依發生順序保留。離桌中的原紙只標空位，不顯示內容；放回後原紙恢復，失信與修復歷史仍在。",
      target, "shipNote");
    var hasWork = records.length || matrixKeys.some(function (key) { return !!traces[key]; }) ||
      !!matrix.archival1794 || !!commitment.replicationAccepted || !!commitment.exclusiveClaim ||
      (commitment.attemptedClaims || []).length || (verdicts.attempts || []).length ||
      (integrity.incidents || []).length;
    if (!hasWork) {
      ship3El("p", "尚無研究紀錄。", target, "hint");
      return;
    }

    var matrixLines = matrixKeys.map(function (key) {
      var row = recordById(traces[key]);
      if (!row) return (configNames[key] || key) + "：尚未測試";
      if (!traceVisible(key)) return (configNames[key] || key) + "：原紙已離桌，待修復";
      return (configNames[key] || key) + "：" + (row.configNote || "配置未留存") +
        "｜觀測：" + (row.observation || "未留存") + (row.repeated ? "｜重做版本" : "｜首張原紙");
    });
    if (matrix.testedScope && matrix.testedScope.length)
      matrixLines.push("合帳時實際檢查的範圍：" + matrix.testedScope.map(function (key) {
        return configNames[key] || key;
      }).join("、"));
    if (matrix.boardOrder && matrix.boardOrder.length)
      matrixLines.push("上板次序：" + matrix.boardOrder.map(function (key) {
        return configNames[key] || key;
      }).join(" → "));
    addCard("六格接線矩陣", matrixLines);

    if (matrix.archival1794) {
      var archival = recordById(matrix.archival1794);
      addCard("一七九四年舊紙", [archivalVisible()
        ? ((archival && archival.configNote || "無金屬直接接觸") + "｜觀測：" +
          (archival && archival.observation || "原始記述未留存"))
        : "這張舊紙已離桌，內容待修復後才可再讀。"]);
    }

    var commitmentLines = [];
    records.filter(function (row) {
      return ["replication-commitment", "exclusive-claim", "next-config-commitment"].indexOf(row.kind) >= 0;
    }).forEach(function (row) {
      var value = row.choice || row.claim;
      commitmentLines.push("#" + row.seq + "｜" + (choiceNames[value] || value || "未留存") +
        (row.kind === "next-config-commitment" ? (row.discriminating ? "｜能區分兩種說法" : "｜不能區分兩種說法") : "") +
        (row.kind === "exclusive-claim" ? "｜處置：" + (row.disposition || "未留存") : ""));
    });
    addCard("事前承諾與公開主張", commitmentLines);

    var verdictLines = records.filter(function (row) {
      return row.kind === "mid-verdict" || row.kind === "final-verdict";
    }).map(function (row) {
      return "#" + row.seq + "｜" + (row.kind === "mid-verdict" ? "中途判讀" : "終局判讀") +
        "「" + (choiceNames[row.choice] || row.choice || "未留存") + "」｜" +
        (row.correct ? "成立" : "未成立，舊判讀保留");
    });
    addDetails("查看中途與終局判讀履歷", verdictLines);

    var operationLines = records.filter(function (row) {
      return ["matrix-trace", "archival-source", "replication-commitment", "exclusive-claim",
        "next-config-commitment", "mid-verdict", "final-verdict", "integrity-incident",
        "integrity-repair", "attempted-claim"].indexOf(row.kind) < 0;
    }).map(function (row) {
      if (row.kind === "electrometer-touch")
        return "#" + row.seq + "｜電量器接觸：" + (row.pair || "未接") + (row.valid ? "｜接法成立" : "｜接法未成立");
      if (row.kind === "electrometer-feed") return "#" + row.seq + "｜餵給薄盤第 " + row.feed + " 次接觸";
      if (row.kind === "pile-layer") return "#" + row.seq + "｜第 " + row.position + " 層放入「" +
        (materialNames[row.material] || row.material) + "」" + (row.reset ? "｜本次先重排" : "");
      if (row.kind === "pile-test") return "#" + row.seq + "｜測試兩端｜" +
        (row.sustained ? "持續反應" : "沒有形成持續反應") + "｜層序 " +
        (row.layers || []).map(function (item) { return materialNames[item] || item; }).join(" → ");
      if (row.kind === "board-placement") return "#" + row.seq + "｜把「" +
        (configNames[row.trace] || row.trace) + "」放入矩陣第 " + row.position + " 格";
      if (row.kind === "matrix-board") return "#" + row.seq + "｜六格合帳完成";
      if (row.kind === "claim-repair") return "#" + row.seq + "｜把公開主張限縮在原紙上";
      return "#" + row.seq + "｜" + (row.kind || "未命名操作");
    });
    addDetails("查看逐次操作與重做版本", operationLines);

    var incidentLines = records.filter(function (row) {
      return row.kind === "integrity-incident" || row.kind === "attempted-claim" || row.kind === "integrity-repair";
    }).map(function (row) {
      if (row.kind === "integrity-incident")
        return "#" + row.seq + "｜失信事故｜" + (row.reason || "原紙被收離桌面") +
          "｜對象 " + (row.targetId || "未留存");
      if (row.kind === "integrity-repair")
        return "#" + row.seq + "｜修復｜把原紙放回並在原句上限縮｜對應事故 " + row.incidentId;
      return "#" + row.seq + "｜曾試圖送出的主張「" + (choiceNames[row.claim] || row.claim || "未留存") +
        "」｜處置：" + (row.disposition || "未送出");
    });
    if (active) incidentLines.unshift("目前狀態：仍有原紙離桌，需先修復｜" + (active.sourceId || active.id));
    else if ((integrity.incidents || []).length) incidentLines.unshift("目前狀態：原紙已回桌；事故與修復史保留。");
    addCard("原紙誠信與修復史", incidentLines);
  });
  ...
  function renderShipDossierDesignDecision(parent, config) {
    ...
    config.options.forEach(function (option) {
      var button = ship3Btn(choices, option.label, function () {
        ...
        var accepted = config.accept();
        if (!accepted || accepted.error || (accepted.result && accepted.result.ok === false)) return;
        ship3Msg = "✓ " + option.feedback;
        if (option.reply) sayIntoDialogue(option.reply, "", state.cursor ? state.cursor.scene : null);
        renderAll();
        if (config.afterAccept) config.afterAccept(accepted);
      }, "shipAction");
  ...
  function renderShipDossierLab(work, d, lab) {
    ...
    if (mission.id === "cabin") {
      ...
      if (!cabinWindCommitted && !legacyCabinInProgress) {
        ship3PresentCabinWindDialogue(true);
        return;
      }
    ...
    if (mission.id === "explore") ship3DossierChoiceRow(settings, "用哪一艘船", "vesselId", ["small", "captain", "large"], {
      ...
      small: "船較輕，改變船速時由兩名港內水手划短槳。",
      captain: "現成可用；由四名維達爾號水手照原定操法划常槳。",
      large: "船較重，改變船速時由六名貨船水手划長槳。"
    });
  ...
  function renderShipDossier(v, box) {
    ...
    else renderShipDossierLab(work, d, lab);
  ...
  document.addEventListener("bd:embark", function (event) {
    ...
    if (CHAPTER_ID === "ch3" && detail.intermission === "ch3-after-dual-design") {
      ship3FocusDossierRunButton();
      return;
    }
  ...
  function orbit4SayCorrection(action, args, result) {
    args = args || {}; result = result || {};
    var key = result.reason || result.consequence;
    ...
    if (action === "stampLedgerCell" && key === "stamp-bounced") {
      ...
      var models = { inverseSquare:"拉力帳", simpleVortex:"渦旋帳" };
      var stamps = { matches:"對得上", story:"只有說法", mismatch:"對不上" };
      sayWorkbenchCorrection("我在「" + cases[args.caseId] + "」的「" + models[args.model] + "」蓋下「" + stamps[args.stamp] + "」。",
        "哈雷", "把章痕旁的原始結果重新露出來",
        "章不能替結果改口。這一格要看固定規則算出的數字、路徑或缺口；你剛蓋的字和紙上留下的結果不是同一件事。");
      return;
    }
    ...
    if (action === "connectCometTracks" && key === "comet-kink") {
      sayWorkbenchCorrection("我把兩疊星圖的端點直接接起來。", "哈雷", "沿接縫前後各量一次方向",
        "端點碰上了，日期與參考星留下的方向卻突然折了一角。天上沒有記下這次急轉；接法不能比觀測多畫一個動作。");
      return;
    }
    if (action === "setModelProtocol" && result.ok === false) {
      sayWorkbenchCorrection("我先用這套標準比較兩個模型。", "哈雷", "壓住兩張模型紙，只露出各自的規則欄與初始資料欄",
        "若替每顆天體重調規則，或逼所有天體共用同一個起點，我們比較到的就不是規則本身。先把『同一條規則』和『各自觀測到的起點』分開。");
      return;
    }
  ...
  function doOrbit(action, args, okText) {
    ...
    if (action === "commitOrbitBeat" && rr.ok === false) {
      ...
      var paperShape = rr.run.actualShape;
      var paperReply = {
    ...
    if (oldEvidence !== JSON.stringify(after)) {
      ...
      ["k1", "k2", "k3", "k4", "k5"].forEach(function (k) {
        if (!before[k] && after[k] && !evidenceCueIndex[k.toUpperCase()])
    ...
    [
      {
        ...
        title:"地表｜1 秒",
        note:"牛頓估算：約下落 4.9 公尺",
        alt:"教學重建紙底：地表物體一秒落距；精確文字由網頁標示"
      },
      {
  ...
  function orbit4ScaleGeometry(parent, sc) {
    ...
    draw("circle", { cx:105, cy:175, r:58, class:"orbitEarth" });
    draw("text", { x:105, y:181, "text-anchor":"middle", class:"orbitStringLabel orbitEarthLabel" }, "地球");
    draw("path", { d:"M500 184 Q493 119 455 65", class:"orbitScaleObserved" });
    draw("line", { x1:500, y1:184, x2:500, y2:58, class:"orbitScaleTangent" });
    draw("circle", { cx:500, cy:184, r:8, class:"orbitMoon" });
    draw("circle", { cx:500, cy:58, r:7, class:"orbitScaleEndpoint tangent" });
    draw("circle", { cx:455, cy:65, r:7, class:"orbitScaleEndpoint actual" });
    draw("line", { x1:493.1, y1:59.1, x2:462, y2:63.9, class:"orbitScaleSag" });
    draw("path", { d:"M462 63.9 L471 57.5 L472 68.2 Z", class:"orbitScaleSagHead" });
  ...
  function orbit4Svg(parent, lab, phase) {
    ...
    var legacyOrbitVisual = !!(o.ruleSeal || (o.ruleRuns || []).length ||
      (o.manualBeats || []).length || (o.manualAttempts || []).length);
    var activePaperTrial = (o.paperTrials || []).filter(function (run) {
      return run.id === o.paperTrialActiveId;
    })[0];
    ...
    if (phase === "planets") {
      ...
      if (phase === "vectors" && legacyOrbitVisual) {
      ...
      if ((o.deflectionVectors || []).length ||
          (legacyOrbitVisual && isFinite(o.aimAngle)) || showPaperInward) {
        ...
        if (legacyOrbitVisual && isFinite(o.aimAngle)) {
        ...
        if (showPaperInward) {
          var moonX = cx + last.x * scale;
          var moonY = cy - last.y * scale;
          var inwardX = cx - moonX, inwardY = cy - moonY;
          var inwardLength = Math.sqrt(inwardX * inwardX + inwardY * inwardY) || 1;
          var inwardUnitX = inwardX / inwardLength, inwardUnitY = inwardY / inwardLength;
          draw("line", {
            x1:moonX + inwardUnitX * 15, y1:moonY + inwardUnitY * 15,
            x2:moonX + inwardUnitX * 62, y2:moonY + inwardUnitY * 62,
            class:"orbitDeflection", "marker-end":"url(#orbit-inward-arrow)"
          });
          draw("text", {
            x:moonX + inwardUnitX * 34, y:moonY + inwardUnitY * 34 - 11,
    ...
    var cap = ship3El("figcaption", null, fig);
    cap.textContent = phase === "vectors" && (o.activeRule || activePaperTrial)
      ? ({
  ...
  function renderOrbit(v, box) {
    ...
    if (phase === "vectors") {
      ...
      if (!legacyOrbit) {
        ...
        ship3El("p", "一次只換一項，跑完的舊紙不會消失。", fixedRule, "orbitNote");
        var paperSpeed = orbit4BlankSelect(fixedRule, [
        ...
        ship3Btn(paperBox, "照這套規矩快速續畫", function () {
          ...
          doOrbit("runOrbitPaperTrial", {
            speed:paperSpeed.value, strength:paperStrength.value
          }, "✓ 這張設定與整條路都留在桌上；下一張只改一項，才有得比較。");
        }, "orbitAction primary");
        var paperTrials = o.paperTrials || [];
      ...
      if (o.ruleSeal && !o.continuedAt) {
          ...
          ship3Btn(turnRow, "落下這一拍的箭", function () {
            doOrbit("commitOrbitBeat", {}, function (rr) {
              return rr.manualComplete
                ? "✓ 三拍方向都站得住。現在把同一條規則快速外推，看路徑會撞地、繞住還是逃離。"
        ...
        if (o.manualComplete && (o.manualBeats || []).length === 3) {
          ship3Btn(work, "把三拍規則快速外推", function () {
    ...
    if (phase === "scale") {
      ...
      ship3El("p", "月球距離約 60 個地球半徑、週期約 27.3 日，來自天文尺度的教學整理。它只提供幾何輸入，不先提供一秒偏折答案。", scaleSources);
      ship3El("p", sc.conversionCorrect
        ? "現在只做一件事：並排讀地表與月球的一秒偏折。"
        : "現在只做一件事：先在下圖分清弧長與兩個端點之間的短差。",
        work,"orbitSealStatus");
      orbit4ScaleGeometry(work, sc);
      var timePaper = ship3El("section", null, work, "orbitScalePath time");
      ship3El("small", "月球紙｜你盯端點，牛頓核天文尺", timePaper);
      ...
      if (sc.conversionCorrect && !sc.ratioCorrect) {
        ...
        ship3El("small", "兩張一秒紙｜並排讀數", distancePaper);
      ...
      if (sc.ratioCorrect && !sc.relationCorrect) {
        ...
        ship3Btn(relationBox, "疊起來核對", function () {
          ...
          doOrbit("judgeScaleRelation", { choice:relationChoice.value },
            "✓ 你把 60 與約 3600 接起來了。兩張紙已保留，回到桌邊說清楚它最多支持到哪裡。");
        }, "orbitAction primary");
      }
      ...
      if (!proof.press.openingChoice) {
        ...
        if (ml.methodVersion==="hearing-v1" && !ml.protocolLocked) {
          ...
          ship3Btn(protocolBox,"採用這個比較方法",function(){
            ...
            doOrbit("setModelProtocol",{protocol:protocolChoice.value},function(rr){
              return (rr.ok?"✓ 公平標準鎖定｜":"✕ 這樣無法比較規則本身｜")+rr.note;
            });
          },"orbitAction primary");
          (ml.protocolAttempts||[]).forEach(function(attempt){
            ship3El("p",(attempt.ok?"✓ ":"✕ ")+attempt.note,protocolBox,
              attempt.ok?"orbitSealStatus":"orbitWarning");
          });
        }
        ...
        if (activeCase) {
          ...
          function rawLedgerText(model, run) {
            if (!run) return "尚未開列";
            if (model==="inverseSquare" && run.residual!=null)
              return run.note+"｜殘差 "+Number(run.residual).toFixed(2)+"%。";
            if (model==="inverseSquare") return run.note;
        ...
        if ((ml.completedRows || []).length) {
          ...
          orbit4Table(completedDetails,["資料","平方反比","簡化漩渦","事後補充"],(ml.completedRows || []).map(function(caseId){
            ...
            var loanDisplay="—";
            if(ml.loanDecisions[caseId]==="no-loan") loanDisplay="無";
            if(loan) loanDisplay=caseId==="planets"
              ?"有｜改表後貼合，但不是事前預測"
              :"有｜新增穿流假設，本批資料未獨立量到";
            return [
              caseNames[caseId],
        ...
        if ((ml.completedRows || []).length===3 && !ml.comparisonSealed) {
          ...
          var claimBoundary="這只比較同一規則跨資料的表現，不等於已說明拉力如何穿過空間。";
          var actualClaimText=!planetLoan&&!cometLoan
            ?"三份資料、兩套固定規矩：拉力帳三格都對得上；渦旋帳一格只有說法，另外兩格對不上。"+claimBoundary
  ...
  function collision5Error(code, result) {
    var map = {
      ...
      "clay-ball-locked": "第一球已落下，這一輪必須繼續使用同一顆球。",
      "clay-series-complete": "三個高度都已留下原紙；請用這三筆判讀關係。",
      "clay-prediction-required": "先用前兩筆封存第三球的坑深範圍，才能從九格落下。",
      "clay-prediction-not-applicable": "這份舊存檔沿用原來的自由落球流程，不倒補預測紙。",
      "clay-prediction-sealed": "第三球的預測已封存，不能改寫。",
      "unknown-clay-prediction": "這個坑深範圍不在預測紙上。",
      "clay-two-records-required": "先完成一格與四格兩筆讀數，才有依據預測第三球。",
  ...
  function collision5SayCorrection(action, args, code, result) {
    args = args || {};
    var cognitive = [
      "too-few-records", "both-heads-required", "mixed-masses", "mixed-speeds",
      "same-records-required", "both-ledger-outcomes-required", "j1-concept-mismatch",
      "j2-concept-mismatch", "too-few-clay-records", "mixed-ball-masses",
      "three-speeds-required", "j3-concept-mismatch", "square-relation-not-supported"
    ];
    if (cognitive.indexOf(code) < 0) return;
    var concept = args.concept || "";
    var conceptLabels = {
      "both-close":"鋼頭與油灰都閉合",
      "steel-only":"只有鋼頭閉合",
      "putty-breaks":"油灰使動量帳失效",
      "steel-close-putty-short":"鋼頭閉合、油灰短少",
      "both-short":"鋼頭與油灰都短少",
      "speed-squared":"坑深跟著速度平方走",
      "speed-linear":"坑深只跟著速度走",
      "height-independent":"坑深與速度沒有穩定關係"
    };
    var ids = (args.runIds || []).map(function (id) { return "#" + id; }).join("、");
    var playerText = "我用" + (ids || "這批") + "原紙，主張「" + (conceptLabels[concept] || concept || "這批紙已足夠") + "」。";
    var reply;
    var speaker = "杜佩院士";
    var actionText = "把所選原紙的條件欄逐張對齊";
    if (code === "too-few-records" || code === "too-few-clay-records")
      reply = "你這句想說的是能重現的關係，可現在勾到的獨立紀錄還不夠。兩個點能畫線，不能看出那條線穩不穩。";
    else if (code === "both-heads-required")
      reply = "你選的紙只留下其中一種碰法。沒有鋼頭和油灰同時在桌上，這句就不能替兩種收場一起作答。";
    else if (code === "mixed-masses" || code === "mixed-ball-masses")
      reply = "砝碼欄沒有鎖住。重量和碰法、速度一起變時，你不能把帳上的差異只歸給其中一項。";
    else if (code === "mixed-speeds")
      reply = "撞前速度也跟著換了。先選同一速度的兩種碰法；不然你是在拿兩個問題互相比。";
    else if (code === "same-records-required")
      reply = "第二本帳要重算第一本帳的同一批事件。你換了原紙，就分不清差別來自算法，還是來自資料被抽換。";
    else if (code === "both-ledger-outcomes-required")
      reply = "活力帳的句子同時談鋼頭與油灰，可你勾的紙沒有把一邊閉合、另一邊短少的兩種收場一起留下。";
    else if (code === "three-speeds-required")
      reply = "兩種速度只給你兩個點；一次方和平方都能穿過它們。第三種速度不是湊數，是用來分開兩種關係。";
    else if (code === "square-relation-not-supported")
      reply = "你選的三張坑深，除以速度平方後沒有聚在一起。先查球重與原紙，別讓一句漂亮的關係替數字站隊。";
    else {
      speaker = "杜夏特萊";
      actionText = "把兩本帳的同一列用尺壓住";
      reply = code === "j1-concept-mismatch"
        ? "兩種碰法的動量欄都在你眼前。若你的句子只承認一種，或把油灰當成帳外例外，就沒有讀完所選六張紙。"
        : (code === "j2-concept-mismatch"
          ? "這還是剛才那六張紙，只是換成活力帳。鋼頭與油灰的末欄沒有留下同一種收場；你的句子把差別抹平了。"
          : "把速度和坑深逐列並排。坑深增加得比速度本身快；你剛選的關係沒有跟著三個讀數一起走。");
    }
    sayWorkbenchCorrection(playerText, speaker, actionText, reply);
  }
  function collision5Do(action, args, okText, explicitFocusKey) {
    ...
    var r = N.labAction(state, action, args || {});
    if (r.error) {
      collision5Msg = "✕ " + collision5Error(r.error, r.result);
      collision5SayCorrection(action, args, r.error, r.result);
    }
    else {
      setState(r.state);
  ...
  function collision5ClayRangeLabel(min, max) {
    return min.toFixed(1) + "～" + max.toFixed(1) + " 分";
  }
  function collision5ClayPrediction(parent) {
    if (state.lab.clayProtocol !== "predict-third-v1") return;
    var prediction = state.lab.clayPrediction;
    var rows = state.lab.clayRuns;
    if (rows.length < 2) {
      ship3El("p", "先留下前兩筆讀數。第三球仍在架上，現在還不用猜。", parent, "collision5Fixed");
      return;
    }
    if (!prediction) {
      var factor = rows[0].ballMass === "heavy" ? 2 : 1;
      var wrap = ship3El("section", null, parent, "collision5Judgment");
      ship3El("b", "第三球將從九格落下（速度 6）。你預測坑深會落在哪一帶？", wrap);
      ship3El("p", "先讀前兩筆，再封存一個範圍；選項不告訴你哪個模型會贏。", wrap);
      [
        ["band-low", 2.8, 3.6],
        ["band-middle", 4.0, 4.8],
        ["band-high", 5.2, 6.0]
      ].forEach(function (option) {
        var min = option[1] * factor, max = option[2] * factor;
        var button = ship3Btn(wrap, collision5ClayRangeLabel(min, max), function () {
          collision5Do("sealClayPrediction", { band: option[0] },
            "預測已封存：九格第三球會落在 " + collision5ClayRangeLabel(min, max) + "。現在可以親手揭曉。",
            "clay-prediction-" + option[0]);
        }, "collision5JudgmentOption");
        button.setAttribute("data-c5-focus", "clay-prediction-" + option[0]);
      });
      return;
    }
    var card = ship3El("section", null, parent, "collision5Mission");
    ship3El("b", "封存預測｜" + collision5ClayRangeLabel(prediction.min, prediction.max), card);
    if (!prediction.revealed) {
      ship3El("p", "第三球尚未落下；預測紙已鎖住，不能改寫。", card);
      return;
    }
    ship3El("p", "揭曉｜實際坑深 " + prediction.actualDepth.toFixed(1) + " 分，" +
      (prediction.matched ? "落在封存範圍內。" : "落在封存範圍外。"), card);
    ship3El("small", "猜中或猜錯都不直接授證；請用三張實測原紙判斷較接近 v 還是 v²。", card);
  }
  function renderCollision5(v, box) {
    ...
    if (phase === "clay" || phase === "complete") {
      ...
      var clayNewProtocol = state.lab.clayProtocol === "predict-third-v1";
      var clayCount = state.lab.clayRuns.length;
      if (clayNewProtocol) {
        var nextClay = ["一格（速度 2）", "四格（速度 4）", "九格（速度 6）"][clayCount];
        ship3El("p", nextClay ? "下一球｜" + nextClay : "三個高度｜已完成", apparatus, "collision5Fixed");
      } else {
        collision5Select(apparatus, "落下高度", "clayHeight",
          [["h1", "一格（v=2）"], ["h4", "四格（v=4）"], ["h9", "九格（v=6）"]]);
      }
      collision5Select(apparatus, "球的質量", "ballMass",
        [["light", "輕球"], ["heavy", "重球"]], state.mode !== "scholar" || (clayNewProtocol && clayCount > 0));
      ship3El("p", "黏土盤｜已固定｜每次重新拍平", apparatus, "collision5Fixed");
      ship3El("p", "量深方式｜已固定｜同一支深度尺", apparatus, "collision5Fixed");
      if (!clayNewProtocol || clayCount < 2 || state.lab.clayPrediction) {
        var clayButtonLabel = clayNewProtocol && clayCount === 2
          ? "從九格落下第三球，揭曉預測"
          : (clayNewProtocol && clayCount < 2
            ? "從" + (clayCount === 0 ? "一格" : "四格") + "落下一球"
            : "放球入黏土（一次一筆）");
        ship3Btn(apparatus, clayButtonLabel, function () {
          collision5Do("runClay", {}, function (r) {
            return "✓ #" + r.record.id + "：速度 " + r.record.speed +
              "，坑深 " + r.record.depth.toFixed(1) + " 分。";
          });
        }, "collision5Action primary", clayNewProtocol && clayCount >= 3);
      } else {
        ship3El("p", "前兩筆已完成。請到右頁封存第三球的坑深範圍。", apparatus, "collision5Coach");
      }
    } else {
    ...
    var mission = {
      ...
      clay: {
        ...
        procedure: "做法：先量速度 2、4 的兩筆；封存速度 6 的坑深範圍，再親手落第三球揭曉。最後勾三張原紙，比較 v 與 v²。"
      },
      complete: {
    ...
    if (phase === "momentum") {
      ...
      collision5ClayPrediction(actions);
      if (state.lab.clayRuns.length >= 3 || state.lab.clayProtocol === "legacy-free-v0") {
  ...
  function heat6SayCorrection(action, args, code) {
    args = args || {};
    if (code === "source-ledger-mismatch") {
      sayWorkbenchCorrection(
        "我把「" + HEAT6_SOURCE[args.source] + "」掛到「" + args.position + "」，並寫下「" + args.consequence + "」。",
        "史坦格・鑽炮長", "把來源紙移回炮圖，沿實物逐一比對",
        "這張紙的來源位置和它聲稱會先改變的地方沒有接在同一條裝置上。先沿著實物走一次；來源若真在那裡，後果得能從那裡發生。"
      );
      return;
    }
    if (code === "source-prediction-band-mismatch") {
      sayWorkbenchCorrection(
        "我替「" + HEAT6_SOURCE[args.source] + "」放入「" + (HEAT6_BAND[args.band] || args.band) + "」的終點帶。",
        "凱斯勒院士", "把開場來源紙與終點帶上下疊齊",
        "這個來源版本自己說得出何時應該變弱，就不能換上一張永遠等不到的紙逃開檢驗；反過來，原本沒有終點的說法，也不能事後替它捏造期限。"
      );
      return;
    }
    if (code === "source-verdict-mismatch") {
      var verdicts = { "not-fulfilled":"預測未兌現", insufficient:"本輪尚未走到終點", fulfilled:"預測已命中" };
      var band = state.lab && state.lab.finiteSources && state.lab.finiteSources.bands[args.source];
      sayWorkbenchCorrection(
        "我判「" + HEAT6_SOURCE[args.source] + "｜" + HEAT6_BAND[band] + "」是「" + verdicts[args.verdict] + "」。",
        "凱斯勒院士", "把封存帶的終點壓在長時段曲線上",
        band === "no-endpoint"
          ? "這張紙當初沒有說何時會變弱。長紙走完仍不能叫它命中或落空；能寫的只有：這一輪還不足以碰到它自己沒有標出的終點。"
          : "這張紙先答應會在曲線走完前變弱。現在長紙已越過它自己的終點，升溫卻沒有照那句話轉弱；判讀要跟封口前的承諾對帳。"
      );
      return;
    }
    if (code === "audit-placement-mismatch") {
      var slots = { chips:"碎屑容量版本", air:"空氣必要來源版本", cannon:"炮身有限存量版本", water:"水中有限存量版本", condition:"持續升溫的操作條件" };
      sayWorkbenchCorrection(
        "我把「" + args.evidence + "」放進「" + slots[args.slot] + "」。",
        "史坦格・鑽炮長", "翻到那張原紙真正操作過的器材欄",
        "先看這張紙到底改了什麼、量了什麼。它若沒碰過這個來源，就不能只因結論順眼而塞進這一格。"
      );
      return;
    }
    if (code === "latent-disposition-mismatch") {
      var claims = {
        "proves-motion":"這些反例已經證明熱就是運動",
        "evidence-useless":"這些反例什麼都不能排除",
        "discard-caloric":"熱質說的所有版本都可以刪掉"
      };
      sayWorkbenchCorrection("我在稽核板寫下：" + (claims[args.disposition] || args.disposition) + "。",
        "凱斯勒院士", "把四張被擊中的有限來源紙和仍空白的機制欄分開",
        args.disposition === "evidence-useless"
          ? "它們已經排除了幾個把熱放在有限來源裡的版本，不能說毫無作用；只是排除一個來源，和證明熱究竟是什麼，仍是兩件事。"
          : "你從這批反例拿走的是幾個有限來源版本，不是熱的本體。運動說還欠自己的正面測量；我也不能替所有熱質說一次簽退。"
      );
      return;
    }
    if (code === "unknown-joint-column") {
      sayWorkbenchCorrection("我先把這項責任寫進「" + args.column + "」欄。",
        "史坦格・鑽炮長", "把四個人的操作、讀數與主張紙分開",
        "誰碰過裝置、誰讀過數字、誰主張模型，都有自己的紙。這一欄不能替另一個人簽他沒有做過的事。"
      );
    }
  }
  ...
  var EM1_LABEL = {
    ...
    bimetal: "雙金屬｜黃銅鉤—鐵片",
    sameMetal: "同材質｜單金屬弧",
    noMetal: "無金屬｜神經—肌肉直接接觸",
    electrometer: "無蛙｜銅鋅接觸與電量器",
    pile: "無動物組織｜銅鋅鹽水布疊層"
  };
  var EM1_FIRST_REPLY = {
    baseline: "Galvani：牠今天肯作證。先確認證人醒著，再開庭。接下來別再用它——用它，就分不清是誰在說話。",
    bimetal: "Galvani：跟我一七九一年那頁，一個樣。複驗一次，舊紙就年輕一次。",
    sameMetal: "Galvani：跟上一格，差在哪一欄？",
    noMetal: "Galvani：一七九四年。我做過。今天你重做出來了——謝謝你。這格從今天起有兩張紙。"
  };
  var EM1_REPEAT_REPLY = {
    baseline: "Galvani：牠還醒著。記。",
    bimetal: "Galvani：跟上次一樣？一樣也記。",
    sameMetal: "Galvani：差多少，寫多少。",
    noMetal: "Galvani：複驗不嫌多。這格的紙，越厚越好。"
  };
  function em1El(name, text, parent, className) {
    var el = document.createElement(name);
    if (text !== null && text !== undefined) el.textContent = displayText(text);
    if (className) el.className = className;
    parent.appendChild(el);
    return el;
  }
  function em1Do(action, args, successText) {
    var r = N.labAction(state, action, args || {});
    if (r.error) {
      em1Msg = "✕ " + r.error;
      renderAll();
  ...
  function em1WithholdButton(card, lab, key) {
    var sourceId = "ch7:withdraw-trace:" + key;
    if (!lab.matrix.traces[key] || em1IncidentUsed(lab, sourceId) ||
        !(window.GB.Engine7 && window.GB.Engine7.visibleTrace(lab, key))) return;
    mkBtn(card, "把這格的紙收離桌面", function () {
      em1PendingConfirm = { kind: "trace", key: key };
      renderAll();
    });
    if (em1PendingConfirm && em1PendingConfirm.kind === "trace" && em1PendingConfirm.key === key) {
      var confirmTrace = em1El("div", null, card, "em1Confirm");
      confirmTrace.setAttribute("role", "group");
      confirmTrace.setAttribute("aria-label", "確認收離這格原紙");
      em1El("p", "這格是你親手做、親手記的。桌上會留下一個空位。", confirmTrace);
      mkBtn(confirmTrace, "收。", function () { em1Do("withdrawMatrixTrace", { trace: key }); });
      mkBtn(confirmTrace, "留著。", function () { em1PendingConfirm = null; renderAll(); });
    }
  }
  function em1Gate(box, v) {
    var gate = em1El("div", null, box, "em1Gate " + (v.ready ? "ready" : "pending"));
    if (!v.ready) {
      em1El("p", "還缺一筆能讓劇情繼續的原紙。", gate);
      return;
    }
    mkBtn(gate, "▶ 把這組原紙帶回對話", function () {
      var r = N.embedComplete(state);
      if (r.error) { em1Msg = "✕ " + r.error; renderAll(); return; }
      setState(r.state);
      addLine("system", "(互動段落完成)", "system");
      em1Msg = "";
      renderAll();
  ...
  function renderEm1(v, box) {
    var lab = state.lab;
    box.className = "em1Workbench";
    em1El("h2", "接線矩陣｜配置與觀測分開記", box);
    em1El("p", v.text || v.hint || "每一格都留下接法與觀測；不先替它命名勝負。", box, "em1Lead");
    if (em1Msg) em1El("div", em1Msg, box, "labmsg");

    if (v.phase === "matrix") {
      em1El("p", "順序由你決定。重做會增加原紙，但不會洗掉第一次。", box);
      ["baseline", "bimetal", "sameMetal", "noMetal"].forEach(function (key) {
        var card = em1El("section", null, box, "em1Card");
        em1El("h3", EM1_LABEL[key], card);
        var record = lab.matrix.traces[key] && lab.records.find(function (row) { return row.id === lab.matrix.traces[key]; });
        if (record) {
          em1El("p", "配置：" + record.configNote, card);
          em1El("p", "觀測：" + record.observation, card);
          var count = lab.records.filter(function (row) { return row.kind === "matrix-trace" && row.config === key; }).length;
          em1El("small", "原紙 " + count + " 張｜具名留痕已封存", card);
        } else em1El("p", "尚未操作；這格沒有結果。", card);
        mkBtn(card, record ? "再做一次並另留原紙" : "親手接線並留痕", function () {
          em1Do("runEm1Config", { config: key }, function (result) {
            return result.firstTrace ? EM1_FIRST_REPLY[key] : EM1_REPEAT_REPLY[key];
          });
        });
        em1WithholdButton(card, lab, key);
        if (key === "noMetal" && lab.matrix.archival1794 &&
            !em1IncidentUsed(lab, "ch7:conceal-1794") &&
            window.GB.Engine7.visibleArchival1794(lab)) {
          mkBtn(card, "把一七九四年的舊紙收進箱底", function () {
            em1PendingConfirm = { kind: "archival1794" };
            renderAll();
          });
          if (em1PendingConfirm && em1PendingConfirm.kind === "archival1794") {
            var confirmArchival = em1El("div", null, card, "em1Confirm");
            confirmArchival.setAttribute("role", "group");
            confirmArchival.setAttribute("aria-label", "確認收起一七九四年舊紙");
            em1El("p", "這一頁是四年前的觀測。收起來，桌上就沒有紙證明「沒有金屬也踢」發生過。", confirmArchival);
            mkBtn(confirmArchival, "收。", function () { em1Do("concealArchival1794", {}); });
            mkBtn(confirmArchival, "放回桌上。", function () { em1PendingConfirm = null; renderAll(); });
          }
        }
      });
    } else if (v.phase === "electrometer") {
      em1El("p", "Volta 已退開。接觸、餵盤與讀針都由你的手完成。", box);
      var e = lab.electrometer;
      em1El("p", "金屬對：" + (e.pair || "尚未選") + "｜餵盤次數：" + e.feeds + "｜提盤：" + (e.lifted ? "已完成" : "未完成"), box);
      mkBtn(box, "讓銅碰鋅", function () { em1Do("touchElectrometerMetals", { pair: "copper-zinc" }, "銅與鋅已相觸；接觸嘗試留在紙上。" ); });
      mkBtn(box, "讓銅碰銅（對照）", function () { em1Do("touchElectrometerMetals", { pair: "copper-copper" }); });
      mkBtn(box, "把這一次接觸餵給薄盤", function () { em1Do("feedElectrometerPlate", {}, "薄盤多記下一次接觸。" ); }, !e.touched);
      mkBtn(box, "提起薄盤，讀細針", function () { em1Do("liftElectrometerPlate", {}, "細針動了。很小，但乾淨；針格已留痕。" ); }, e.feeds < 1 || !!lab.matrix.traces.electrometer);
    } else if (v.phase === "pile") {
      em1El("p", "依圖說從底部往上排：銅 → 鋅 → 浸鹽水布，至少重複兩組。", box);
      em1El("p", "目前層序：" + (lab.pile.layers.length ? lab.pile.layers.map(function (m) {
        return { copper: "銅", zinc: "鋅", brine: "浸鹽水布" }[m];
      }).join(" → ") : "（空）"), box);
      [["copper", "加一片銅"], ["zinc", "加一片鋅"], ["brine", "加一片浸鹽水布"]].forEach(function (item) {
        mkBtn(box, item[1], function () { em1Do("setPileLayer", { material: item[0] }, "這一層已留在排列紀錄。" ); });
      });
      mkBtn(box, "清空並從銅重新開始", function () { em1Do("setPileLayer", { material: "copper", reset: true }, "舊排列仍在嘗試紀錄；桌面從一片銅重新排。" ); });
      mkBtn(box, "同觸頂端與底端", function () { em1Do("testPileEnds", {}, function (result) {
  ...
  function renderAll() {
    ...
    if (v.type === "end" || state.ended) { /* B-4:終幕文字由 scenes 資料單一來源輸出,UI 僅補狀態行 */
      addLine("system", "(本章筆記與研究紀錄已存。)", "system");
      save();
      return;
    }
    if (v.type === "choice") {
      ...
      var btn = mkBtn(box, "▶ 繼續", function () {
        ...
        if (r.error) { addLine("system", r.error, "system"); return; }
        setState(r.state);
        if (r.node) addLine(r.node.speaker, r.node.text, classFor(r.node.speaker), sourceScene, r.node.id);
        renderAll();
      });
      btn.focus();
    }
  ...
  function configureSeriesTitle() {
    ...
    var hasExplicitChapter = /(?:^|[?&])chapter=/.test(window.location.search || "");
    document.title = hasExplicitChapter
      ? "《發現之前》" + current.label + "：" + current.title + "｜互動物理史遊戲"
      : "發現之前 Before Discovery｜互動物理史遊戲";
    var progress = readSeriesProgress();
  ...
  function initTitle() {
    ...
    if (window.BD_CHAPTER_ERROR) {
      showNewWarn(window.BD_CHAPTER_ERROR + "。已停止載入；請返回旅程目錄重新選章。");
