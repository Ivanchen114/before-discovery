...
tests.push({
  ...
  fn: () => {
    const finalNode = scenes6.scenes.find((scene) => scene.id === "HE-1")
      ?.nodes.find((node) => node.id === "x_final");
    const finalOption = finalNode?.options?.find((option) => option.id === "sign-unmeasured-rate");
    if (finalNode?.type !== "choice" || finalNode.options.length !== 1 ||
        finalOption?.next !== "x_signed" ||
        finalOption.effects?.[0]?.labAction?.action !== "finalizeJointPage")
      throw new Error("x_final 不再是玩家親手簽署、再顯示封口結果的單選儀式");
    let state = ch6ReadyForFinitePredictions();
    state = Engine6.sealFinitePredictions(state).state;
...
tests.push({
  ...
  fn: () => {
    ...
    for (const scene of scenes6.scenes) {
      for (let index = 0; index < scene.nodes.length; index += 1) {
        ...
        for (const effect of node.effects || [])
          if (effect.evidence) {
            if (!evidenceAt.has(effect.evidence)) evidenceAt.set(effect.evidence, new Set());
            evidenceAt.get(effect.evidence).add(key);
          }
      }
    }
    ...
    function evidenceDominates(targetKey, evidenceId) {
      const grants = evidenceAt.get(evidenceId);
      if (!grants?.size) return false;
      const seen = new Set();
      const queue = [start];
      while (queue.length) {
        const key = queue.shift();
        if (seen.has(key) || grants.has(key)) continue;
        if (key === targetKey) return false;
        seen.add(key);
        for (const [next, preds] of predecessors)
          if (preds.has(key) && !seen.has(next)) queue.push(next);
      }
      return true;
    }
    ...
    for (const decision of scenes6.decisionRegistry) {
      ...
      for (const option of decision.options) {
        ...
        for (const ref of option.refutedBy) {
          const sourceKey = ref.includes("/") ? ref : null;
          if (sourceKey && !nodeByKey.has(sourceKey))
            throw new Error("refutedBy 無法解析:" + decision.id + "/" + option.id + "→" + ref);
          if (!sourceKey && !evidenceAt.has(ref))
            throw new Error("refutedBy 無法解析:" + decision.id + "/" + option.id + "→" + ref);
          if (sourceKey && nodeByKey.get(sourceKey).scene.id === decision.scene && nodeOrder.get(sourceKey) >= nodeOrder.get(decisionKey))
            throw new Error("refutedBy 指向同場作答後節點:" + decision.id + "/" + option.id + "→" + ref);
          if (sourceKey && !dominators.get(decisionKey).has(sourceKey))
            throw new Error("refutedBy 尚未見:" + decision.id + "/" + option.id + "→" + ref);
          if (!sourceKey && !evidenceDominates(decisionKey, ref))
            throw new Error("refutedBy 尚未見:" + decision.id + "/" + option.id + "→" + ref);
        }
      }
    }
...
tests.push({
  name: "器材踏查|四章配置完備+紙件判讀可用精簡踏查+舊存檔相容",
  fn: () => {
    const assets = require("../data/assets.js");
    const ids = new Set(assets.entries.map((e) => e.id));
    for (const key of ["ch1:A2-2", "ch2:B2-3", "ch3:C1-1", "ch5:E1-1"]) {
    ...
    const c5 = assets.apparatusBriefings["ch5:E1-1"];
    if (c5.itemNoun !== "紙件" || c5.triggerNode !== "q1" || c5.items.length !== 3 ||
        !["dupreLedger", "visVivaLedger", "clayReport"].every((id) =>
          c5.items.some((item) => item.id === id)))
      throw new Error("第五章沒有在公平比較前踏查三份來源紙");
    if (!stageEvents.includes("storySurvey.triggerNode === d.nodeId"))
      throw new Error("純對話節點沒有接上精簡紙件踏查");
    const stageInputs = readFileSync(path.join(here, "../src/stage/07-intro-inputs.part.js"), "utf-8");
    for (const frag of ["source.itemNoun || \"器材\"", "檢查\" + cfg.itemNoun"])
      if (!stageInputs.includes(frag)) throw new Error("踏查仍把所有物件硬稱為器材:" + frag);
  }
});

tests.push({
  name: "第五章章首|先辨識時空與帳意，再看懂兩種記法並決定公平比較",
  fn: () => {
    const scene = (id) => scenes5.scenes.find((item) => item.id === id);
    const node = (sceneId, nodeId) => scene(sceneId).nodes.find((item) => item.id === nodeId);
    const arrival = node("E0-1", "x_arrival");
    if (!arrival || !arrival.text.includes("這又是哪裡") || !arrival.text.includes("哪一年") ||
        node("E0-1", "n2").next !== "x_time")
      throw new Error("第五章章首仍直接跳進西雷書房，旅人沒有辨識時空");
    const accountQuestion = node("E0-2", "x3a");
    const accountDefinition = node("E0-2", "x3b");
    for (const frag of ["不是在算錢", "帳"])
      if (!accountQuestion.text.includes(frag)) throw new Error("旅人沒有追問帳的意思:" + frag);
    for (const frag of ["碰撞", "之前", "之後", "對平"])
      if (!accountDefinition.text.includes(frag)) throw new Error("帳／對平定義缺關鍵語意:" + frag);
    const bridge = node("E1-1", "x1b");
    for (const frag of ["質量乘速度", "再乘一次速度", "mv²"])
      if (!bridge.text.includes(frag)) throw new Error("mv² 仍未由白話記法搭橋:" + frag);
    const question = node("E1-1", "q1");
    const correct = question.options.find((option) => option.id === "ledger");
    if (question.text !== "要怎麼公平比較兩本帳？" || !correct ||
        !correct.text.includes("同一批碰撞紀錄") || !correct.text.includes("碰撞前後"))
      throw new Error("玩家沒有親手決定同批紀錄、各自比較前後的公平規矩");
    if (!node("E1-1", "bad1").text.includes("算法不同") ||
        !node("E1-1", "bad1").text.includes("不能直接比"))
      throw new Error("直接比數字的錯項沒有得到人物層解釋");
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    for (const [sc, treatment] of Object.entries(assets.sceneTreatment || {})) {
      if (!sceneIds.has(sc)) throw new Error("sceneTreatment 指向不存在場景:" + sc);
      if (treatment.mode !== "time-passage")
        throw new Error("sceneTreatment 未知模式:" + treatment.mode);
      if (!String(treatment.label || "").trim() || !String(treatment.caption || "").trim())
        throw new Error("短幕時間折疊缺年份錨點或短標:" + sc);
      if (assets.sceneFx && assets.sceneFx[sc])
        throw new Error("同一場景不得同時使用短幕折疊與重大時間蒙太奇:" + sc);
    }
    const dIntTreatment = assets.sceneTreatment && assets.sceneTreatment["D-INT-1"];
    if (!dIntTreatment || dIntTreatment.label !== "1665 → 1679" ||
        dIntTreatment.caption !== "十四年，折進一頁")
      throw new Error("第四章十四年幕間缺資料驅動的短幕時間折疊");
    for (const fragment of [
      'id="timePassageVeil"', 'id="timePassageLabel"', 'id="timePassageCaption"',
      "body.scene-treatment-time-passage #timePassageVeil", "z-index: 24",
      "#dialogue { position: absolute; z-index: 25",
      'body.scene-treatment-time-passage[data-view="choice"] #panelWrap { z-index: 26; }',
      'body.scene-treatment-time-passage[data-view="choice"] #timePassageMark {',
      'transform: translate(-31vw, 12px);'
    ]) if (!stageHtml.includes(fragment))
      throw new Error("短幕時間折疊沒有把近黑世界留在對話框與選項後方:" + fragment);
    for (const fragment of [
      "function applySceneTreatment(sceneId)", 'treatment.mode === "time-passage"',
      'body.classList.toggle("scene-treatment-time-passage"',
      '$("timePassageLabel").textContent', '$("timePassageCaption").textContent'
    ]) if (!sui.includes(fragment))
      throw new Error("短幕時間折疊未由共用場景資料驅動:" + fragment);
    const treatmentStart = sui.indexOf("function applySceneTreatment(sceneId)");
    const treatmentEnd = sui.indexOf("function setScene(sceneId)", treatmentStart);
    const treatmentRuntime = treatmentStart >= 0 && treatmentEnd > treatmentStart
      ? sui.slice(treatmentStart, treatmentEnd) : "";
    if (!treatmentRuntime || /setTimeout|setInterval|requestAnimationFrame/.test(treatmentRuntime))
      throw new Error("短幕時間折疊不得以固定時間代玩家推進");
    if (!stageHtml.includes("#timePassageVeil, #timePassageMark, #hud, #sceneChip { transition: none; }"))
      throw new Error("短幕時間折疊缺 prefers-reduced-motion 即時狀態");
...
tests.push({
  ...
  fn: () => {
    ...
    const a22n2a = a22.nodes.find((n) => n.id === "n2a");
    const a22n2b = a22.nodes.find((n) => n.id === "n2b");
    const a22n2c = a22.nodes.find((n) => n.id === "n2c");
    const a22n2d = a22.nodes.find((n) => n.id === "n2d");
    const qOdd = a22.nodes.find((n) => n.id === "qOdd");
    const qCum = a22.nodes.find((n) => n.id === "qCum");
    if (a22n2?.next !== "n2a" || a22n2a?.next !== "n2b" ||
        a22n2b?.next !== "n2c" || a22n2c?.next !== "n2d" || a22n2d?.next !== "e1" ||
        !String(a22n2?.text || "").includes("前四段") ||
        !String(a22n2b?.text || "").includes("第五段") ||
        !String(a22n2b?.text || "").includes("先蓋") ||
        !String(a22n2b?.text || "").includes("先說中") ||
        !String(qOdd?.text || "").includes("第五段翻開了") ||
    ...
    const n4 = a22.nodes.find((n) => n.id === "n4");
    const n4a = a22.nodes.find((n) => n.id === "n4a");
    if (!n3 || n3.next !== "qCum" ||
        qOdd.options.find((o) => o.id === "odd")?.next !== "n3" ||
        qCum.options.find((o) => o.id === "squares")?.next !== "n3b" ||
        !n3b || n3b.speaker !== "stage" || !n3b.text.includes("兩支筆") || n3b.next !== "n4" ||
        !n4?.text.includes("時間刻度乘上自己") || n4.next !== "n4a" ||
        !n4a?.text.includes("從第五格倒算回第一格") || n4a.next !== "n4b")
...
tests.push({
  ...
  fn: () => {
    ...
    const scenesJson = JSON.parse(readFileSync(path.join(here, "../data/scenes.json"), "utf-8"));
    const e1Spoken = scenesJson.scenes.find((s) => s.id === "E-1").nodes
      .map((n) => TextFormat.normalizeZhPunctuation(n.text || "")).join("\n");
    if (!e1Spoken.includes("它往前，又往下——兩件事，同時發生。"))
...
tests.push({
  ...
  fn: () => {
    ...
    for (const frag of ['$("continueMeta").textContent', 'loaded.mode === "scholar"', '研究進度已保留'])
      if (!cui.includes(frag)) throw new Error("首頁進度未標明模式/研究紀錄:" + frag);
...
tests.push({
  name: "天數退居內部研究歷程|公開舞台不再顯示行政數字",
  fn: () => {
    const cui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    const scenes1 = readFileSync(path.join(here, "../data/scenes.json"), "utf-8");
    const scenes2 = readFileSync(path.join(here, "../data/scenes2.json"), "utf-8");
    const decisions = readFileSync(path.join(here, "../decisions.md"), "utf-8");
    for (const shellName of ["stage.html", "chapter.html", "chapter2.html", "chapter3.html"]) {
      const shell = readFileSync(path.join(here, "../" + shellName), "utf-8");
      for (const frag of [
        'id="dayVal"', "第幾天", "每次實驗都花天數", "天數=實驗成本"
      ]) if (shell.includes(frag)) throw new Error(shellName + " 仍顯示天數資源:" + frag);
    }
    for (const frag of [
      "總耗天數", '"第 " + lab2.days + " 天"', "第一次借用多花一天",
      '"・第 " + loaded.lab.days + " 天"'
    ]) if (cui.includes(frag)) throw new Error("共用 UI 仍把內部時間當玩家資源:" + frag);
    if (scenes1.includes("總耗天數") || scenes2.includes("總耗天數"))
      throw new Error("canonical 終幕仍以總耗天數評價玩家");
    for (const engineName of ["engine.js", "engine2.js", "engine3.js", "engine4.js", "engine5.js", "engine6.js"]) {
      const engineText = readFileSync(path.join(here, "../src/" + engineName), "utf-8");
      if (!engineText.includes("days")) throw new Error(engineName + " 誤刪內部研究歷程相容欄位");
    }
    if (!decisions.includes("GB-ADR-054") || !decisions.includes("1665 → 1679"))
      throw new Error("天數退役法源未同時保留敘事時間邊界");
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    for (const frag of [
      ...
      "body.choice-focus-split[data-view=\"choice\"] #panelWrap {",
      "right: 2.5%; width: 62%; max-width: 62%;"
    ...
    ]) if (!sui.includes(frag)) throw new Error("K2 地心判讀沒有場景限定避讓:" + frag);
...
tests.push({
  ...
  fn: () => {
    ...
    if (byId("B2-3", "s1").next !== "p1" || firstClaim.next !== "n5" ||
        firstClaim.speaker !== "旅人(你)" ||
        !firstClaim.text.includes("我用的是") || !firstClaim.text.includes("這只說") ||
        !firstClaim.text.includes("先換球查重量") || !byId("B2-3", "n4").legacyOnly)
...
tests.push({
  name: "六章自然聲線|功能優先、角色接話有別、強發現與冷聲線受契約保護",
  fn: () => {
    const chapters = [scenes, scenes2, scenes3, scenes4, scenes5, scenes6];
    const activeNodes = (chapter) => chapter.scenes.flatMap((scene) =>
      (scene.nodes || []).filter((node) => node.legacyOnly !== true)
        .map((node) => ({ ...node, sceneId: scene.id })));
    const travelerSpeakers = new Set(["旅人(你)", "旅人・心聲"]);
    const countWord = (text, word) => String(text || "").split(word).length - 1;

    for (const [index, chapter] of chapters.entries()) {
      const strong = activeNodes(chapter).filter((node) =>
        String(node.text || "").includes("啊！"));
      if (strong.length > 1)
        throw new Error(`第${index + 1}章的「啊！」超過一次：` +
          strong.map((node) => node.sceneId + "/" + node.id).join("、"));
      if (strong.some((node) => node.speaker !== "旅人(你)"))
        throw new Error(`第${index + 1}章的「啊！」不屬於旅人`);

      const nodes = activeNodes(chapter);
      if (JSON.stringify(nodes).includes("痾"))
        throw new Error(`第${index + 1}章使用了「痾」而非正字「呃」`);

      const stuck = nodes.filter((node) => JSON.stringify(node).includes("呃"));
      if (stuck.length > 1)
        throw new Error(`第${index + 1}章的「呃」超過一次：` +
          stuck.map((node) => node.sceneId + "/" + node.id).join("、"));
      for (const node of stuck) {
        const text = String(node.text || "");
        if (!travelerSpeakers.has(node.speaker) || !text.startsWith("呃……") ||
            /[（(]/.test(text) || countWord(JSON.stringify(node), "呃") !== countWord(text, "呃"))
          throw new Error("「呃」未遵守旅人話輪事故格式：" + node.sceneId + "/" + node.id);
        const scene = chapter.scenes.find((candidate) => candidate.id === node.sceneId);
        const handedOff = (scene?.nodes || []).some((candidate) =>
          candidate.next === node.id && candidate.type === "line" &&
          !travelerSpeakers.has(candidate.speaker) &&
          !["stage", "system"].includes(candidate.speaker));
        if (!handedOff)
          throw new Error("「呃」前一拍沒有由另一名角色交來話輪：" + node.sceneId + "/" + node.id);
      }

      const hey = nodes.filter((node) => JSON.stringify(node).includes("欸"));
      const heyKinds = { initial: 0, final: 0 };
      for (const node of hey) {
        const text = String(node.text || "");
        if (!travelerSpeakers.has(node.speaker) || countWord(text, "欸") !== 1 ||
            countWord(JSON.stringify(node), "欸") !== 1)
          throw new Error("「欸」不是旅人的單一受管語域標記：" + node.sceneId + "/" + node.id);
        if (text.startsWith("欸")) heyKinds.initial += 1;
        else if (/欸(?=[。！？…]|$)/.test(text)) heyKinds.final += 1;
        else throw new Error("「欸」不在受管的句首或句尾位置：" + node.sceneId + "/" + node.id);
      }
      if (heyKinds.initial > 1 || heyKinds.final > 1)
        throw new Error(`第${index + 1}章的「欸」超出句首／句尾各一次上限`);

      const spoken = nodes.filter((node) => node.type === "line" && node.speaker &&
        !["stage", "system"].includes(node.speaker));
      for (const node of spoken) {
        const text = String(node.text || "");
        if (/[嘖哼]/.test(text))
          throw new Error("角色台詞以輕蔑感嘆詞丑角化對手：" + node.sceneId + "/" + node.id);
        if (/(?:碰|砰|轟|咚|啪)[！!](?:\s*(?:碰|砰|轟|咚|啪)[！!])?/.test(text))
          throw new Error("角色台詞以純狀聲字串代替可記錄聲音：" + node.sceneId + "/" + node.id);
      }
    }

    const allStuck = chapters.flatMap((chapter) => activeNodes(chapter))
      .filter((node) => String(node.text || "").includes("呃"));
    if (allStuck.length > 6)
      throw new Error("全案「呃」超過六次保險絲上限");
    const d31 = new Map(scenes4.scenes.find((scene) => scene.id === "D3-1")
      .nodes.map((node) => [node.id, node]));
    if (d31.get("n4")?.speaker !== "牛頓" || d31.get("n4")?.text !== "你沒有老。" ||
        d31.get("n5")?.speaker !== "旅人(你)" ||
        d31.get("n5")?.text !== "呃……這件事很難解釋。")
      throw new Error("第四章話輪事故試點不再是牛頓突問、旅人語塞");

    const heyLocations = chapters.flatMap((chapter) => activeNodes(chapter))
      .filter((node) => String(node.text || "").includes("欸"))
      .map((node) => node.sceneId + "/" + node.id).sort();
    const expectedHey = ["A2-3/n2", "C0-1/x7", "P0-1/m1a"].sort();
    if (JSON.stringify(heyLocations) !== JSON.stringify(expectedHey))
      throw new Error("現行三處「欸」遭刪除、挪用或擴散：" + heyLocations.join("、"));

    const newton = activeNodes(scenes4).filter((node) => node.speaker === "牛頓");
    for (const node of newton) {
      const text = String(node.text || "");
      if (/嗯|喔|哦|啊！/.test(text))
        throw new Error("牛頓用情緒填充詞代替動作：" + node.sceneId + "/" + node.id);
      if (text.includes("等等") && !/再算|截住/.test(text))
        throw new Error("牛頓的「等等」沒有立刻接重算或截住動作：" + node.sceneId + "/" + node.id);
    }

    const galileoHmm = activeNodes(scenes).filter((node) =>
      node.speaker === "伽利略" && String(node.text || "").includes("嗯"));
    if (galileoHmm.length !== 1 || galileoHmm[0].sceneId !== "A2-1" || galileoHmm[0].id !== "nb")
      throw new Error("伽利略的「嗯」又被拿去包裝完成判決：" +
        galileoHmm.map((node) => node.sceneId + "/" + node.id).join("、"));
    if ((debate.chapter?.fr?.explore?.steps || []).some((step) =>
      String(step.prompt || "").includes("嗯")))
      throw new Error("第一章動態辯論仍以無功能的「嗯」包裝判讀提示");

    const d12n20 = scenes4.scenes.find((scene) => scene.id === "D1-2")
      ?.nodes.find((node) => node.id === "n20");
    if (d12n20?.text !== "……我最多只能把地心標出來，還不能把整顆地球當成縮在那一點。")
      throw new Error("D1-2 三條理由匯流後沒有使用中性、零填充詞的自我限縮句");

    const close = new Map(scenes3.scenes.find((scene) => scene.id === "C3-1")
      .nodes.map((node) => [node.id, node]));
    if (close.get("n3b")?.speaker !== "馬蒂厄" || close.get("n3b")?.next !== "n3c" ||
        !String(close.get("n3b")?.text || "").endsWith("由我簽。") ||
        close.get("n3c")?.speaker !== "艾蒂安" || close.get("n3c")?.next !== "n3d" ||
        !/每一趟離岸、加槳、收槳/.test(String(close.get("n3c")?.text || "")) ||
        close.get("n3d")?.speaker !== "伽桑狄")
      throw new Error("馬蒂厄只答自己一欄／艾蒂安主動補岸紙條件的對照未落地");

    const duChateletStop = scenes5.scenes.find((scene) => scene.id === "E2-2")
      ?.nodes.find((node) => node.id === "x2");
    if (duChateletStop?.speaker !== "杜夏特萊" || duChateletStop.text !== "不要。")
      throw new Error("杜夏特萊的截停標記不再是唯一的「不要。」");
    const kessler = activeNodes(scenes6).filter((node) => node.speaker === "凱斯勒院士");
    if (kessler.some((node) => String(node.text || "").includes("嗯……")) ||
        !kessler.some((node) => String(node.text || "").includes("我接受這句")) ||
        !kessler.some((node) => String(node.text || "").includes("另起一行")))
      throw new Error("凱斯勒仍靠「嗯……」思考，或獨特的承認／改紙動作遺失");

    const voiceLaw = readFileSync(path.join(here,
      "../../02_設計/發現之前_角色聲線與對話規範_v0.1.md"), "utf-8");
    for (const phrase of [
      "功能優先於角色配額",
      "停**：`等等`／`且慢`／`不要`",
      "疑**：`嗯……`／`喔……`",
      ...
      "等等。我再算一遍。",
      "每章至多一次"
    ]) if (!voiceLaw.includes(phrase))
      throw new Error("自然聲線法源缺契約：" + phrase);

    const voiceCr = readFileSync(path.join(here,
      "../../01_治理/發現之前_NARRATIVE-CR-037_旅人話輪事故與欸納管_20260806.md"), "utf-8");
    for (const phrase of [
      "話輪事故",
      "不批次改寫六章",
      "D3-1/n4→n5",
      "D4-1/n3",
      "真人 A/B 朗讀"
    ]) if (!voiceCr.includes(phrase))
      throw new Error("NARRATIVE-CR-037 缺施工邊界：" + phrase);
  }
});

tests.push({
  name: "工作台兩道門檻|複雜度不冒充完整桌面，對話授證與K3現況分開治理",
  fn: () => {
    const law = readFileSync(path.join(here,
      "../../02_設計/發現之前_工作台與辯論架構_一二章實證規格_v0.1.md"), "utf-8");
    for (const phrase of [
      "第一道只判斷要不要離開對話舞台",
      "第二道才判斷是否需要完整工作台",
      "這不是加總分數",
      "跨兩次以上判斷反覆回查各自條件後才能提出主張",
      "同批翻開且可一次完成判讀的有限卡組",
      "具名人物接住玩家剛完成的判讀",
      "精簡操作板／完整工作台的斷言成功路徑",
      "K3 現行的揭曉後判讀",
      "不得把尚不存在的分岔倒寫成現行玩法"
    ]) if (!law.includes(phrase))
      throw new Error("工作台分層法源缺契約：" + phrase);

    const d12 = new Map(scenes4.scenes.find((scene) => scene.id === "D1-2")
      .nodes.map((node) => [node.id, node]));
    if (d12.get("n12")?.speaker !== "牛頓" ||
        !String(d12.get("n12")?.text || "").includes("同樣一秒") ||
        !String(d12.get("n12")?.text || "").includes("遠約六十倍") ||
        !String(d12.get("n12")?.text || "").includes("三千六百分之一") ||
        d12.get("n12a")?.speaker !== "旅人(你)" ||
        !String(d12.get("n12a")?.text || "").includes("六十平方分之一") ||
        d12.get("n12c")?.speaker !== "牛頓" ||
        !String(d12.get("n12c")?.text || "").includes("別急著叫它定律") ||
        !String(d12.get("n12b")?.text || "").includes("畫了一道線，便把筆放下") ||
        d12.get("n13")?.speaker !== "旅人(你)" ||
        !String(d12.get("n13")?.text || "").includes("火星、木星") ||
        d12.get("n13a")?.speaker !== "牛頓" ||
        !String(d12.get("n13a")?.text || "").includes("只算過月亮"))
      throw new Error("K2 對話授證缺具名簽收或下一問橋接");

    const runtime = JSON.stringify(scenes4);
    for (const nonexistent of ["已知距離預測週期", "已知週期反推距離"])
      if (runtime.includes(nonexistent))
        throw new Error("K3 把提案中的未實作研究分岔寫成現行 runtime：" + nonexistent);
  }
});

tests.push({
  name: "GB-ADR-051/052|K2 三種地心理由可逐一追問，缺口保留且不強迫清單式全點",
  fn: () => {
    const d12 = new Map(scenes4.scenes.find((scene) => scene.id === "D1-2")
      .nodes.map((node) => [node.id, node]));
    const prompt = d12.get("c_center_reason");
    const followup = d12.get("c_center_reason_more");
    const options = prompt?.options || [];
    if (d12.get("n19")?.next !== "c_center_reason" || prompt?.type !== "choice" ||
        prompt?.speaker !== "system" || options.length !== 3 ||
        followup?.type !== "choice" || followup?.speaker !== "system")
      throw new Error("地心量距未在牛頓提問後交給玩家三選並提供後續追問");

    const expected = new Map([
      ["center_round", ["wc_round", "ch4CenterRoundTried", "中心在哪裡", "拉扯加起來"]],
      ["center_symmetry", ["wc_symmetry", "ch4CenterSymmetryTried", "方向朝中心", "拉得多大"]],
      ["center_fall", ["wc_fall", "ch4CenterFallTried", "結果當成理由", "從地心拉來"]]
    ]);
    function assertReasonOption(option, repeated) {
      const contract = expected.get(option.id);
      const effects = option.effects || [];
      if (!contract || option.next !== contract[0] ||
          JSON.stringify(effects) !== JSON.stringify([
            { flag: [contract[1], "1"] },
            { flag: ["ch4CenterReasonStarted", "1"] }
          ]) ||
          repeated && JSON.stringify(option.require) !== JSON.stringify({ flagAbsent: contract[1] }))
        throw new Error("地心量距理由只能留下導覽旗標，不得計分、授證或改章別引擎：" + option.id);
      const response = d12.get(option.next);
      if (response?.speaker !== "牛頓" || response?.next !== "c_center_reason_more" ||
          !String(response?.text || "").includes(contract[2]) ||
          !String(response?.text || "").includes(contract[3]))
        throw new Error("地心量距分支沒有指出自己的特定缺口並回到追問：" + option.id);
    }
    options.forEach((option) => assertReasonOption(option, false));
    const repeatedReasons = (followup.options || []).filter((option) => expected.has(option.id));
    repeatedReasons.forEach((option) => assertReasonOption(option, true));
    const leave = (followup.options || []).find((option) => option.id === "center_leave_question");
    if (repeatedReasons.length !== 3 || !leave || leave.next !== "n20" ||
        "effects" in leave ||
        JSON.stringify(leave.require) !== JSON.stringify({ flag: ["ch4CenterReasonStarted", "1"] }))
      throw new Error("第一次試答後沒有提供零代價的留下問號選項");

    const optionLengths = options.map((option) => Array.from(option.text || "").length);
    if (Math.max(...optionLengths) / Math.min(...optionLengths) > 1.2)
      throw new Error("地心量距三個理由的選項長度失衡");

    const N4Center = Narrative._factory(scenes4, Engine4, {});
    const orders = [
      ["center_round", "center_symmetry", "center_fall"],
      ["center_round", "center_fall", "center_symmetry"],
      ["center_symmetry", "center_round", "center_fall"],
      ["center_symmetry", "center_fall", "center_round"],
      ["center_fall", "center_round", "center_symmetry"],
      ["center_fall", "center_symmetry", "center_round"]
    ];
    for (const order of orders) {
      let state = N4Center.initialState("explore");
      const initialLab = JSON.stringify(state.lab);
      state.cursor = { scene: "D1-2", node: "c_center_reason" };
      for (let index = 0; index < order.length; index += 1) {
        const reason = order[index];
        const before = N4Center.view(state).options.map((option) => option.id);
        if (!before.includes(reason))
          throw new Error("同一輪無法依序追問地心理由：" + order.join("→"));
        const picked = N4Center.choose(state, reason);
        if (picked.error) throw new Error("地心理由選擇遭拒：" + picked.error);
        state = N4Center.advance(picked.state).state;
        const after = N4Center.view(state).options.map((option) => option.id);
        if (after.includes(reason) || !after.includes("center_leave_question"))
          throw new Error("已試理由仍可重複，或第一次後不能留下問號：" + reason);
      }
      const remaining = N4Center.view(state).options.map((option) => option.id);
      if (JSON.stringify(remaining) !== JSON.stringify(["center_leave_question"]))
        throw new Error("三項全試後不是只剩留下問號：" + remaining.join("、"));
      if (state.rep !== 3 || Object.keys(state.evidence).length !== 0 ||
          JSON.stringify(state.lab) !== initialLab)
        throw new Error("地心理由追問改動了信譽、證據或章別引擎狀態");
      const closed = N4Center.choose(state, "center_leave_question");
      if (closed.error || closed.state.cursor.node !== "n20")
        throw new Error("三項追問後無法回到旅人親口限縮");
    }

    let earlyState = N4Center.initialState("explore");
    earlyState.cursor = { scene: "D1-2", node: "c_center_reason" };
    earlyState = N4Center.choose(earlyState, "center_symmetry").state;
    earlyState = N4Center.advance(earlyState).state;
    const earlyLeave = N4Center.choose(earlyState, "center_leave_question");
    if (earlyLeave.error || earlyLeave.state.cursor.node !== "n20")
      throw new Error("玩家不能在試過一項後自行決定先留下問號");

    if (d12.get("n20")?.speaker !== "旅人(你)" ||
        !String(d12.get("n20")?.text || "").includes("還不能把整顆地球當成縮在那一點") ||
        d12.get("n21")?.speaker !== "牛頓" ||
        !String(d12.get("n21")?.text || "").includes("數字對上了，理由還沒有"))
      throw new Error("追問沒有匯流到旅人親口限縮與牛頓留問號");

    const notebook = String(d12.get("n24")?.text || "");
    if (!notebook.includes("同樣一秒") || !notebook.includes("三千六百分之一") ||
        /加速度|作用按距離平方變弱/.test(notebook))
      throw new Error("K2 筆記沒有停在距離與同時間落距的可見關係");
  }
});

tests.push({
  name: "ENGINE-CR-036|K3 不洩答，K2／K3 視聽授證落在發現與簽收行",
  fn: () => {
    const scene = (sceneId) => scenes4.scenes.find((item) => item.id === sceneId);
    const line = (sceneId, nodeId) => scene(sceneId)?.nodes.find((item) => item.id === nodeId);
    if (line("D1-2", "n11c")?.evidenceCue !== "K2")
      throw new Error("K2 視聽授證未掛在旅人的 60×60 發現句");
    if (JSON.stringify(line("D1-2", "n11c")?.evidenceCueFrom) !== JSON.stringify(["c_relation"]))
      throw new Error("K2 授證延後範圍未限定為現行 60×60 判讀路徑");
    if (line("D3-1", "n38")?.evidenceCue !== "K3")
      throw new Error("K3 視聽授證未掛在哈雷的具名簽收句");
    if (JSON.stringify(line("D3-1", "n38")?.evidenceCueFrom) !== JSON.stringify(["e1"]))
      throw new Error("K3 授證延後範圍未限定為現行封存揭曉板");
    if (!/先寫下火星和木星的預測/.test(String(line("D3-1", "n38")?.text || "")) ||
        !/還不能說整片天空/.test(String(line("D3-1", "n38")?.text || "")))
      throw new Error("K3 成功後缺少哈雷對玩家判讀的自然回饋");

    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    if (ui.includes("真正接受檢驗的是觀測揭曉以前，由同一條距離律算出的週期"))
      throw new Error("K3 判讀前仍把正解邏輯逐字說完");
    for (const phrase of [
      "evidenceCueIndex",
      "releaseAt",
      "nodeId",
      "evidenceCue",
      "!evidenceCueIndex[k.toUpperCase()]"
    ]) if (!ui.includes(phrase))
      throw new Error("結構化授證掛點接線不完整：" + phrase);

    const cr = readFileSync(path.join(here,
      "../../01_治理/發現之前_ENGINE-CR-036_第四章K2K3發現後授證與K3防洩答_20260806.md"), "utf-8");
    for (const phrase of ["K2 → `D1-2/n11c`", "K3 → `D3-1/n38`", "B-4 尚無實測摩擦證據"])
      if (!cr.includes(phrase)) throw new Error("ENGINE-CR-036 缺裁決邊界：" + phrase);
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    const validate = (data) => {
      ...
      if (allInner.length !== 47 || activeInner.length !== 20 ||
          legacyInner.length !== 27)
...
tests.push({
  ...
  fn: () => {
    ...
    const f2 = byId("B2-3", "p1").text;
    if (!f2.includes("還沒有證明前進會不會拖慢下墜") || !f2.includes("一拋一放直接問") ||
        f2.includes("各按自己的鐘走"))
    ...
    if (b24.nodes.filter((node) => !node.legacyOnly).length !== 16 ||
        ["np2", "np3"].some((id) => !byId("B2-4", id)?.legacyOnly) ||
        byId("B2-4", "na1")?.legacyOnly)
      throw new Error("B2-4 未維持 16 個 active 節點、押注讀回與舊游標落點");
    const prediction = byId("B2-4", "q1");
    const expectedPrediction = new Map([["a", "late"], ["b", "unresolved"], ["c", "abstain"]]);
    if (prediction.options.length !== 3 || prediction.options.some((option) => option.next !== "c_commit") ||
        prediction.options.some((option) => option.effects?.[0]?.flag?.[0] !== "ch2DropPrediction" ||
          option.effects?.[0]?.flag?.[1] !== expectedPrediction.get(option.id)) ||
        prediction.options.some((option) => (option.effects || []).some((effect) => "rep" in effect)))
      throw new Error("B2-4 未在看結果前提供三個不計分預判");
    if (byId("B2-4", "c_commit")?.next !== "q2" ||
        byId("B2-4", "q4")?.options?.find((option) => option.id === "a")?.next !== "r_late" ||
        byId("B2-4", "q4")?.options?.find((option) => option.id === "b")?.next !== "nb1")
      throw new Error("CH2-CR-013 承諾拍或誠實／補寫結果分流漂移");
    ...
    /* 新主線不得再進入 legacyOnly；退役游標必須真的能續讀。na1 已恢復為玩家反應。 */
    const legacyByScene = {
      "B2-4": new Set(["np2", "np3"]),
      "B2-5": new Set(["n2", "n3", "n4", "q3", "w3"])
    };
    const legacyContinuation = {
      "B2-4/np2":"q3", "B2-4/np3":"q4",
    ...
    const predictionResults = prediction.options.map((option) => {
      ...
      if (picked.error || picked.state.rep !== state.rep || picked.state.cursor.node !== "c_commit" ||
          !picked.state.transcript.some((line) => line.text === option.text))
        throw new Error("B2-4 預判沒有保留玩家原句，或偷改信譽:" + option.id);
      state = N2.advance(picked.state).state;
...
tests.push({
  ...
  fn: () => {
    ...
    if (orchardText.includes("六十個地球半徑") ||
        ...
        !/(?:兩張紙分開放|把兩張紙分開)/.test(scaleText) || !/(?:只量過|只算過)月亮/.test(scaleText) ||
...
tests.push({
  ...
  fn: () => {
    ...
    for (const frag of ['v.system === "catapult"', "renderCatapult", "compareBalls", "assertLaw", "abandonSeries",
      "cat2CompareFailure", "r.result.ok === false", "catDataTable", "cat2SelectionDraft",
      "在上方同一張表勾兩組", "比較勾選的兩組・提出斷言",
      ...
      "用勾選的數據提出斷言", "選擇數據支持的概念", "workshopPartGuide", "catPartBrief",
...
tests.push({
  name: "第二章 M2a|engine2:fixture 判定+12.0/12.5 邊界+換球守衛+黃金路徑保留 10 刻度(R-LAB2)",
  fn: () => {
    const E2 = require("../src/engine2.js");
    const FULL = [["release", "latchRelease"], ["edge", "polishedEdge"], ["rangeBed", "rakedSand"]];
    const build = () => {
...
tests.push({
  name: "第三章資料鏡像與敘事圖|10 場主線＋1 修復場、236 節點、活躍圖全可達、舊游標相容",
  fn: () => {
    ...
    if (allNodes !== 236) throw new Error("第三章節點數不是 236，實得:" + allNodes);
    ...
    const legacyNodes = scenes3.scenes.flatMap((scene) =>
      scene.nodes.filter((node) => node.legacyOnly)
        .map((node) => scene.id + "/" + node.id));
    if (legacyNodes.length !== 28)
      throw new Error("第三章 legacyOnly 節點應保留 28 個舊游標，實得:" + legacyNodes.length);
...
tests.push({
  ...
  fn: () => {
    ...
    if (!ui2.includes("function renderShipDossierLab(work, d, lab)") ||
        !ui2.includes("renderShipDossierLab(work, d, lab)") ||
        !dossierLabUi.includes("lab.design && lab.design.dual && lab.design.dual.locked"))
      throw new Error("雙視角設計畫面沒有讀取當前第三章 lab 狀態，鎖定後仍可能重畫舊選項");
    const designDecisionUi = ui2.slice(ui2.indexOf("function renderShipDossierDesignDecision"),
      ui2.indexOf("function renderShipDossierNotebookIndex"));
    if (!designDecisionUi.includes("var accepted = config.accept()") ||
        !designDecisionUi.includes("accepted.error") ||
        !designDecisionUi.includes("accepted.result && accepted.result.ok === false") ||
        !designDecisionUi.includes("config.afterAccept(accepted)"))
      throw new Error("研究安排即使被引擎拒絕，介面仍可能自行顯示成功或推進");
    ...
    if (!dossierLabUi.includes('ship3OpenIntermission("ch3-after-dual-design"') ||
        !dossierLabUi.includes("▸ 回到甲板，執行雙視角紀錄") ||
        !embarkListenerUi.includes('detail.intermission === "ch3-after-dual-design"') ||
        !embarkListenerUi.includes("ship3FocusDossierRunButton()"))
      throw new Error("雙視角研究安排成立後，缺少對話退場與下一個可操作按鈕的明確交接");
    ...
    for (const spokenChoice of [
      "不然，等一個沒風的日子再試？",
      "那就進船艙吧，把外頭的風隔開。",
      "要不要先在甲板立一面擋風板？"
    ]) if (!ui2.includes(spokenChoice))
      throw new Error("排除甲板風的選項仍是系統方案名稱，不是旅人能說出口的對話:" + spokenChoice);
    if (!assertionIntermissionUi.includes('{ speaker: "旅人(你)", text: option.label }') ||
        !ui2.includes('function ship3PresentCabinWindDialogue(fromReload)') ||
        !dossierLabUi.includes("ship3PresentCabinWindDialogue(true)") ||
        dossierLabUi.includes('id: "cabin-wind"') ||
        dossierLabUi.includes('action: "commitDossierCabinWindPlan"'))
      throw new Error("甲板風三選仍可能在工作台出現，或玩家選項沒有先由旅人親口說出");
...
tests.push({
  ...
  fn: () => {
    ...
    const commitNodes = [];
    for (let i = 0; i < 4; i += 1) {
      commitNodes.push(branch.N3.view(branch.state).nodeId);
      branch.state = branch.N3.advance(branch.state).state;
    }
    if (commitNodes.join(",") !== "a_commit1,a_commit2,a_commit3,a_commit4" ||
        branch.N3.view(branch.state).nodeId !== "n2" || branch.state.rep !== 2)
      throw new Error("搶答沒有進入四拍承諾並帶著信譽代價前進");
    const legacyRetry = structuredClone(branch.state);
    legacyRetry.cursor = { scene:"C0-3", node:"c1" };
    const retry = branch.N3.view(legacyRetry);
    if (retry.options.some((option) => option.id === "all") ||
        !retry.options.some((option) => option.id === "all_again"))
      throw new Error("舊游標重選時仍可重複觸發扣分版本，或遺失同句相容選項");
    const repeated = branch.N3.choose(legacyRetry, "all_again");
    if (repeated.error || repeated.state.rep !== 2)
      throw new Error("重看同一句應保留回應，但不得重複扣信譽");
    if (repeated.state.eventLog.filter((item) => item.t === "rep").length !== 1)
      throw new Error("legacy all_again 產生了第二筆信譽事件");
...
tests.push({
  ...
  fn: () => {
    ...
    hookeBase.proof.shellPagePlaced = true;
    hookeBase = Engine4.acknowledgeCreditDispute(hookeBase).state;
    ...
    for (const rows of Object.values(ch4ReachRepair.lab.claims)) {
      for (const row of rows) {
        if (row.action === "judgePlanetComparison") continue;
    ...
    for (const row of ch4ReachRepair.lab.planetLab.comparisonAttempts) {
      ch4HelperEvents.push({
        t:"lab", action:"judgePlanetComparison", at:"test-helper",
        sequence:row.at
      });
    }
    for (const row of ch4ReachRepair.lab.modelLab.protocolAttempts) {
      ch4HelperEvents.push({
        t:"lab", action:"setModelProtocol", at:"test-helper",
        sequence:row.at
      });
    }
    for (const row of Object.values(ch4ReachRepair.lab.modelLab.predictions)) {
      ch4HelperEvents.push({
        t:"lab", action:"sealModelPrediction", at:"test-helper",
        sequence:row.sealedAt
      });
    }
    ...
    ch4Act("assembleKnownProofSources");
    ch4Act("judgeShellBalance", { choice:"matched-cones" });
    ch4Act("placeShellPage");
    ch4Act("acknowledgeCreditDispute");
...
tests.push({
  ...
  fn: () => {
    ...
    const pick = { "C0-1": "own-drawing", "C0-3": "bounded", "C3-1": "bounded", "C3-2": "cabin" };
...
tests.push({
  ...
  fn: () => {
    ...
    for (const scene of s4.scenes.filter((item) => item.id !== "SC4-R1" && item.id !== "D4-1")) {
    ...
    if (s4.scenes.find((scene) => scene.id === "D4-1")?.title !== "三份天空")
      throw new Error("CH4-CR-018 沒有把哈雷來訪後的主挑戰收於三份天空");
    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    if (!ui.includes("閱讀和重排不會用掉送印機會") || !ui.includes("公開質詢只計算已共同檢查"))
...
tests.push({
  ...
  fn: () => {
    const sceneSets = [
      ...
      JSON.parse(readFileSync(path.join(here, "../data/scenes5.json"), "utf-8")),
      JSON.parse(readFileSync(path.join(here, "../data/scenes6.json"), "utf-8")),
      JSON.parse(readFileSync(path.join(here, "../data/debate.json"), "utf-8")),
      JSON.parse(readFileSync(path.join(here, "../data/debate2.json"), "utf-8")),
      JSON.parse(readFileSync(path.join(here, "../data/debate5.json"), "utf-8"))
    ];
    const visibleKeys = new Set([
      "text", "title", "label", "prompt", "reply", "slotPrompt", "frUnlocked",
      "hint", "description", "summary", "name", "alt", "responsePrompt",
      "playerCorrect", "playerWrong", "breakReply", "press", "insight",
      "operationContract", "objective", "procedure"
    ]);
    const leaks = [];
    /* 實驗紀錄 #1、主張 #1 是玩家親手產生且需要引用的流水號，刻意不列為內部 ID。 */
    const internalId = /(?:^|[^A-Za-z0-9])(?:E[0-9](?:\.[a-z])?|F[0-9]|G[0-9]|J[0-9]|K[0-9]|S[0-9]|T[0-9]|P[1-9]|(?:P0|INT|[A-H][0-9]+|BE|CE|DE|HE|SC)-[A-Za-z0-9-]+|GB-ADR-\d+|R-[A-Z0-9-]+|run\s*#)(?:$|[^A-Za-z0-9])/;
    function walk(value, key, inheritedVisible) {
      const visible = !!inheritedVisible || visibleKeys.has(key);
      if (typeof value === "string" && visible && internalId.test(value)) leaks.push(value);
      else if (Array.isArray(value)) value.forEach((x) => walk(x, "", visible));
      else if (value && typeof value === "object") Object.entries(value).forEach(([k, v]) => walk(v, k, false));
    }
    sceneSets.forEach((x) => walk(x, "", false));
    if (leaks.length) throw new Error("玩家可見字串仍含製作代碼：" + leaks[0]);
    const debate2 = sceneSets[7];
    ...
    for (const frag of [
      ...
      'id.toUpperCase(); }).join', 'p[0] + "｜已夾回筆記"', '"✓ K4 對帳紙成立',
      '"✓ J1 入卷', '"✓ J2 入卷', '"✓ J3 入卷', '"這是 K0 來源紙',
      '"球殼頁不是 K1', '"這一步尚未成立（" + code'
    ]) if (ui.includes(frag)) throw new Error("章節介面仍可能顯示內部資料 ID：" + frag);
    const stageIntro = readFileSync(path.join(here, "../src/stage/07-intro-inputs.part.js"), "utf-8");
    for (const frag of ["J1 證明", "T4 才會入卷"])
      if (stageIntro.includes(frag)) throw new Error("舞台備忘仍顯示內部證據代碼：" + frag);
...
tests.push({
  ...
  fn: () => {
    ...
    const events = readFileSync(path.join(here, "../src/stage/05-events.part.js"), "utf-8");
    const notebook = readFileSync(path.join(here, "../src/stage/09-notebook.part.js"), "utf-8");
    for (const frag of ["collectNewEvidence(before, state)", "pendingEvidence.push(projectEvidenceVisual({", "takePendingEvidence().forEach", "evidence: replaying ? [] : takePendingEvidence("])
...
tests.push({
  name: "第四章證據成品|K4 四世界線唯讀投影、fail-closed 與放大文字版(CH4-CR-014)",
  fn: () => {
    const assets = require("../data/assets.js");
    const chapterUi = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    const focus = readFileSync(path.join(here, "../src/stage/03-focus-visual.part.js"), "utf-8");
    ...
    const projection = Function("CHAPTER_ID",
      "return (" + extractFunction(chapterUi, "evidenceVisualKeyForState") + ");")("ch4");
    const stateFor = (planet, comet) => ({
      lab: {
        evidence: { k4: true },
        modelLab: {
          comparisonSealed: true,
          evidencePackage: { stamps: [
            { caseId: "moon", loanDecision: null },
            { caseId: "planets", loanDecision: planet },
            { caseId: "comet", loanDecision: comet }
          ] }
        }
      }
    });
    const worlds = [
      ["no-loan", "no-loan", "no_loans"],
      ["loan", "no-loan", "planets_loan"],
      ["no-loan", "loan", "comet_loan"],
      ["loan", "loan", "both_loans"]
    ];
    for (const [planet, comet, key] of worlds)
      if (projection(stateFor(planet, comet), "K4") !== key)
        throw new Error("K4 封存世界線投影錯誤:" + key);
    if (projection(stateFor("loan", "loan"), "K1") !== null)
      throw new Error("K1 合法 null 被誤當成狀態圖");
    const broken = stateFor("loan", "loan");
    broken.lab.modelLab.evidencePackage.stamps[2].loanDecision = "unknown";
    if (projection(broken, "K4") !== null) throw new Error("非法 K4 狀態被猜成世界線");
    const duplicate = stateFor("loan", "loan");
    ...
    for (const [, , key] of worlds) {
      const resolved = resolver("K4", key);
      if (resolved.fallback || resolved.visualKey !== key || !resolved.items?.[0]?.asset?.includes(key === "both_loans" ? "both_loans" : key))
        throw new Error("K4 resolver 沒有選到封存變體:" + key);
    }
    const fallback = resolver("K4", null);
    if (!fallback.fallback || fallback.items?.[0]?.asset !== "card_K4" ||
        fallback.fallbackNotice !== "此存檔的黃色補充紙（桌上稱借條）狀態無法安全還原。")
    ...
    ]) if (!chapterUi.includes(fragment) && !notebook.includes(fragment))
    ...
    if (!html.includes('role="dialog" aria-modal="true" aria-labelledby="evidenceReaderTitle"') ||
      ...
      throw new Error("放大閱讀器缺 modal 焦點圍欄或焦點歸還");
    if (/localStorage|sessionStorage|window\.GB\.Narrative/.test(
      extractFunction(focus, "resolveEvidenceVisual") +
      extractFunction(notebook, "renderEvidenceCards") + events))
      throw new Error("舞台 resolver 越權讀取引擎或存檔");
  }
});
...
function ch4K0K2State() {
  ...
  s = Engine4.convertMoonTime(s, "sagitta-geometry").state;
...
function ch4CompleteK1Paper(state0) {
  let s = state0;
  for (const [speed, strength] of [
    ["slow", "short"], ["medium", "medium"], ["medium", "short"]
  ]) {
    const trial = Engine4.runOrbitPaperTrial(s, speed, strength);
    if (trial.error || !trial.ok) throw new Error("測試助手無法完成紙上試跑");
    s = trial.state;
  }
  const claim = Engine4.assertK1(s,
    ["tangent", "trial:1", "trial:2", "trial:3"],
    "forward-plus-inward-turn");
  if (!claim.ok) throw new Error("測試助手無法由三張試跑紙完成 K1");
  return claim.state;
}
...
function ch4CompleteK3(state0) {
  ...
  const claim = Engine4.judgePlanetComparison(s, "theory-before-observation");
...
function ch4CompleteK4(state0, loanCases = ["comet"]) {
  ...
  s = Engine4.sealModelPrediction(s, "inverseSquare", "one-law-three-skies").state;
  s = Engine4.sealModelPrediction(s, "simpleVortex", "patches-beyond-moon").state;
...
function ch4CompleteK5(state0) {
  let s = Engine4.assembleKnownProofSources(state0).state;
  s = Engine4.judgeShellBalance(s, "matched-cones").state;
  s = Engine4.placeShellPage(s).state;
  s = Engine4.acknowledgeCreditDispute(s).state;
...
tests.push({
  name: "第四章 v0.11 場景圖|12 場主線＋1 修復場、K0/K2 對話判讀、四座取證工作台與 JSON 鏡像一致",
  fn: () => {
    ...
    if (allNodes !== 343) throw new Error("第四章節點數不是 343，實得:" + allNodes);
    ...
    if (activeInner.length !== 12 || legacyInner.length !== 16 || legacyNodes.length !== 26)
    ...
    const activeNodes = scenes4.scenes.flatMap((scene) => scene.nodes
      ...
      .map((node) => scene.id + "/" + node.id));
    const unreachableActive = activeNodes.filter((key) => !visited.has(key));
    if (activeNodes.length !== 317 || unreachableActive.length)
    ...
    for (const key of legacyNodes) {
      ...
      state.cursor = { scene: sceneId, node: nodeId };
      const legacyRecallPick = {
        d42_recall_curve:"curve",
        d42_recall_inward:"inward",
        d42_recall_tangent:"tangent"
      }[nodeId];
      if (sceneId === "D4-2" && legacyRecallPick)
        state.eventLog.push({ t:"choice", at:"D1-1/c1", pick:legacyRecallPick });
      ...
      } else if (shown.type === "embed" && shown.phase === "scale") {
        state.lab = ch4K0K2State();
        next = N4Legacy.embedComplete(state);
    ...
    const intermissionChoiceState = N4Legacy.initialState("explore");
    intermissionChoiceState.cursor = { scene:"D-INT-1", node:"c1" };
    const intermissionTurn = N4Legacy.choose(intermissionChoiceState, "turn");
    const intermissionTurnView = intermissionTurn.error ? null : N4Legacy.view(intermissionTurn.state);
    if (intermissionTurn.error || !intermissionTurnView || intermissionTurnView.scene !== "D2-1")
      throw new Error("D-INT-1/c1 的親手翻頁選項無法進入 1679 場景");
    ...
    if (JSON.stringify(actualGates) !== JSON.stringify(expectedGates))
      throw new Error("第四章四座取證工作台漂移:" + JSON.stringify(actualGates));
    ...
    const d21Setup = ["n23", "n24", "n25", "n26", "n27", "n28", "n28a", "n28b"]
      .map((id) => String(sceneMap.get("D2-1").get(id)?.text || "")).join(" ");
    for (const fragment of ["很多小步", "朝它此刻看見的地心", "一次只改一件事", "先試跑、記結果"])
      if (!d21Setup.includes(fragment))
        throw new Error("D2-1 沒有在試跑前交代紙上小步與控制變因:" + fragment);
    ...
    if (new Set(de1Texts.map((text) => text.split("。")[0])).size !== 1 ||
        ...
        de1Texts.some((text) => !/來源.*改過的舊稿.*還沒證明/.test(text)))
    ...
    if (!String(d41.get("n23").text).includes("每次只判一個結果") ||
        !String(d41.get("n28").text).includes("親手另貼一張") ||
        !String(d41.get("n29").text).includes("沒貼過的黃色補充紙"))
      throw new Error("D4-1 模型比較規則仍是後台術語，或沒有寫出可見後果");
    ...
    if (printerNodes.some(([sceneId, nodeId]) => sceneMap.get(sceneId).get(nodeId).speaker !== "印刷工") ||
        !/印刷現場是遊戲重建/.test(String(sceneMap.get("D4-2").get("n12").text)))
    ...
    if (!String(sceneMap.get("D2-2").get("n12").text).includes("只記兩封信各自提出了什麼，不把信本身算成新證據") ||
...
tests.push({
  name: "第四章 K0→K2|共同計算從正矢判讀起步、舊封紙相容、K2 可先於 K1",
  fn: () => {
    let s = Engine4.initialState();
    ...
    if (s.scaleLab.scalePrediction !== null || s.scaleLab.predictionAttempts.length)
      throw new Error("K2 新局仍預先放入量級封紙");
    r = Engine4.convertMoonTime(s, "arc-length"); s = r.state;
    if (r.ok || r.consequence !== "arc-is-not-sag" ||
        s.scaleLab.conversionAttempts.length !== 1 || s.scaleLab.conversionCorrect)
      throw new Error("把弧長當正矢的錯讀未保留，或系統代做正解");
    s = Engine4.convertMoonTime(s, "sagitta-geometry").state;
    if (s.scaleLab.moonOneSecondSagMm !== 1.36)
      throw new Error("月球一秒正矢沒有由天文 fixture 算成 1.36 mm");
    const sag1 = Engine4._moonSagM(1);
    const sag60 = Engine4._moonSagM(60);
    if (Math.abs(sag1 * 1000 - 1.3562377155) > 1e-8 ||
        Math.abs(sag60 - 4.8824557655) > 1e-8 ||
        Math.abs(Engine4._expectedScaleRatio() - 3612.9359506) > 1e-6)
      throw new Error("第四章月球正矢／倍率 fixture 不可重算");
    const changedFixture = { ...Engine4._FIXTURE.teaching, moonPeriodDays:54.6 };
    if (!(Engine4._moonSagM(1, changedFixture) < sag1 * 0.26) ||
        Engine4._moonSagM(1, changedFixture) === sag1)
      throw new Error("改週期後正矢仍像固定常數");
    const engineSource = readFileSync(path.join(here, "../src/engine4.js"), "utf-8");
    if (engineSource.includes("moonSixtySecondSagM"))
      throw new Error("第四章引擎仍保留月球六十秒 4.9 m 假答案常數");
    ...
    if (!r.ok || !s.evidence.k2 || s.evidence.k1 ||
        ...
        s.scaleLab.scalePrediction !== null)
      throw new Error("K2 未能由共同幾何起步、先於 K1 成立，或新局偷偷補了量級封紙");

    let legacy = Engine4.initialState();
    legacy = Engine4.sealTangentPrediction(legacy, "tangent").state;
    legacy = Engine4.sealScalePrediction(legacy, "one-sixtieth").state;
    legacy = Engine4.convertMoonTime(legacy, "sagitta-geometry").state;
    if (!(legacy.scaleLab.scalePrediction.openedAt >
          legacy.scaleLab.scalePrediction.sealedAt) ||
        legacy.scaleLab.scalePrediction.matched !== false)
      throw new Error("舊 K2 量級封紙無法在首次幾何判讀時相容開封");
  }
});
...
tests.push({
  name: "第四章 K1 新試跑|固定朝此刻地心、兩項自由試跑、選三筆後才可斷言",
  fn: () => {
    let s = ch4K0K2State();
    const matrix = [
      ["slow", "short", "circle"],
    ...
    for (const [speed, strength, shape] of matrix) {
      const trial = Engine4.runOrbitPaperTrial(s, speed, strength);
      if (trial.error || !trial.ok || trial.run.target !== "earth-center" ||
          trial.run.actualShape !== shape ||
          !Engine4._orbitPaperTrialAudit(trial.run))
        throw new Error("紙上試跑沒有由兩項設定重算:" + speed + "/" + strength);
      s = trial.state;
    }
    const duplicateBefore = JSON.stringify(s);
    const duplicate = Engine4.runOrbitPaperTrial(s, "medium", "short");
    if (duplicate.error !== "orbit-paper-trial-duplicate" ||
        JSON.stringify(s) !== duplicateBefore)
      throw new Error("同組試跑未被退回，或退回時改動了原紀錄");
    if (Engine4.assertK1(s,
      ["tangent", "trial:1", "trial:2"],
      "forward-plus-inward-turn").ok)
      throw new Error("只選兩張紙就取得 K1");
    if (Engine4.assertK1(s,
      ["tangent", "trial:1", "trial:2", "trial:3"],
      "forward-push").ok)
      throw new Error("正確三紙卻用向前推力結論取得 K1");
    const claim = Engine4.assertK1(s,
      ["tangent", "trial:1", "trial:2", "trial:3"],
      "forward-plus-inward-turn");
    if (!claim.ok || !claim.state.evidence.k1 ||
        claim.state.orbitLab.ruleSeal != null ||
        claim.state.orbitLab.manualBeats.length)
      throw new Error("新 K1 仍暗中依賴舊四欄預測或親手三拍");
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    ).ok) throw new Error("尚未完成揭曉後判讀即可沿舊 assertK3 旁路取得 K3");
    r = Engine4.judgePlanetComparison(s, "intuition-decides"); s = r.state;
    if (r.ok || s.evidence.k3 || s.planetLab.comparisonAttempts.length !== 1)
      throw new Error("錯押直覺被當成理論檢驗，或錯誤判讀沒有留痕");
    r = Engine4.judgePlanetComparison(s, "theory-before-observation"); s = r.state;
    if (!r.ok || !s.evidence.k3 || !s.planetLab.comparisonSealed ||
        s.planetLab.comparisonAttempts.length !== 2)
      throw new Error("直覺押錯但正確核對兩筆理論預測，仍未成立 K3");

    let lucky = ch4CompleteK1(ch4K0K2State());
    lucky = Engine4.sealPlanetPrediction(lucky, "mars", "middle").state;
    lucky = Engine4.sealPlanetPrediction(lucky, "jupiter", "long").state;
    lucky = Engine4.revealPlanetPredictions(lucky).state;
    const luckyWrong = Engine4.judgePlanetComparison(lucky, "after-reveal-retune");
    if (luckyWrong.ok || luckyWrong.state.evidence.k3 ||
        luckyWrong.state.planetLab.comparisonAttempts[0].intuitionMatches !== 2)
      throw new Error("直覺全押中但比較錯誤，仍錯誤取得 K3");
...
tests.push({
  ...
  fn: () => {
    ...
    if (Engine4.beginLedgerRow(s, "moon").error !== "model-protocol-required")
      throw new Error("尚未訂公平標準即可開帳");
    r = Engine4.setModelProtocol(s, "retune-law-per-body"); s = r.state;
    if (r.ok || s.modelLab.protocolLocked ||
        s.modelLab.protocolAttempts.at(-1).patchTags !== 3)
      throw new Error("逐天體重調定律被當成公平標準，或錯誤選擇未留痕");
    r = Engine4.setModelProtocol(s, "shared-law-observed-initials"); s = r.state;
    if (!r.ok || !s.modelLab.protocolLocked ||
        s.modelLab.protocolAttempts.length !== 2)
      throw new Error("同規則、各用觀測初值的公平標準未能鎖定");
    if (Engine4.beginLedgerRow(s, "moon").error !== "two-model-predictions-required")
      throw new Error("兩本帳尚未事前押注即可開列");
    s = Engine4.sealModelPrediction(s, "inverseSquare", "one-law-three-skies").state;
    s = Engine4.sealModelPrediction(s, "simpleVortex", "patches-beyond-moon").state;
    if (!s.modelLab.predictions.inverseSquare.beforeRuns ||
        !s.modelLab.predictions.simpleVortex.beforeRuns ||
        !(s.modelLab.predictions.inverseSquare.sealedAt <
          s.modelLab.predictions.simpleVortex.sealedAt))
      throw new Error("兩張模型預測紙沒有在開帳前依序封存");
    ...
    if (s.modelLab.loans.length !== 1 || s.modelLab.loans[0].caseId !== "comet" ||
        s.modelLab.loans[0].patchedOutcome.independentlyTested !== false ||
        s.modelLab.loans[0].patchedOutcome.residual !== null ||
        Engine4.declineModelLoan(s, "comet").error !== "loan-decision-locked")
      throw new Error("借條不是玩家實際操作的 append-only 紀錄，或事後補法被假裝成獨立預測");
    ...
    if (!r.ok || !s.evidence.k4 || r.evidencePackage.loans.length !== 1 ||
        ...
        !r.claimText.includes("不等於已說明拉力如何穿過空間"))
    ...
    for (const hearingApi of ["setModelProtocol", "sealModelPrediction"])
      if (typeof Engine4[hearingApi] !== "function")
        throw new Error("現行 K4 聽證缺少玩家公平標準或事前預測 API:" + hearingApi);
    for (const legacyApi of ["runModelSuite", "runModel", "assertK4"])
...
tests.push({
  name: "CH4-CR-018|K4 一次一項、月亮→行星→彗星與人物接話",
  fn: () => {
    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    const start = ui.indexOf('ship3El("h3","一次只開一袋');
    const end = ui.indexOf('if (phase === "proof")', start);
    const modelsUi = ui.slice(start, end);
    for (const fragment of [
      'var caseSequence=["moon","planets","comet"]',
      'var currentModelDef=!activeStage.forceStamp',
      '一次只開一袋，讓兩套規則各自作答',
      '人物接話後，再看另一套規則',
      '現代重算，不是十七世紀留下的原始殘差表',
      '黃色補充紙'
    ]) if (!modelsUi.includes(fragment))
      throw new Error("K4 引導式逐案比較缺少契約：" + fragment);
    for (const oldWall of [
      '下一列先看誰？',
      '三列先看哪一列，由你決定',
      'orbit4Table(work,["完成次序"'
    ]) if (modelsUi.includes(oldWall))
      throw new Error("K4 仍保留自由跳列或常駐資訊牆：" + oldWall);
    for (const reply of [
      '它照同一條規則，算出月亮一秒會朝地球偏多少',
      '沒有數字可查',
      '固定同一張流速表',
      '高傾又逆向',
      '照同一套算法，火星和木星都對得上',
      '兩套規則都不必改，三份資料表現相同',
      '這個簡化漩渦對不上，所以所有漩渦說都錯',
      '平方那套不必逐項補規則；簡化漩渦需要更多事後補充'
    ]) if (!ui.includes(reply))
      throw new Error("K4 玩家判斷後缺人物接話：" + reply);

    let ordered = ch4CompleteK3(ch4CompleteK1(ch4K0K2State()));
    ordered = Engine4.deferPress(ordered, "等三份天空都比完").state;
    ordered = Engine4.setModelProtocol(ordered, "shared-law-observed-initials").state;
    ordered = Engine4.sealModelPrediction(
      ordered, "inverseSquare", "one-law-three-skies"
    ).state;
    ordered = Engine4.sealModelPrediction(
      ordered, "simpleVortex", "patches-beyond-moon"
    ).state;
    const beforeSkip = JSON.stringify(ordered);
    const skippedMoon = Engine4.beginLedgerRow(ordered, "planets");
    if (skippedMoon.error !== "model-case-sequence-required" ||
        JSON.stringify(skippedMoon.state) !== beforeSkip || JSON.stringify(ordered) !== beforeSkip)
      throw new Error("K4 狀態權威允許略過月亮先開行星");
    const moonOpened = Engine4.beginLedgerRow(ordered, "moon").state;
    const beforeParallel = JSON.stringify(moonOpened);
    const parallelRow = Engine4.beginLedgerRow(moonOpened, "planets");
    if (parallelRow.error !== "model-row-in-progress" ||
        JSON.stringify(parallelRow.state) !== beforeParallel || JSON.stringify(moonOpened) !== beforeParallel)
      throw new Error("K4 月亮尚未完成即可同時開第二袋");
    ...
    for (const gate of [
      ...
      '"ship", "lab", "orbit"'
    ]) if (!typewriter.includes(gate))
      throw new Error("K4 人物回應未真正鎖住底層工作台：" + gate);
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    const beforeEarlyScope = JSON.stringify(s);
    const scopedTooEarly = Engine4.setHookeScope(s, "precise-scope");
    if (scopedTooEarly.error !== "shell-page-placement-required" ||
        JSON.stringify(scopedTooEarly.state) !== beforeEarlyScope ||
        JSON.stringify(s) !== beforeEarlyScope)
      throw new Error("球殼頁尚未入槽即可先判 Hooke 範圍，或拒絕時改動 state");
    const earlyAcknowledgement = Engine4.acknowledgeCreditDispute(s);
    if (earlyAcknowledgement.error !== "shell-page-placement-required" ||
        JSON.stringify(earlyAcknowledgement.state) !== beforeEarlyScope)
      throw new Error("球殼頁尚未入槽即可先確認署名爭議");
    if (Engine4.clipEvidence(s, "K1").error !== "completed-proof-required")
      throw new Error("K5 尚未送出即可先夾頁並鎖死後續校樣");
    s = Engine4.assembleKnownProofSources(s).state;
    s = Engine4.judgeShellBalance(s, "matched-cones").state;
    ...
    const beforeCreditDispute = JSON.stringify(s);
    const bypassCreditDispute = Engine4.setHookeScope(s, "precise-scope");
    if (bypassCreditDispute.error !== "credit-dispute-acknowledgement-required" ||
        JSON.stringify(bypassCreditDispute.state) !== beforeCreditDispute ||
        JSON.stringify(s) !== beforeCreditDispute)
      throw new Error("新局未確認署名對話即可直跳 Hooke 範圍");
    const acknowledged = Engine4.acknowledgeCreditDispute(s); s = acknowledged.state;
    if (!acknowledged.ok || s.proof.creditDisputeAcknowledgements.length !== 1 ||
        s.proof.creditDisputeAcknowledgements[0].source !== "hooke-letter-1679" ||
        s.proof.creditDisputeAcknowledgements[0].at <= s.proof.shellPagePlacedAt)
      throw new Error("署名爭議對話沒有在球殼頁入槽後留下具名 trace");
    const beforeRepeatAcknowledgement = JSON.stringify(s);
    const repeatAcknowledgement = Engine4.acknowledgeCreditDispute(s);
    if (repeatAcknowledgement.error !== "credit-dispute-already-acknowledged" ||
        JSON.stringify(repeatAcknowledgement.state) !== beforeRepeatAcknowledgement)
      throw new Error("署名爭議確認可以重複寫入或拒絕時改動 state");
    s = Engine4.setHookeScope(s, "precise-scope").state;
    for (const [contribution, person] of Object.entries(Engine4._CREDIT_EXPECT))
      s = Engine4.assignCredit(s, contribution, person).state;
    s = Engine4.setBoundary(s, "ruleEstablished").state;
    ...
    for (const required of [
      "orbit4ProofPreviewText", "透光檢查目前校樣", "親手壓下校樣，送進印刷",
...
tests.push({
  name: "CH4-CR-018|K5 共同組證、球殼判讀與合併署名仍由真 trace 控制",
  fn: () => {
    const Sanitize = require("../src/sanitize.js");
    let s = ch4CompleteK4(ch4CompleteK3(ch4CompleteK1(ch4K0K2State())));
    let r = Engine4.assembleKnownProofSources(s); s = r.state;
    if (!r.ok || r.placed !== 5 || s.proof.slotAttempts.length !== 5 ||
        s.proof.slots.length !== 5)
      throw new Error("共同排紙沒有留下五筆既有來源 trace");
    const beforeWrong = s.sequence;
    r = Engine4.judgeShellBalance(s, "near-side-wins"); s = r.state;
    if (r.ok || s.proof.shellPageReady || s.proof.shellInsightAttempts.length !== 1 ||
        s.sequence !== beforeWrong + 1)
      throw new Error("球殼錯判沒有留痕，或錯判直接揭露證明頁");
    r = Engine4.judgeShellBalance(s, "matched-cones"); s = r.state;
    if (!r.ok || !s.proof.shellPageReady || s.proof.shellInsightAttempts.length !== 2 ||
        !(s.proof.shellPageRevealedAt > s.proof.shellInsightAttempts.at(-1).at))
      throw new Error("正確球殼判讀沒有先留玩家判斷再揭露證明頁");
    if (Engine4.assignCreditPlan(s, "work-by-work").error !== "hooke-scope-required")
      throw new Error("未處理 Hooke 範圍即可跳到完整分工");
    s = Engine4.placeShellPage(s).state;
    const preCreditDisputeLab = structuredClone(s);
    s = Engine4.acknowledgeCreditDispute(s).state;
    s = Engine4.setHookeScope(s, "precise-scope").state;
    r = Engine4.assignCreditPlan(s, "all-to-newton"); s = r.state;
    if (r.ok || r.consequence !== "credit-lines-break" ||
        s.proof.attributionAttempts.length !== 4)
      throw new Error("錯誤完整分工沒有留下四筆可修復的署名後果");
    r = Engine4.assignCreditPlan(s, "work-by-work"); s = r.state;
    if (!r.ok || s.proof.attributionAttempts.length !== 8 ||
        JSON.stringify(s.proof.attribution) !== JSON.stringify(Engine4._CREDIT_EXPECT))
      throw new Error("正確完整分工沒有以新一輪 append-only trace 修復");
    if (Engine4.previewProof(s).preview.complete)
      throw new Error("共同組證與分工完成後，旅人未退出／邊界未寫就自動完成");
    const N4Cr018 = Narrative._factory(scenes4, Engine4, {});
    const preCreditDisputeSave = N4Cr018.initialState("explore");
    preCreditDisputeSave.cursor = { scene:"D4-2", node:"e1" };
    preCreditDisputeSave.lab = preCreditDisputeLab;
    preCreditDisputeSave.evidence = { K1:true, K2:true, K3:true, K4:true };
    const beforeNarrativeBypass = JSON.stringify(preCreditDisputeSave);
    const narrativeBypass = N4Cr018.labAction(
      preCreditDisputeSave, "setHookeScope", { choice:"precise-scope" }
    );
    if (narrativeBypass.error !== "credit-dispute-acknowledgement-required" ||
        JSON.stringify(narrativeBypass.state) !== beforeNarrativeBypass ||
        JSON.stringify(preCreditDisputeSave) !== beforeNarrativeBypass)
      throw new Error("敘事層可在信用對話前直跳 Hooke 範圍，或拒絕時改動 state");
    const narrativeAcknowledgement = N4Cr018.labAction(
      preCreditDisputeSave, "acknowledgeCreditDispute"
    );
    const acknowledgementEvent = narrativeAcknowledgement.state.eventLog.at(-1);
    if (narrativeAcknowledgement.error ||
        narrativeAcknowledgement.state.lab.proof.creditDisputeAcknowledgements.length !== 1 ||
        acknowledgementEvent?.action !== "acknowledgeCreditDispute" ||
        acknowledgementEvent?.sequence !==
          narrativeAcknowledgement.state.lab.proof.creditDisputeAcknowledgements[0].at)
      throw new Error("署名爭議對話沒有同時留下 engine trace 與敘事 lab event");
    const save = N4Cr018.initialState("explore");
    save.cursor = { scene:"D4-2", node:"e1" };
    save.lab = s;
    save.evidence = { K1:true, K2:true, K3:true, K4:true };
    const claimEvents = Object.values(s.claims).flat().map((row) => {
      const event = { t:"lab", action:row.action, sequence:row.at, at:"D4-2/e1" };
      if (["assertK1","assertK2","assertK3","assertK4"].includes(row.action))
        event.args = { records:row.sources.slice(), concept:row.concept, claim:row.concept };
      return event;
    });
    save.eventLog = [{
      t:"lab", action:"deferPress",
      sequence:s.proof.press.delays[0].at, at:"D4-1/e1"
    }].concat(claimEvents).concat((s.modelLab.protocolAttempts || []).map((row) => ({
      t:"lab", action:"setModelProtocol", sequence:row.at, at:"D4-1/e1"
    }))).concat(Object.keys(s.modelLab.predictions || {}).map((model) => ({
      t:"lab", action:"sealModelPrediction",
      sequence:s.modelLab.predictions[model].sealedAt, at:"D4-1/e1"
    }))).concat((s.proof.creditDisputeAcknowledgements || []).map((row) => ({
      t:"lab", action:"acknowledgeCreditDispute",
      sequence:row.at, at:"D4-2/e1"
    }))).sort((a,b) => a.sequence - b.sequence);
    const cleaned = Sanitize.sanitizeImport4(save, scenes4, Engine4);
    if (!cleaned.ok || cleaned.state.lab.proof.shellInsightAttempts.length !== 2 ||
        cleaned.state.lab.proof.creditDisputeVersion !== "dialogue-v1" ||
        cleaned.state.lab.proof.creditDisputeAcknowledgements.length !== 1)
      throw new Error("球殼判讀／合併署名完成後的 schema2 中途存檔不能原地續接：" +
        (cleaned.reason || "unknown"));
    const missingCreditEvent = structuredClone(save);
    missingCreditEvent.eventLog = missingCreditEvent.eventLog.filter((event) =>
      event.action !== "acknowledgeCreditDispute"
    );
    if (Sanitize.sanitizeImport4(missingCreditEvent, scenes4, Engine4).ok)
      throw new Error("刪掉信用對話 lab event 後仍能保留 engine trace 匯入");
    const missingCreditTrace = structuredClone(save);
    missingCreditTrace.lab.proof.creditDisputeAcknowledgements = [];
    if (Sanitize.sanitizeImport4(missingCreditTrace, scenes4, Engine4).ok)
      throw new Error("刪掉信用對話 engine trace 後仍能保留 lab event 匯入");
    const legacyCreditSave = structuredClone(save);
    legacyCreditSave.lab = structuredClone(preCreditDisputeLab);
    legacyCreditSave.lab.proof.creditDisputeVersion = "legacy-past-scope";
    legacyCreditSave.lab.proof.creditDisputeAcknowledgements = [];
    legacyCreditSave.lab = Engine4.setHookeScope(
      legacyCreditSave.lab, "precise-scope"
    ).state;
    delete legacyCreditSave.lab.proof.creditDisputeVersion;
    delete legacyCreditSave.lab.proof.creditDisputeAcknowledgements;
    legacyCreditSave.eventLog = legacyCreditSave.eventLog.filter((event) =>
      event.action !== "acknowledgeCreditDispute"
    );
    const legacyCredit = Sanitize.sanitizeImport4(legacyCreditSave, scenes4, Engine4);
    if (!legacyCredit.ok ||
        legacyCredit.state.lab.proof.creditDisputeVersion !== "legacy-past-scope" ||
        legacyCredit.state.lab.proof.creditDisputeAcknowledgements.length !== 0 ||
        Engine4.setHookeScope(legacyCredit.state.lab, "precise-scope").error)
      throw new Error("功能上線前已跨過 Hooke 判讀的 schema2 舊檔不能原地續接：" +
        (legacyCredit.reason || "unknown"));
    const forgedShell = structuredClone(save);
    forgedShell.lab.proof.shellInsightAttempts[0].choice = "matched-cones";
    if (Sanitize.sanitizeImport4(forgedShell, scenes4, Engine4).ok)
      throw new Error("篡改球殼判讀答案仍能通過匯入淨化");

    if (typeof Engine4.revealShellPage === "function")
      throw new Error("舊球殼揭頁 API 仍可繞過玩家判讀");
    const bypass = N4Cr018.labAction(save, "revealShellPage");
    if (bypass.error !== "未知實驗台動作:revealShellPage" ||
        JSON.stringify(bypass.state) !== JSON.stringify(save))
      throw new Error("敘事層仍可直接翻出球殼頁");

    const almostFullSlots = structuredClone(
      ch4CompleteK4(ch4CompleteK3(ch4CompleteK1(ch4K0K2State())))
    );
    almostFullSlots.proof.slotAttempts = Array.from({length:98}, (_,index) => ({id:index+1}));
    const beforeAtomicSources = JSON.stringify(almostFullSlots);
    const overflowSources = Engine4.assembleKnownProofSources(almostFullSlots);
    if (overflowSources.error !== "proof-attempt-limit" ||
        JSON.stringify(overflowSources.state) !== beforeAtomicSources ||
        JSON.stringify(almostFullSlots) !== beforeAtomicSources)
      throw new Error("共同排紙超過留痕上限時仍寫入部分狀態");

    const archivedCredit = structuredClone(s);
    archivedCredit.archiveLab = { clipped:["K1"], clipAttempts:[{id:1,evidenceId:"K1",at:1}], complete:false };
    const beforeAtomicCredit = JSON.stringify(archivedCredit);
    const creditAfterArchive = Engine4.assignCreditPlan(archivedCredit, "work-by-work");
    if (creditAfterArchive.error !== "archive-records-locked" ||
        JSON.stringify(creditAfterArchive.state) !== beforeAtomicCredit ||
        JSON.stringify(archivedCredit) !== beforeAtomicCredit)
      throw new Error("合併分工在歸檔後仍寫入部分署名");

    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    const proofStart = ui.indexOf('if (phase === "proof")', ui.indexOf('function renderOrbit4'));
    const proofUi = ui.slice(proofStart,
      ui.indexOf('if (phase === "archive")', proofStart));
    for (const fragment of [
      '和牛頓一起把五張紙排好',
      '近側比較近，遠側卻有更大一片',
      'matched-cones',
      ...
      '不用再逐格填名',
      '親手壓下校樣，送進印刷'
    ]) if (!proofUi.includes(fragment))
      throw new Error("K5 對話式球殼／短出版尾聲缺契約：" + fragment);
    const creditDialogueStart = ui.indexOf('action === "acknowledgeCreditDispute" && rr.ok');
    const creditDialogueEnd = ui.indexOf('} else if (action === "judgeShellBalance"', creditDialogueStart);
    const creditDialogue = ui.slice(creditDialogueStart, creditDialogueEnd);
    for (const line of [
      '虎克的兩封信還放在版框旁',
      '這兩封信不能替你證明三百頁',
      '他還是會說整本都是他的',
      '他做過什麼，只寫到那裡',
      'hasCreditChoice("curve")',
      'hasCreditChoice("inward")',
      'hasCreditChoice("tangent")'
    ]) if (!creditDialogue.includes(line))
      throw new Error("K5 確認署名爭議後缺少動作綁定對話／讀回：" + line);
    if (creditDialogueStart < 0 || creditDialogueEnd < creditDialogueStart)
      throw new Error("K5 署名爭議對話沒有綁在單次 lab action 成功分支");
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    s.scaleLab.conversionAttempts = Array.from({ length:30 }, (_, i) => ({
      choice:"arc-length", ok:false, at:i + 3
    }));
    expectLocked("K2", s,
      (state) => Engine4.convertMoonTime(state, "arc-length"),
      "scale-attempt-limit");
...
tests.push({
  name: "第四章全章走查|12 場主線、K0/K2 對話判讀、四座工作台、五證據與 schema2 完章淨化",
  fn: () => {
    const N4 = Narrative._factory(scenes4, Engine4, {});
    ...
    while (!s.ended && guard++ < 600) {
      ...
      if (view.type === "choice") {
        const choice = view.scene === "D0-2" && view.nodeId === "c0" ? "evidence-boundary" :
          ...
          (view.scene === "D1-1" && view.nodeId === "c2" ? "gap" :
          (view.scene === "D1-2" && view.nodeId === "c_geom" ? "gap" :
          (view.scene === "D1-2" && view.nodeId === "c_ratio" ? "r3600" :
          (view.scene === "D1-2" && view.nodeId === "c_relation" ? "multiply" :
      ...
      if (view.type === "embed" && view.system === "orbit") {
        if (view.phase === "tangent-seal") {
          ...
          act("convertMoonTime", { choice:"arc-length" });
          act("convertMoonTime", { choice:"sagitta-geometry" });
          ...
          act("runOrbitPaperTrial", { speed:"slow", strength:"short" });
          act("runOrbitPaperTrial", { speed:"medium", strength:"medium" });
          act("runOrbitPaperTrial", { speed:"medium", strength:"short" });
          act("assertK1", {
            records:["tangent", "trial:1", "trial:2", "trial:3"],
            concept:"forward-push"
          });
          act("assertK1", {
            records:["tangent", "trial:1", "trial:2", "trial:3"],
            concept:"forward-plus-inward-turn"
          });
          act("runOrbitPaperTrial", { speed:"fast", strength:"long" });
          act("assertK1", {
            records:["tangent", "trial:1", "trial:3", "trial:4"],
            concept:"forward-plus-inward-turn"
          });
          ...
          act("judgePlanetComparison", { choice:"intuition-decides" });
          act("judgePlanetComparison", { choice:"theory-before-observation" });
        } else if (view.phase === "models") {
          act("deferPress", { reason:"等待三列對帳完成" });
          act("setModelProtocol", { protocol:"retune-law-per-body" });
          act("setModelProtocol", { protocol:"shared-law-observed-initials" });
          act("sealModelPrediction", {
            model:"inverseSquare", prediction:"one-law-three-skies"
          });
          act("sealModelPrediction", {
            model:"simpleVortex", prediction:"patches-beyond-moon"
          });
          ...
          act("assembleKnownProofSources");
          act("judgeShellBalance", { choice:"matched-cones" });
          act("placeShellPage");
          act("submitProof");
          act("acknowledgeCreditDispute");
    ...
    const legacyK2Precision = JSON.parse(JSON.stringify(finished));
    legacyK2Precision.lab.scaleLab.moonOneSecondSagMm = 1.4;
    legacyK2Precision.lab.scaleLab.trials.forEach((trial) => {
      if (trial.moonSagM === 4.9) trial.moonErrorPct = 0;
    });
    const legacyK2Before = JSON.stringify(legacyK2Precision);
    const normalizedK2 = San.sanitizeImport4(legacyK2Precision, scenes4, Engine4);
    if (!normalizedK2.ok ||
        normalizedK2.state.lab.scaleLab.moonOneSecondSagMm !== 1.36 ||
        normalizedK2.state.lab.scaleLab.trials.some((trial) =>
          trial.moonSagM === 4.9 && trial.moonErrorPct !== 0.4) ||
        JSON.stringify(legacyK2Precision) !== legacyK2Before)
      throw new Error("合法舊 K2 顯示精度未正規化為真實幾何，或 sanitizer 改動輸入 state");
    ...
    if (!tangentRedirect.ok || tangentRedirect.state.cursor.scene !== "D1-1" ||
        tangentRedirect.state.cursor.node !== "g1")
    ...
    rejectMutation("K1 刪掉一張玩家試跑紙", (forged) => {
      forged.lab.orbitLab.paperTrials.pop();
    });
    rejectMutation("K1 篡改紙上試跑半徑", (forged) => {
      forged.lab.orbitLab.paperTrials.at(-1).minRadius = 999;
    });
    rejectMutation("K1 試跑紙倒序後重編號", (forged) => {
      forged.lab.orbitLab.paperTrials.reverse()
        .forEach((run, index) => { run.id = index + 1; });
      forged.lab.orbitLab.paperTrialActiveId = 1;
    });
    ...
    rejectMutation("K1 刪掉紙上試跑操作事件", (forged) => {
      const ranAt = forged.lab.orbitLab.paperTrials[0].ranAt;
      forged.eventLog = forged.eventLog.filter((event) =>
        !(event && event.t === "lab" &&
          event.action === "runOrbitPaperTrial" && event.sequence === ranAt));
    });
    rejectMutation("K1 新試跑混入舊版規則紙", (forged) => {
      forged.lab.orbitLab.ruleSeal = {
        target:"earth-center", speed:"medium", strength:"medium",
        prediction:"circle", sealedAt:1
      };
    });
    ...
    rejectMutation("K1 篡改目前 complete", (forged) => {
      forged.lab.orbitLab.complete = true;
    });
    rejectMutation("K1 篡改目前 step", (forged) => {
      forged.lab.orbitLab.step = 2;
    });
    rejectMutation("K1 篡改目前試跑紙 ID", (forged) => {
      forged.lab.orbitLab.paperTrialActiveId = 1;
    });
    rejectMutation("K1 篡改目前 position", (forged) => {
      forged.lab.orbitLab.position.x += 1;
    });
    rejectMutation("K1 篡改最後成功三紙選集", (forged) => {
      const successful = forged.lab.claims.k1.filter((claim) => claim.ok).at(-1);
      successful.sources = ["tangent", "trial:1", "trial:2", "trial:4"];
    });
    rejectMutation("K2 篡改月球每秒下墜量", (forged) => {
      forged.lab.scaleLab.moonOneSecondSagMm = 999;
    });
    rejectMutation("K2 篡改成功幾何讀法", (forged) => {
      forged.lab.scaleLab.conversionAttempts.find((row) => row.ok).choice = "arc-length";
    });
    rejectMutation("K2 篡改共同計算時間線", (forged) => {
      forged.lab.scaleLab.conversionAttempts.find((row) => row.ok).at =
        forged.lab.scaleLab.ratioAttempts.find((row) => row.ok).at;
    });
    ...
    rejectMutation("K3 刪除玩家揭曉後判讀", (forged) => {
      forged.lab.planetLab.comparisonAttempts = [];
      forged.lab.planetLab.comparisonClaim = null;
      forged.lab.planetLab.comparisonSealed = false;
      forged.lab.planetLab.comparisonSealedAt = null;
    });
    rejectMutation("K3 判讀缺敘事層玩家操作", (forged) => {
      forged.eventLog = forged.eventLog.filter((event) =>
        !(event && event.t === "lab" && event.action === "judgePlanetComparison")
      );
    });
    rejectMutation("K4 篡改公平比較協定", (forged) => {
      forged.lab.modelLab.protocolAttempts.at(-1).protocol = "same-start-all";
    });
    rejectMutation("K4 刪除一張開帳前預測", (forged) => {
      delete forged.lab.modelLab.predictions.simpleVortex;
    });
    rejectMutation("K4 預測紙晚於第一列開帳", (forged) => {
      forged.lab.modelLab.predictions.simpleVortex.sealedAt =
        forged.lab.modelLab.rowStage.moon.openedAt;
    });
    rejectMutation("K4 聽證缺敘事層玩家操作", (forged) => {
      const index = forged.eventLog.findIndex((event) =>
        event && event.t === "lab" && event.action === "sealModelPrediction"
      );
      forged.eventLog.splice(index, 1);
    });
    ...
    rejectMutation("K4 事後借條假裝成獨立預測", (forged) => {
      forged.lab.modelLab.loans[0].patchedOutcome.independentlyTested = true;
      forged.lab.modelLab.evidencePackage.loans =
        JSON.parse(JSON.stringify(forged.lab.modelLab.loans));
    });
...
tests.push({
  ...
  fn: () => {
    ...
    for (const fragment of [
      ...
      '"orbit-rule": "vectors"', "sealTangentPrediction",
      "runOrbitPaperTrial", "commitOrbitBeat", "continueOrbitRule", "sealPlanetPrediction",
      "revealPlanetPredictions", "beginLedgerRow", "stampLedgerCell",
      "addModelLoan", "declineModelLoan", "sealModelComparison", "judgeShellBalance",
    ...
    const scaleSheetsEnd = ui.indexOf("function orbit4ScaleGeometry(parent, sc)", scaleSheetsStart);
    ...
    if (/createElementNS|<svg|canvas|getContext/.test(scaleSheetsUi))
      throw new Error("第四章紙張氣氛底圖混入精確幾何；正矢應由獨立 SVG 承擔");
    ...
    if (sharedSpread.includes("orbitHeadCopy") || !ui.includes('"兩條路分開算，最後再疊起來"') ||
        !ui.includes('"這兩張紙從哪裡來？"') ||
        !ui.includes("選兩個端點之間的短差"))
      throw new Error("第四章同尺紙仍保留滿版說明牆，或缺少白話當前任務");
    if (!ui.includes('"arc-is-not-sag"') || !ui.includes("return orbit4Error(rr.reason || rr.consequence)"))
      throw new Error("正矢錯讀沒有共用錯誤碼與可重試回饋");
    for (const fragment of [
      "月球紙｜你盯端點，牛頓核天文尺", "兩張一秒紙｜並排讀數", "疊合台｜兩條獨立路徑",
      "月距約 60R；你選擇 60×60≈3600", "約 1.36 mm",
      "發現後，把極小差距放大成 60 秒"
    ...
    if (scaleUi.includes('doOrbit("sealScalePrediction"') ||
        scaleUi.includes("先押量級") || scaleUi.includes("封存這個答案"))
      throw new Error("K2 新局仍要求玩家先猜尚未計算的月球量級");
    for (const fragment of [
      "background:rgba(255,250,236,.46)",
      ".orbitScaleGeometry .orbitStringLabel",
      ".orbitScaleOverlay.complete"
    ]) if (!html.includes(fragment))
      throw new Error("K2 米白紙面或端點標示樣式未鎖定:" + fragment);
    ...
    const catUiStart = ui.indexOf("/* ---------- 第二章彈射工坊面板");
    const catUiEnd = ui.indexOf("/* ---------- 第三章航船實驗", catUiStart);
    const catUi = ui.slice(catUiStart, catUiEnd);
    if (!catUi.includes("collision5Table catDataTable") ||
        !catUi.includes('input.type = "checkbox"') ||
        !catUi.includes('selectionMode === "compare" ? s.status === "complete" && !!s.accepted') ||
        !catUi.includes("在上方同一張表勾兩組") ||
        !catUi.includes("我主張：兩組只換了球") || !html.includes(".catCompareClaim") ||
        !catUi.includes("compareButton.disabled = !valid || !claimCheck.checked") ||
        catUi.includes("先選銅球紀錄") || catUi.includes("再選木球紀錄"))
      throw new Error("第二章換球比較仍讓不可引用紀錄進入斷言");
    if (!stageUi.includes("知道了，開始畫幾何") ||
        !stageUi.includes("地表已記 1 秒；月球只給距離 60R 與週期 27.3 日") ||
        !stageUi.includes("先從圖上指出切線端點與實際端點之間的短差") ||
...
tests.push({
  name: "第五章 E-3 展開|十四個選擇節點、零空殼場、心聲用途與舊節點隔離",
  fn: () => {
    const allowedPurposes = new Set([
      "time_dislocation", "future_knowledge", "cross_chapter_memory",
      "private_observation", "private_hypothesis", "emotional_risk", "naming"
    ...
    if (wrong.error || wrong.state.flags.ch5ScaleBoundary ||
        wrong.state.flags.ch5ScaleClaim !== "vis-viva-open")
      throw new Error("過度宣稱竟取得範圍承諾");
    const committed = N5.advance(wrong.state);
    if (committed.error || N5.view(committed.state).nodeId !== "v_commit" ||
        N5.view(N5.advance(committed.state).state).nodeId !== "n1a")
      throw new Error("範圍過度宣稱沒有帶著原句前進");
    const rightBase = N5.initialState("explore");
    rightBase.cursor = { scene: "E2-3", node: "c_scale" };
    const right = N5.choose(rightBase, "measurement-only");
    if (right.error || right.state.flags.ch5ScaleBoundary !== "measurement-only" ||
        right.state.flags.ch5ScaleClaim !== "measurement-only")
...
tests.push({
  name: "第五章三輪工作台|輪二零實驗、同批重算、2b 可達、J3 前兩筆預測第三筆",
  fn: () => {
    ...
    let heavyClay = Engine5.setDraft(s, "ballMass", "heavy").state;
    heavyClay = Engine5.runClay(heavyClay).state;
    heavyClay = Engine5.runClay(heavyClay).state;
    heavyClay = Engine5.sealClayPrediction(heavyClay, "band-middle").state;
    if (heavyClay.clayPrediction.min !== 8 || heavyClay.clayPrediction.max !== 9.6)
      throw new Error("重球預測範圍沒有依同一質量倍率換算");
    heavyClay = Engine5.runClay(heavyClay).state;
    if (heavyClay.clayRuns[2].depth !== 8.9 || heavyClay.clayPrediction.matched !== true)
      throw new Error("重球第三球揭曉或命中判定不符");
    if (Engine5.sealClayPrediction(s, "band-middle").error !== "clay-two-records-required")
      throw new Error("沒有前兩筆就能封存第三球預測");
    act("runClay");
    if (Engine5.sealClayPrediction(s, "band-middle").error !== "clay-two-records-required")
      throw new Error("只有第一筆就能封存第三球預測");
    act("runClay");
    if (Engine5.runClay(s).error !== "clay-prediction-required")
      throw new Error("未封存坑深範圍就能揭曉第三球");
    const sealedClay = act("sealClayPrediction", "band-low").prediction;
    if (sealedClay.min !== 2.8 || sealedClay.max !== 3.6 || sealedClay.revealed)
      throw new Error("第三球預測紙沒有保存數值範圍");
    if (Engine5.sealClayPrediction(s, "band-middle").error !== "clay-prediction-sealed")
      throw new Error("封存後仍能改寫第三球預測");
    act("runClay");
    ...
    if (!s.clayPrediction.revealed || s.clayPrediction.actualDepth !== 4.4 ||
        s.clayPrediction.matched !== false ||
        Engine5.runClay(s).error !== "clay-series-complete")
      throw new Error("第三球沒有如實揭曉偏離，或三球後仍可洗資料");
...
tests.push({
  ...
  fn: () => {
    ...
    while (!s.ended && guard++ < 200) {
      ...
      if (v.type === "embed" && v.system === "collision") {
        if (s.lab.phase === "momentum") {
          ...
          lab("sealClayPrediction", { band: "band-middle" });
          lab("runClay");
          lab("assertJ3", {
    ...
    const legacyClaySave = JSON.parse(JSON.stringify(finished5));
    delete legacyClaySave.lab.clayProtocol;
    delete legacyClaySave.lab.clayPrediction;
    const migratedLegacyClay = San.sanitizeImport5(legacyClaySave, scenes5);
    if (!migratedLegacyClay.ok || migratedLegacyClay.state.lab.clayProtocol !== "legacy-free-v0" ||
        migratedLegacyClay.state.lab.clayPrediction !== null)
      throw new Error("第五章舊黏土存檔被倒補成曾封存第三球預測");
    ...
    const forgedClayPrediction = JSON.parse(JSON.stringify(finished5));
    forgedClayPrediction.lab.clayPrediction.min = 0;
    if (San.sanitizeImport5(forgedClayPrediction, scenes5).ok)
      throw new Error("竄改第三球封存範圍後仍可通過匯入");
    const missingClayPrediction = JSON.parse(JSON.stringify(finished5));
    missingClayPrediction.lab.clayPrediction = null;
    if (San.sanitizeImport5(missingClayPrediction, scenes5).ok)
      throw new Error("新流程第三球已揭曉卻沒有預測紙仍可通過匯入");
    ...
    for (const fragment of ["renderCollision5", "round2-no-new-experiment",
      ...
      "sealClayPrediction", "從九格落下第三球，揭曉預測"])
...
tests.push({
  name: "共同發現感跨章契約|旅人完成連結，科學家以核對、簽名或範圍回應",
  fn: () => {
    const mapChapter = (chapter) => new Map(chapter.scenes.map((scene) => [
      scene.id, new Map(scene.nodes.map((node) => [node.id, node]))
    ]));
    const maps = [scenes, scenes2, scenes3, scenes4, scenes6].map(mapChapter);
    const [m1, m2, m3, m4, m6] = maps;
    const node = (map, sceneId, nodeId) => map.get(sceneId)?.get(nodeId) || {};

    const ch1Stage = node(m1, "A2-2", "n3b");
    const ch1Traveler = node(m1, "A2-2", "n4");
    const ch1Galileo = node(m1, "A2-2", "n4a");
    if (ch1Stage.speaker !== "stage" || ch1Stage.next !== "n4" ||
        ch1Traveler.speaker !== "旅人(你)" || ch1Traveler.next !== "n4a" ||
        !String(ch1Traveler.text || "").includes("時間的平方") ||
        ch1Galileo.speaker !== "伽利略" || !String(ch1Galileo.text || "").includes("倒算"))
      throw new Error("第一章共同讀紙沒有保留旅人命名、伽利略反算的交棒");

    const ch1Paradox = node(m1, "A1-7", "m1");
    const ch1ParadoxCheck = node(m1, "A1-7", "m2");
    const ch1BallRead = node(m1, "A2-3", "n5");
    const ch1BallCheck = node(m1, "A2-3", "n5a");
    const ch1SlopeRead = node(m1, "A2-3", "nsch");
    const ch1SlopeCheck = node(m1, "A2-3", "nscha");
    if (ch1Paradox.speaker !== "旅人(你)" ||
        !String(ch1Paradox.text || "").includes("同一個前提") ||
        ch1ParadoxCheck.speaker !== "伽利略" ||
        ch1BallRead.speaker !== "旅人(你)" || ch1BallRead.next !== "n5a" ||
        ch1BallCheck.speaker !== "伽利略" || !String(ch1BallCheck.text || "").includes("逐格讀") ||
        ch1SlopeRead.speaker !== "旅人(你)" || ch1SlopeRead.next !== "nscha" ||
        ch1SlopeCheck.speaker !== "伽利略" || !String(ch1SlopeCheck.text || "").includes("逐行核對"))
      throw new Error("第一章悖論、換球或換坡交棒退回科學家代答");

    const ch2P1 = node(m2, "B2-3", "p1");
    const ch2N4 = node(m2, "B2-3", "n4");
    const ch2Signature = node(m2, "B2-4", "n3");
    if (ch2P1.speaker !== "旅人(你)" || ch2P1.next !== "n5" ||
        !String(ch2P1.text || "").includes("先換球查重量") || ch2N4.legacyOnly !== true ||
        ch2Signature.speaker !== "伽利略" ||
        !/簽名/.test(String(ch2Signature.text || "")) ||
        !/劃掉/.test(String(ch2Signature.text || "")) ||
        !/能分辨的範圍/.test(String(ch2Signature.text || "")))
      throw new Error("第二章判讀換手、下一步或簽名範圍回應漂移");

    const ch3Question = node(m3, "C3-1", "n1");
    const ch3Claim = node(m3, "C3-1", "x1");
    const ch3Written = node(m3, "C3-1", "x2");
    const ch3Bounded = ch3Claim.options?.find((option) => option.id === "bounded");
    const ch3Over = ch3Claim.options?.find((option) => option.id === "earth-moves");
    const ch3Under = ch3Claim.options?.find((option) => option.id === "nothing-learned");
    if (ch3Question.speaker !== "官員" || ch3Question.next !== "x1" ||
        ch3Claim.type !== "choice" || ch3Claim.options?.length !== 3 ||
        ch3Bounded?.next !== "x2" ||
        !String(ch3Bounded?.text || "").includes("沒有直接量") ||
        ch3Over?.next !== "x1_over" || node(m3, "C3-1", "x1_over").next !== "x1" ||
        ch3Under?.next !== "x1_under" || node(m3, "C3-1", "x1_under").next !== "x1" ||
        ch3Written.speaker !== "stage" || !String(ch3Written.text || "").includes("逐字"))
      throw new Error("第三章公開有限主張不再由旅人提交並被逐字記錄");

    const ch4Gap = node(m4, "D1-1", "ok1c");
    const ch4Definition = node(m4, "D1-1", "ok2");
    const ch4Measure = node(m4, "D1-1", "ok3");
    const ch4Readback = node(m4, "D1-2", "n10");
    const ch4Bridge = node(m4, "D1-2", "n11b");
    const ch4Relation = node(m4, "D1-2", "c_relation");
    const ch4Discovery = node(m4, "D1-2", "n11c");
    const ch4Check = node(m4, "D1-2", "n11d");
    const ch4Confirm = node(m4, "D1-2", "n12");
    const ch4TravelerScale = node(m4, "D1-2", "n12a");
    const ch4NewtonBoundary = node(m4, "D1-2", "n12c");
    const ch4ScaleAfterglow = node(m4, "D1-2", "n12b");
    if (ch4Gap.speaker !== "stage" || ch4Gap.next !== "ok1d" ||
        ch4Definition.speaker !== "旅人(你)" || ch4Definition.next !== "ok3" ||
        !String(ch4Definition.text || "").includes("切線端點") ||
        ch4Measure.speaker !== "牛頓" || !String(ch4Measure.text || "").includes("沒被拉") ||
        !String(ch4Measure.text || "").includes("往地球偏") ||
        /平方/.test(String(ch4Readback.text || "")) ||
        ch4Bridge.speaker !== "牛頓" || String(ch4Bridge.text || "").includes("你看見什麼") ||
        ch4Relation.speaker !== "system" || !String(ch4Relation.text || "").includes("兩人都沒說話") ||
        ch4Discovery.speaker !== "旅人(你)" || !String(ch4Discovery.text || "").includes("六十乘六十") ||
        !String(ch4Discovery.text || "").includes("啊！") ||
        ch4Check.speaker !== "牛頓" || !String(ch4Check.text || "").includes("等等") ||
        !String(ch4Check.text || "").includes("再算一遍") ||
        ch4Confirm.speaker !== "牛頓" || !String(ch4Confirm.text || "").includes("同樣一秒") ||
        !String(ch4Confirm.text || "").includes("遠約六十倍") ||
        !String(ch4Confirm.text || "").includes("三千六百分之一") ||
        ch4TravelerScale.speaker !== "旅人(你)" ||
        !String(ch4TravelerScale.text || "").includes("六十平方分之一") ||
        ch4NewtonBoundary.speaker !== "牛頓" ||
        !String(ch4NewtonBoundary.text || "").includes("別急著叫它定律") ||
        ch4ScaleAfterglow.speaker !== "stage" || ch4ScaleAfterglow.next !== "n13")
      throw new Error("第四章端點定義、旅人發現或牛頓獨立核對的順序漂移");

    const ch6Inner = node(m6, "H3-1", "a1");
    const ch6Public = node(m6, "H3-1", "a1b");
    if (ch6Inner.speaker !== "旅人・心聲" || ch6Inner.next !== "a1b" ||
        ch6Public.speaker !== "旅人(你)" || ch6Public.next !== "a2" ||
        !String(ch6Public.text || "").includes("炮紙沒有因此變假") ||
        node(m6, "H1-1", "a1b").id)
      throw new Error("第六章內心成本與公開承認未決的接線錯位");

    const assets = JSON.parse(readFileSync(path.join(here, "../data/assets.json"), "utf-8"));
    const focusContracts = [
      ["A2-2", "兩支筆刮過紙面的聲音", "ch01_focus_shared_paper_calculation_v01"],
      ["B2-3", "先換球查重量", "ch02_focus_evidence_next_test_v01"],
      ["C3-1", "官員逐字把旅人的話寫進結果欄", "ch03_focus_public_limited_claim_v01"],
      ["D1-2", "兩人各重做一遍", "ch04_focus_shared_moon_calculation_v01"],
      ["H1-1", "旅人把兩條紙帶的起點對齊", "ch06_focus_four_hands_strip_alignment_v01"],
      ["H3-1", "炮紙沒有因此變假", "ch06_focus_public_blank_admission_v01"]
    ];
    const allScenes = [scenes, scenes2, scenes3, scenes4, scenes6]
      .flatMap((chapter) => chapter.scenes);
    for (const [sceneId, match, assetId] of focusContracts) {
      const rule = assets.lineFocusVisual.find((entry) =>
        entry.scene === sceneId && entry.match === match &&
        entry.items?.some((item) => item.asset === assetId));
      const scene = allScenes.find((entry) => entry.id === sceneId);
      const asset = assets.entries.find((entry) => entry.id === assetId);
      const runtimePath = asset && path.join(here, "..", assets.basePath, asset.path);
      if (!rule || !scene?.nodes.some((entry) => String(entry.text || "").includes(match)) ||
          !asset || asset.w !== 1672 || asset.h !== 941 || !existsSync(runtimePath))
        throw new Error("共同發現焦點圖未接到真實台詞或 runtime 檔案:" + assetId);
    }
  }
});

tests.push({
  name: "GB-ADR-044|六章關係鏡頭後才切紙面閱讀，稀疏已知文字不洩答",
  fn: () => {
    const assets = JSON.parse(readFileSync(path.join(here, "../data/assets.json"), "utf-8"));
    const chapterData = { ch01: scenes, ch02: scenes2, ch03: scenes3,
      ch04: scenes4, ch05: scenes5, ch06: scenes6 };
    const contracts = [
      {
        chapter: "ch01", pairId: "shared-discovery-ch01",
        relationship: ["A2-2", "兩支筆刮過紙面的聲音", "ch01_focus_shared_paper_calculation_v01"],
        reading: ["A2-2", "等一下——一、四、九、十六、二十五", "ch01_focus_square_pattern_reading_v01"],
        embeddedText: ["1 4 9 16 25"]
      },
      {
        chapter: "ch02", pairId: "shared-discovery-ch02",
        relationship: ["B2-3", "先換球查重量", "ch02_focus_evidence_next_test_v01"],
        reading: ["B2-3", "銅球與木球的四列沙痕並排", "ch02_focus_ball_comparison_reading_v01"],
        embeddedText: ["銅球", "木球"]
      },
      {
        chapter: "ch03", pairId: "shared-discovery-ch03",
        relationship: ["C3-1", "官員逐字把旅人的話寫進結果欄", "ch03_focus_public_limited_claim_v01"],
        reading: ["C3-1", "逐字讀完才拿起筆", "ch03_focus_limited_claim_reading_v01"],
        embeddedText: ["排除", "未直接量"]
      },
      {
        chapter: "ch04", pairId: "shared-discovery-ch04",
        relationship: ["D1-2", "兩人各重做一遍", "ch04_focus_shared_moon_calculation_v01"],
        reading: ["D1-2", "月亮這張，約一點三六毫米", "ch04_focus_one_second_papers_reading_v01"],
        embeddedText: ["地表 1 秒：約 4.9 m", "月球 1 秒：約 1.36 mm", "待比較"]
      },
      {
        chapter: "ch05", pairId: "shared-discovery-ch05",
        relationship: ["E2-2", "她把手按在那六張紀錄紙上", "ch05_focus_same_six_records_relation_v01"],
        reading: ["E2-2", "這一組……少了一截", "ch05_focus_two_ledgers_reading_v01"],
        embeddedText: ["動量帳", "活力帳"]
      },
      {
        chapter: "ch06", pairId: "shared-discovery-ch06",
        relationship: ["H1-1", "旅人把兩條紙帶的起點對齊", "ch06_focus_four_hands_strip_alignment_v01"],
        reading: ["H1-1", "這組紙最多支持到哪裡？", "ch06_focus_heat_strips_reading_v01"],
        embeddedText: ["碎屑", "薄片"]
      }
    ];

    const findTrigger = ([sceneId, match, assetId]) => assets.lineFocusVisual.find((rule) =>
      rule.scene === sceneId && rule.match === match &&
      rule.items?.length === 1 && rule.items[0].asset === assetId);
    const assetUses = (assetId) => assets.lineFocusVisual.filter((rule) =>
      rule.items?.some((item) => item.asset === assetId));

    for (const contract of contracts) {
      const chapter = chapterData[contract.chapter];
      const pair = assets.entries.filter((entry) => entry.pairId === contract.pairId);
      const relationAsset = pair.find((entry) => entry.shotRole === "relationship");
      const readingAsset = pair.find((entry) => entry.shotRole === "reading");
      if (pair.length !== 2 || !relationAsset || !readingAsset || relationAsset.id === readingAsset.id)
        throw new Error("雙鏡頭沒有形成一組相異的關係／閱讀資產:" + contract.pairId);
      if (relationAsset.id !== contract.relationship[2] || readingAsset.id !== contract.reading[2])
        throw new Error("雙鏡頭資產角色接錯:" + contract.pairId);
      if (JSON.stringify(readingAsset.embeddedText || []) !== JSON.stringify(contract.embeddedText) ||
          readingAsset.embeddedText.length > 3)
        throw new Error("閱讀圖允許文字超量或未逐字登錄:" + readingAsset.id);

      for (const asset of pair) {
        const runtimePath = path.join(here, "..", assets.basePath, asset.path);
        const masterPath = path.join(here, "../..", asset.sourceMaster);
        if (asset.w !== 1672 || asset.h !== 941 || !existsSync(runtimePath) || !existsSync(masterPath))
          throw new Error("雙鏡頭母版、runtime 或尺寸缺失:" + asset.id);
        if (statSync(runtimePath).size > 512 * 1024)
          throw new Error("雙鏡頭 runtime 超過 512KB:" + asset.id);
      }

      const relationshipRule = findTrigger(contract.relationship);
      const readingRule = findTrigger(contract.reading);
      if (!relationshipRule?.caption || !relationshipRule.items[0].alt ||
          !readingRule?.caption || !readingRule.items[0].alt)
        throw new Error("雙鏡頭缺少 caption 或 alt:" + contract.pairId);
      if (assetUses(readingAsset.id).length !== 1)
        throw new Error("閱讀圖出現在未核准的揭曉時點:" + readingAsset.id);

      const sceneIndex = (sceneId) => chapter.scenes.findIndex((scene) => scene.id === sceneId);
      const nodeIndex = (sceneId, match) => chapter.scenes[sceneIndex(sceneId)]?.nodes.findIndex((node) =>
        String(node.text || "").includes(match)) ?? -1;
      const relationScene = sceneIndex(contract.relationship[0]);
      const readingScene = sceneIndex(contract.reading[0]);
      const relationNode = nodeIndex(contract.relationship[0], contract.relationship[1]);
      const readingNode = nodeIndex(contract.reading[0], contract.reading[1]);
      if (relationScene < 0 || readingScene < 0 || relationNode < 0 || readingNode < 0 ||
          relationScene > readingScene || (relationScene === readingScene && relationNode >= readingNode))
        throw new Error("閱讀鏡頭早於關係鏡頭或沒有接到真實節點:" + contract.pairId);
    }

    const ch6ChoiceView = assets.viewFocusVisual.find((rule) =>
      rule.scene === "H1-1" && rule.match === "這組紙最多支持到哪裡？" &&
      rule.nodeIds?.length === 1 && rule.nodeIds[0] === "c1");
    if (!ch6ChoiceView)
      throw new Error("第六章閱讀鏡頭沒有在 c1 選項停留期間保持可見");

    const adr = readFileSync(path.join(here, "../decisions.md"), "utf-8");
    const current = JSON.parse(readFileSync(path.join(here, "../../03_規格/current-specs.json"), "utf-8"));
    if (!adr.includes("GB-ADR-044｜共同探究雙鏡頭") ||
        JSON.stringify(current.adrCoverage?.["GB-ADR-044"]) !==
          JSON.stringify(["shared", "ch01", "ch02", "ch03", "ch04", "ch05", "ch06"]))
      throw new Error("雙鏡頭規則沒有進入正式 ADR 與 CURRENT 覆蓋");
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    const d21n28 = ["n26", "n27", "n28", "n28a", "n28b"]
      .map((id) => String(sceneMap4.get("D2-1")?.get(id)?.text || "")).join(" ");
    const d31n29 = ["n29", "n29a", "n29b"]
      .map((id) => String(sceneMap4.get("D3-1")?.get(id)?.text || "")).join(" ");
    for (const [text, fragments] of [
      [d21n28, ["方向先固定朝此刻的地心", "一次只改一件事", "先試跑、記結果", "不能證明真正的天空"]],
      [d31n29, ["離得遠，拉得弱", "一年不會只跟著長一倍", "先估一個大致範圍"]]
    ...
    for (const fragment of [
      ...
      "用三張紙完成兩次單變因對照",
      "用三張紙寫下綜合斷言",
      "兩張一起翻。原先猜對或猜錯的預測都放回桌上",
      "把預測紙和落點紙並排"
    ]) if (!ui.includes(fragment))
      throw new Error("K1 試跑或 K3 封存預測沒有回到故事層:" + fragment);
    const newK1Start = ui.indexOf("if (!legacyOrbit)");
    const newK1End = ui.indexOf("} else {", newK1Start);
    const newK1Ui = ui.slice(newK1Start, newK1End);
    if (newK1Ui.includes("predictionSelect") ||
        newK1Ui.includes("先預測路徑") ||
...
tests.push({
  ...
  fn: () => {
    ...
    const required = [
      "用三張紙完成兩次單變因對照",
      "已引用來源｜無作用切線紙",
      "第一問｜在朝地心箭頭長度相同時，原本速度會怎麼影響軌道？",
      "在表格首欄勾兩張",
      "確認第一組對照",
      "先和牛頓把這組結果說清楚，再接第二問",
      "兩張的箭頭一樣長",
      "第一問只回答速度的影響",
      "第二問｜在原本速度相同時，朝地心箭頭長度又會怎麼影響軌道？",
      "接上第三張紙",
      "重選第一問",
      "兩次比較已完成",
      "把兩次比較合起來",
      "用三張紙寫下綜合斷言",
      "orbit4K1FirstSelection(paperTrials",
      "orbit4K1SecondSelection(paperTrials",
      "orbit4K1SecondQuestionReady(paperDraft)",
      "orbit4K1FinalClaimReady(paperDraft)",
      "orbit4K1FirstAckMatches(orbit4PendingK1FirstAck",
      "orbit4K1SecondAckMatches(orbit4PendingK1SecondAck",
      ...
      'data-orbit-focus", "orbit-k1-first-',
      'data-orbit-focus", "orbit-k1-second-'
    ];
    for (const fragment of required) if (!ui.includes(fragment))
      throw new Error("K1 兩問證據鏈缺少必要結構或文字:" + fragment);
    for (const forbidden of [
      "挑三張真正能互相比的紙",
      "同時看得見『繞得住』和『跑偏』",
      "資料與結論沒有接上。先看哪兩筆紀錄真的比較了同一件事。"
    ]) if (ui.includes(forbidden))
      throw new Error("K1 舊的一次三紙模糊提示回潮:" + forbidden);
    if (ui.includes("else orbit4ReleaseK1FirstAck(firstAckToken)") ||
        ui.includes("else orbit4ReleaseK1SecondAck(secondAckToken)"))
      throw new Error("K1 對話框缺失時仍會繞過人物回應，直接解鎖下一階段");
    if ((ui.match(/orbit4ClearK1FirstAck\(paperDraft\)/g) || []).length < 4 ||
        (ui.match(/orbit4ClearK1FirstAck\(\);/g) || []).length < 2)
      throw new Error("K1 對話 ack 沒有在重選、來源失效、換 embed 或重開遊戲時清除");
    if (/input\.onchange\s*=\s*function \(\) \{ if \(input\.checked\) paperDraft\.repairId = run\.id; \}/.test(ui))
      throw new Error("K1 第二問仍把 radio 暫選直接當成已確認結果");
    if (!ui.includes("firstPicked:{}, pairIds:[], middleId:null,") ||
        !ui.includes("firstReplyAcknowledged:false,") ||
        !ui.includes("secondPicked:{}, repairId:null,") ||
        !ui.includes("secondReplyAcknowledged:false, claim:\"\""))
      throw new Error("K1 兩問草稿不是全空起步，可能預選紙張或結論");
    const readyGate = ui.indexOf("if (!orbit4K1SecondQuestionReady(paperDraft))");
    const secondPrompt = ui.indexOf('"第二問｜在原本速度相同時，朝地心箭頭長度又會怎麼影響軌道？"');
    if (readyGate < 0 || secondPrompt < readyGate)
      throw new Error("K1 第二問在人物回應確認前便能出現");
    const finalGate = ui.indexOf("if (!orbit4K1FinalClaimReady(paperDraft))");
    const finalPrompt = ui.indexOf('"綜合判斷｜把兩次比較合起來：');
    if (finalGate < 0 || finalPrompt < finalGate)
      throw new Error("K1 綜合斷言在第二問人物回應確認前便能出現");
    if (!ui.includes('if (firstReplyAcknowledged !== true) return { error:"first-dialogue-required" };'))
      throw new Error("K1 第二問 handler 只靠畫面遮擋，沒有自己的回應確認 gate");
    const newK1Start = ui.indexOf("if (!legacyOrbit)");
    const newK1End = ui.indexOf("} else {", newK1Start);
    const newK1Ui = ui.slice(newK1Start, newK1End);
    ...
    if (logicStart < 0 || logicEnd < 0) throw new Error("K1 兩問純判讀函式無法抽取");
    const sandbox = {};
    new Script(ui.slice(logicStart, logicEnd) +
      ";this.first=orbit4K1FirstSelection;this.second=orbit4K1SecondSelection;" +
      "this.ready=orbit4K1SecondQuestionReady;this.finalReady=orbit4K1FinalClaimReady;" +
      "this.ackMatches=orbit4K1FirstAckMatches;this.secondAckMatches=orbit4K1SecondAckMatches;" +
      "this.release=orbit4ReleaseK1FirstAck;this.releaseSecond=orbit4ReleaseK1SecondAck;" +
      "this.clearAck=orbit4ClearK1FirstAck;this.clearSecond=orbit4ClearK1SecondAck;")
      .runInNewContext(sandbox);
    const fixture = [
      { id:1, speed:"slow", strength:"short", actualShape:"circle" },
      { id:2, speed:"medium", strength:"medium", actualShape:"circle" },
      { id:3, speed:"fast", strength:"long", actualShape:"circle" },
      { id:4, speed:"fast", strength:"medium", actualShape:"away" },
      { id:5, speed:"fast", strength:"short", actualShape:"away" }
    ];
    const first = sandbox.first(fixture, [2, 4]);
    if (first.error || JSON.stringify(first.pairIds) !== "[2,4]" || first.middleId !== 4)
      throw new Error("K1 第一問 handler 未建立第2↔4的共同中間紙");
    const wrongFirst = sandbox.first(fixture, [3, 4]);
    if (wrongFirst.error !== "strength-not-fixed")
      throw new Error("K1 第一問未拒絕轉向量不同的第3↔4張");
    if (sandbox.ready({ pairIds:first.pairIds, middleId:first.middleId,
        firstReplyAcknowledged:false }))
      throw new Error("K1 第一問人物回應未確認，第二問卻已標為可用");
    if (!sandbox.ready({ pairIds:first.pairIds, middleId:first.middleId,
        firstReplyAcknowledged:true }))
      throw new Error("K1 第一問人物回應確認後，第二問仍未解鎖");
    const pending = { token:"t1", embedKey:"D2-1/e1", pairKey:"2,4|4" };
    if (!sandbox.ackMatches(pending, "t1", "D2-1/e1", first.pairIds, first.middleId) ||
        sandbox.ackMatches(pending, "stale", "D2-1/e1", first.pairIds, first.middleId) ||
        sandbox.ackMatches(pending, "t1", "D2-2/e1", first.pairIds, first.middleId) ||
        sandbox.ackMatches(pending, "t1", "D2-1/e1", [3,4], first.middleId))
      throw new Error("K1 第一問對話 ack 沒有同時鎖 token、embed 與當前 pair");
    const runtimeState = {
      lab:{ orbitLab:{ paperTrials:fixture } },
      untouched:{ saveMarker:"must-not-change" }
    };
    const runtimeBefore = JSON.stringify(runtimeState);
    const transitionDraft = {
      pairIds:first.pairIds.slice(), middleId:first.middleId,
      firstReplyAcknowledged:false
    };
    sandbox.orbit4ClaimDraft = { "k1-paper-two-questions":transitionDraft };
    sandbox.orbit4PendingK1FirstAck = { ...pending };
    sandbox.orbit4EmbedKey = "D2-1/e1";
    sandbox.orbit4Msg = "";
    sandbox.state = runtimeState;
    sandbox.renderCount = 0;
    sandbox.renderedViewport = null;
    sandbox.orbit4CaptureViewport = () => ({ focusKey:"before-ack" });
    sandbox.orbit4RenderPreserving = (viewport) => {
      sandbox.renderCount += 1;
      sandbox.renderedViewport = { ...viewport };
    };
    if (sandbox.release("stale") !== false || transitionDraft.firstReplyAcknowledged ||
        sandbox.renderCount !== 0)
      throw new Error("K1 遲到或錯誤的對話 token 仍會改狀態或重畫第二問");
    if (sandbox.release("t1") !== true || transitionDraft.firstReplyAcknowledged !== true ||
        sandbox.orbit4PendingK1FirstAck !== null || sandbox.renderCount !== 1 ||
        sandbox.renderedViewport.focusKey !== "explicit:orbit-k1-second-1")
      throw new Error("K1 正確對話 ack 沒有解鎖第二問並把焦點交給第一張候選紙");
    if (sandbox.release("t1") !== false || sandbox.renderCount !== 1)
      throw new Error("K1 同一個對話 ack 可以重播解鎖");
    if (JSON.stringify(runtimeState) !== runtimeBefore)
      throw new Error("K1 純 UI 對話 ack 污染了可序列化的 engine/save state");
    sandbox.orbit4PendingK1FirstAck = { ...pending };
    sandbox.clearAck(transitionDraft);
    if (transitionDraft.firstReplyAcknowledged || sandbox.orbit4PendingK1FirstAck !== null)
      throw new Error("K1 重選／換場清理沒有重新鎖住第二問並清除遲到 token");
    const blockedSecond = sandbox.second(fixture, first.pairIds, first.middleId, 3, false);
    if (blockedSecond.error !== "first-dialogue-required")
      throw new Error("K1 未收掉人物回應仍可直接呼叫第二問 handler");
    const second = sandbox.second(fixture, first.pairIds, first.middleId, 3, true);
    if (second.error || second.repairId !== 3)
      throw new Error("K1 第二問 handler 未把第4↔3接回窄帶");
    const wrongSecond = sandbox.second(fixture, first.pairIds, first.middleId, 1, true);
    if (wrongSecond.error !== "speed-not-fixed")
      throw new Error("K1 第二問未拒絕速度不同的第三張紙");
    transitionDraft.repairId = second.repairId;
    transitionDraft.secondReplyAcknowledged = false;
    if (sandbox.finalReady(transitionDraft))
      throw new Error("K1 第二問人物回應未確認，綜合斷言卻已標為可用");
    const secondPending = {
      token:"t2", embedKey:"D2-1/e1", chainKey:"2,4|4|3"
    };
    if (!sandbox.secondAckMatches(secondPending, "t2", "D2-1/e1",
        first.pairIds, first.middleId, second.repairId) ||
        sandbox.secondAckMatches(secondPending, "stale", "D2-1/e1",
          first.pairIds, first.middleId, second.repairId) ||
        sandbox.secondAckMatches(secondPending, "t2", "D2-2/e1",
          first.pairIds, first.middleId, second.repairId) ||
        sandbox.secondAckMatches(secondPending, "t2", "D2-1/e1",
          first.pairIds, first.middleId, 5))
      throw new Error("K1 第二問對話 ack 沒有同時鎖 token、embed 與當前三紙鏈");
    sandbox.orbit4PendingK1SecondAck = { ...secondPending };
    sandbox.renderCount = 0;
    sandbox.renderedViewport = null;
    if (sandbox.releaseSecond("stale") !== false || transitionDraft.secondReplyAcknowledged ||
        sandbox.renderCount !== 0)
      throw new Error("K1 第二問遲到或錯誤 token 仍會開啟綜合斷言");
    if (sandbox.releaseSecond("t2") !== true || !transitionDraft.secondReplyAcknowledged ||
        sandbox.orbit4PendingK1SecondAck !== null || sandbox.renderCount !== 1 ||
        sandbox.renderedViewport.focusKey !== "explicit:orbit-paper-claim" ||
        !sandbox.finalReady(transitionDraft))
      throw new Error("K1 正確第二問 ack 沒有開啟綜合斷言並把焦點交給它");
    if (sandbox.releaseSecond("t2") !== false || sandbox.renderCount !== 1)
      throw new Error("K1 同一個第二問 ack 可以重播解鎖");
    if (JSON.stringify(runtimeState) !== runtimeBefore)
      throw new Error("K1 第二問純 UI 對話 ack 污染了可序列化的 engine/save state");
    sandbox.orbit4PendingK1SecondAck = { ...secondPending };
    sandbox.clearSecond(transitionDraft);
    if (transitionDraft.secondReplyAcknowledged || sandbox.orbit4PendingK1SecondAck !== null ||
        sandbox.finalReady(transitionDraft))
      throw new Error("K1 第二問重選／換場清理沒有重新鎖住綜合斷言");

    let s = ch4K0K2State();
    for (const [speed, strength] of [
      ["slow", "short"], ["medium", "medium"], ["fast", "long"],
      ["fast", "medium"], ["fast", "short"]
    ]) {
      const trial = Engine4.runOrbitPaperTrial(s, speed, strength);
      if (trial.error) throw new Error("無法建立截圖 K1 五紙 fixture:" + trial.error);
      s = trial.state;
    }
    const accepted = Engine4.assertK1(s,
      ["tangent", "trial:2", "trial:4", "trial:3"],
      "forward-plus-inward-turn");
    if (!accepted.ok || !accepted.state.evidence.k1)
      throw new Error("第2↔4、4↔3 的兩問證據鏈未能通過既有 assertK1");
    const rejected = Engine4.assertK1(s,
      ["tangent", "trial:3", "trial:4", "trial:5"],
      "forward-plus-inward-turn");
    if (rejected.ok)
      throw new Error("第3、4、5張缺少第一問速度對照，卻被當成合法三紙鏈");
  }
});

tests.push({
  name: "跨章原紙選取|同型資料只列一次，首欄勾選後再提出判讀",
  fn: () => {
    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    const slice = (from, to) => {
      const start = ui.indexOf(from);
      const end = ui.indexOf(to, start);
      if (start < 0 || end < 0) throw new Error("找不到跨章選紙邊界:" + from);
      return ui.slice(start, end);
    };
    /* CH1／CH3／CH5 原本已採同一資料表首欄勾選；本案不得為一致而重寫它們。 */
    for (const fragment of [
      'cb.type = "checkbox"; cb.className = "labRunSel"',
      'shipDossierLedgerPick',
      'var headings = ["選", "#", "配置"',
      'pick.type = "checkbox"'
    ]) if (!ui.includes(fragment))
      throw new Error("既有章的首欄選紙方式漂移:" + fragment);

    const ch2 = slice("/* ---------- 第二章彈射工坊面板", "/* ---------- 第三章航船實驗");
    if ((ch2.match(/collision5Table catDataTable/g) || []).length !== 1 ||
        !ch2.includes('["選", "#", "球", "4／9／16／25 格射程", "狀態"]') ||
        !ch2.includes('input.type = "checkbox"') ||
        !ch2.includes("pickedRows.length !== 1") ||
        !ch2.includes("pair.rows.length !== 2") ||
        !ch2.includes('selectionMode === "compare" ? s.status === "complete" && !!s.accepted') ||
        ch2.includes("var sourceSel") || ch2.includes("var sa =") || ch2.includes("var sb ="))
      throw new Error("CH2 仍在資料表外重列來源，或勾選數量／可引用門檻漂移");

    const ch4 = slice("/* ---------- 第四章軌道／跨尺度／校樣工作台", "/* ---------- 第五章：兩本帳工作台");
    if ((ch4.match(/collision5Table orbitK1PaperTable/g) || []).length !== 1 ||
        !ch4.includes('["選", "紙", "原本速度", "朝地心箭頭", "軌道結果"]') ||
        !ch4.includes("第二問請正好再勾一張紙") ||
        ch4.includes("orbitTrialChoice"))
      throw new Error("CH4 K1 沒有以同一表格完成兩次變因對照");

    const ch6 = slice('} else if (phase === "model-audit-board")',
      '} else if (phase === "joint-verification-page")');
    if ((ch6.match(/collision5Table heat6AuditTable/g) || []).length !== 1 ||
        !ch6.includes('["選", "原紙", "實際檢查內容"]') ||
        !ch6.includes("目前這一問請正好勾一張原紙") ||
        !ch6.includes('heat6Do("placeAuditEvidence"') ||
        ch6.includes("heat6Select(card"))
      throw new Error("CH6 稽核板仍以每格下拉重抄 T1–T4，或沒有逐問單選門檻");

    const ch7 = slice('} else if (v.phase === "board")', "    em1Gate(box, v);");
    if ((ch7.match(/collision5Table em1BoardTable/g) || []).length !== 1 ||
        !ch7.includes('["選", "原紙", "狀態"]') ||
        !ch7.includes("把勾選的原紙歸回同一頁") ||
        !ch7.includes("pick.disabled = placed || !visible") ||
        ch7.includes("把這張原紙放回同一頁"))
      throw new Error("CH7 六格合帳仍以六張按鈕卡重列，或已歸頁／撤離原紙仍可重送");

    if (!ui.includes("function em1PlaceBoardSelection(keys)") ||
        !ui.includes("if (nextState.lab.matrix.boardOrder.indexOf(key) >= 0) continue") ||
        !ui.includes('N.labAction(nextState, "placeMatrixTrace", { trace:key })'))
      throw new Error("CH7 批次歸頁沒有依 canonical 順序跳過已放原紙，可能重複追加 trace");
  }
});

tests.push({
  name: "CH4-CR-016|第四章以具體動作說明任務，不再要求玩家翻譯自製暗語",
  fn: () => {
    const visibleNodeText = (node) => [
      node.text, node.hint, node.title, node.label,
      ...(node.options || []).flatMap((option) => [option.text, option.hint, option.label])
    ].filter(Boolean).join("\n");
    const activeSceneText = scenes4.scenes.flatMap((scene) => [
      scene.title,
      ...scene.nodes.filter((node) => node.legacyOnly !== true).map(visibleNodeText)
    ]).join("\n");
    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    const notebookStart = ui.indexOf("/* notebook-renderer:ch4 — stable boundary for behavior contracts. */");
    const notebookEnd = ui.indexOf("/* notebook-renderer:ch5 — stable boundary for behavior contracts. */", notebookStart);
    const workbenchStart = ui.indexOf("/* ---------- 第四章軌道／跨尺度／校樣工作台 ---------- */");
    const workbenchEnd = ui.indexOf("/* ---------- 第五章：兩本帳工作台 ---------- */", workbenchStart);
    const overlayStart = ui.indexOf('if (CHAPTER_ID === "ch4" && upper === "K1")');
    const overlayEnd = ui.indexOf('if (CHAPTER_ID === "ch4" && upper === "K3")', overlayStart);
    if ([notebookStart, notebookEnd, workbenchStart, workbenchEnd, overlayStart, overlayEnd]
      .some((index) => index < 0))
      throw new Error("CH4 玩家可見 UI 邊界遺失，無法執行白話契約");
    const stageIntro = readFileSync(path.join(here, "../src/stage/07-intro-inputs.part.js"), "utf-8");
    const stageCh4Start = stageIntro.indexOf('} else if (CHAPTER_ID === "ch4")');
    const stageCh4End = stageIntro.indexOf('} else if (CHAPTER_ID === "ch5")', stageCh4Start);
    if (stageCh4Start < 0 || stageCh4End < 0)
      throw new Error("CH4 入台提示邊界遺失，無法執行白話契約");
    const ch4Ui = ui.slice(notebookStart, notebookEnd) + "\n" +
      ui.slice(workbenchStart, workbenchEnd) + "\n" + ui.slice(overlayStart, overlayEnd) +
      "\n" + stageIntro.slice(stageCh4Start, stageCh4End);
    const banned = /封口|作保|猜中|猜偏|盲驗|聽證|過帳|判讀留痕|開印窗口|失配|反驗|校樣窗口|署名線|信用線|來源線|機制末句|限定斷言|同尺紙|向內扳量|往內扳|往地心轉多少/;
    if (!banned.test("先替整片天空作保"))
      throw new Error("CH4 白話契約的負向探針沒有轉紅");
    const sceneHit = activeSceneText.match(banned);
    if (sceneHit) throw new Error("CH4 active 場景仍有自製暗語:" + sceneHit[0]);
    const uiHit = ch4Ui.match(banned);
    if (uiHit) throw new Error("CH4 工作台或旅人筆記仍有自製暗語:" + uiHit[0]);

    const d31 = scenes4.scenes.find((scene) => scene.id === "D3-1");
    const n38 = d31?.nodes.find((node) => node.id === "n38");
    for (const fragment of [
      "先寫下火星和木星的預測",
      "封好後才拆開觀測資料",
      "還不能說整片天空"
    ]) if (!String(n38?.text || "").includes(fragment))
      throw new Error("D3-1 沒有把預測、封好、核對與證據範圍說成完整因果:" + fragment);

    const assets = JSON.parse(readFileSync(path.join(here, "../data/assets.json"), "utf-8"));
    const evidenceText = JSON.stringify(Object.fromEntries(
      ["K1", "K2", "K3", "K4", "K5"].map((key) => [key, assets.evidenceVisual?.[key]])))
      .replace("漩渦帳原先兩格失配", "");
    const evidenceHit = evidenceText.match(banned);
    if (evidenceHit) throw new Error("CH4 證據 caption／alt／讀屏文字仍有自製暗語:" + evidenceHit[0]);
    for (const fragment of ["月亮", "行星", "彗星"])
      if (!JSON.stringify(assets.evidenceVisual?.K4 || {}).includes(fragment))
        throw new Error("K4 證據讀屏缺少中文資料名稱:" + fragment);
    const focus = (assets.lineFocusVisual || []).find((row) =>
      row.scene === "D3-1" && row.match === "哈雷掏出封蠟");
    if (!focus || focus.caption !== "火星與木星的觀測紙仍封在木匣裡：先寫下預測，兩張都封好後才打開資料。")
      throw new Error("D3-1 焦點圖說明未同步白話因果鏈");
    for (const fragment of [
      "左邊記拉力說，右邊記漩渦說",
      "一張「借條」貼在原結果旁",
      "原先猜對或猜錯的預測都放回桌上"
    ]) if (!ch4Ui.includes(fragment))
      throw new Error("CH4 受控物件詞缺少第一次白話定義或故事回應:" + fragment);
  }
});
...
tests.push({
  ...
  fn: () => {
    ...
    for (const [label, data, modes] of [
      ["第一章", debate, { P1:"player", P2:"player", P3:"player" }],
...
tests.push({
  name: "CH1-CR-012|押值失手延後揭曉、四路讀回與雙入口信譽鏈",
  fn: () => {
    const Sanitize = require("../src/sanitize.js");
    let missLab = Engine.initialState();
    const run = Engine.runExperiment(missLab, {
      ball:"銅大", surface:"打磨", incline:"緩", timer:"水鐘"
    });
    missLab = Engine.judge(run.state, [run.run.id], 0).state;
    const missClaim = missLab.inference.claims[0];
    if (!missClaim.consistent || missClaim.predHit || missClaim.ok)
      throw new Error("測試資料沒有形成『前四段成立、第五段押錯』");

    function a3Node({ miss = false, blurt = false } = {}) {
      const state = Narrative.initialState("explore");
      state.cursor = { scene:"A3-1", node:"n3c" };
      if (miss) state.lab = structuredClone(missLab);
      if (blurt) state.eventLog.push({ t:"choice", at:"P0-2/q1", pick:"a" });
      return Narrative.view(state).nodeId;
    }
    if (a3Node({ miss:true, blurt:true }) !== "n3c" ||
        a3Node({ miss:true }) !== "n3a" ||
        a3Node({ blurt:true }) !== "n3b" || a3Node() !== "n4")
      throw new Error("A3-1 的 C＞A＞B＞無讀回互斥優先序不成立");

    let delayedBlurt = Narrative.initialState("explore");
    delayedBlurt.cursor = { scene:"P0-2", node:"q1" };
    delayedBlurt = Narrative.choose(delayedBlurt, "b").state;
    delayedBlurt = Narrative.advance(delayedBlurt).state; // nB1 -> qB
    delayedBlurt = Narrative.choose(delayedBlurt, "b2").state;
    for (let i = 0; i < 4; i += 1) delayedBlurt = Narrative.advance(delayedBlurt).state;
    const checked = Sanitize.sanitizeImport(structuredClone(delayedBlurt), patterns, scenes);
    if (!checked.ok) throw new Error("先聽完再搶答的合法路徑遭 sanitizer 誤殺:" + checked.reason);
    const broken = structuredClone(delayedBlurt);
    broken.eventLog = broken.eventLog.filter((event) =>
      !(event.t === "choice" && event.at === "P0-2/qB" && event.pick === "b2"));
    if (Sanitize.sanitizeImport(broken, patterns, scenes).ok)
      throw new Error("缺 qB/b2 的偽造信譽事件仍通過 requiresBeforeAny");

    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    const stagePart = readFileSync(path.join(here, "../src/stage/04-typewriter.part.js"), "utf-8");
    for (const phrase of [
      "押線已封存；第五欄仍蓋著。",
      "封條掀開。第五段的刻痕躺在它躺了一下午的地方",
      "至少這張平均紙，現在不肯替我們作證。",
      "紙又答了一次。先別急著第三次",
      "錯的跟對的放在一起。往後誰問起規律是怎麼來的",
      "ch1PredictionPaper", "bd:dialogue-yielded"
    ]) if (!ui.includes(phrase)) throw new Error("第一章施工稿缺少契約文字或掛點:" + phrase);
    if (!stagePart.includes('CustomEvent("bd:dialogue-yielded"') ||
        !stagePart.includes("currentLineYieldToken"))
      throw new Error("舞台沒有在最後一句親手收隊後才交還揭曉權");
  }
});

tests.push({
  name: "CH3-CR-031|全稱主張存活、A3 撤回頁與公開讀回不升證據",
  fn: () => {
    const N3 = Narrative._factory(scenes3, Engine3, {});
    let blurt = N3.initialState("explore");
    blurt.cursor = { scene:"C0-3", node:"c1" };
    blurt = N3.choose(blurt, "all").state;
    if (N3.view(blurt).nodeId !== "a_commit1" || blurt.rep !== 2 ||
        blurt.flags.oldPaperScoped)
      throw new Error("首次全稱主張仍被立即糾正，或失敗路偷補 scoped");
    const commitIds = [];
    for (let i = 0; i < 4; i += 1) {
      commitIds.push(N3.view(blurt).nodeId);
      blurt = N3.advance(blurt).state;
    }
    if (commitIds.join(",") !== "a_commit1,a_commit2,a_commit3,a_commit4" ||
        N3.view(blurt).nodeId !== "n2")
      throw new Error("承諾拍沒有完整走四拍後匯回主線");

    const failAuth = structuredClone(blurt);
    failAuth.cursor = { scene:"C0-3", node:"x6" };
    if (N3.advance(failAuth).node.id !== "x6" || N3.view(N3.advance(failAuth).state).nodeId !== "x7_fail")
      throw new Error("全稱路沒有取得『署名責任』授權拍");
    let legacyReselect = structuredClone(blurt);
    legacyReselect.cursor = { scene:"C0-3", node:"c1" };
    legacyReselect = N3.choose(legacyReselect, "bounded").state;
    legacyReselect.cursor = { scene:"C0-3", node:"x6" };
    if (N3.view(N3.advance(legacyReselect).state).nodeId !== "x7")
      throw new Error("legacy 重選 bounded 後誤顯 x7_fail，或丟失 x7 授權理由拍");

    const c3 = structuredClone(blurt);
    c3.cursor = { scene:"C3-1", node:"n8" };
    if (N3.view(N3.advance(c3).state).nodeId !== "n8_failure")
      throw new Error("章末沒有把早上的全稱主張掛回公開木板");
    const ui = readFileSync(path.join(here, "../src/chapter-ui.js"), "utf-8");
    for (const phrase of [
      "你寫的是『所有』，不是『再一次』。",
      "哪一邊要撕？", "我撤回所有船況。今天只量到",
      "劃掉的字也留著。", "待驗頁｜已撤回",
      "別背你後來那句。把你劃掉『所有』時看的兩批紙攤開。"
    ]) if (!ui.includes(phrase)) throw new Error("第三章失敗前進接縫缺少契約文字:" + phrase);
    if (/grantEvidence|evidence\s*=/.test(ui.slice(ui.indexOf("ship3SayFailureWithdrawal"),
        ui.indexOf("ship3SayFailureWithdrawal") + 900)))
      throw new Error("撤回頁或撤回台詞被誤升為正面證據");
  }
});

tests.push({
  name: "CH2-CR-013|三列押注留痕、誠實揭曉與章末錯籌讀回",
  fn: () => {
    const N2 = Narrative._factory(scenes2, Engine2, require("../data/debate2.js"));
    const San = require("../src/sanitize.js");
    const paths = [
      ["a", "late", "r_late"],
      ["b", "unresolved", "r_unresolved"],
      ["c", "abstain", "r_abstain"]
    ];
    for (const [pick, value, recall] of paths) {
      let state = N2.initialState("explore");
      const rep = state.rep;
      state.cursor = { scene:"B2-4", node:"q1" };
      state = N2.choose(state, pick).state;
      if (state.flags.ch2DropPrediction !== value || state.rep !== rep ||
          N2.view(state).nodeId !== "c_commit")
        throw new Error("押注未封存、偷計分或未進承諾拍:" + value);
      state = N2.advance(state).state;
      for (const option of ["a", "a", "a"])
        state = N2.choose(state, option).state;
      state = N2.choose(state, "a").state;
      if (N2.view(state).nodeId !== recall)
        throw new Error("三列揭曉沒有讀回原押注:" + value);
      state = N2.advance(state).state;
      if (N2.view(state).nodeId !== "na1" || state.rep !== rep)
        throw new Error("誠實押注讀回沒有匯回有限結論，或誤扣信譽:" + value);
      const clean = San.sanitizeImport2(JSON.parse(N2.serialize(state)), scenes2, Engine2);
      if (!clean.ok) throw new Error("新押注旗標不能合法存檔:" + clean.reason);
    }

    let misconduct = N2.initialState("explore");
    misconduct.flags.ch2DropPrediction = "late";
    misconduct.cursor = { scene:"B2-4", node:"q4" };
    const rep = misconduct.rep;
    misconduct = N2.choose(misconduct, "b").state;
    if (N2.view(misconduct).nodeId !== "nb1" || misconduct.rep !== rep - 1 ||
        misconduct.transcript.some((line) => line.text.includes("三列紙沒有替它作證")))
      throw new Error("補寫聲響沒有先走既有失信修正");
    misconduct = N2.advance(misconduct).state;
    misconduct = N2.choose(misconduct, "a").state;
    if (N2.view(misconduct).nodeId !== "r_late")
      throw new Error("失信修正後沒有回到誠實有限結論與原押注讀回");

    const legacy = N2.initialState("explore");
    legacy.cursor = { scene:"B2-4", node:"q4" };
    const legacyDone = N2.choose(legacy, "a").state;
    if (N2.view(legacyDone).nodeId !== "na1")
      throw new Error("缺新旗標的 legacy 存檔被捏造押注讀回");

    const later = N2.initialState("explore");
    later.flags.ch2DropPrediction = "late";
    later.cursor = { scene:"B3-1", node:"n3" };
    const recalled = N2.advance(later).state;
    if (N2.view(recalled).nodeId !== "b3_recall" ||
        !N2.view(recalled).text.includes("我們的錯，也記帳"))
      throw new Error("晚到錯籌沒有活進章末辯論入口");
  }
});

tests.push({
  name: "CH4-CR-013|切線錯紙不擦、事件優先讀回與封存不捏造",
  fn: () => {
    const N4 = Narrative._factory(scenes4, Engine4, {});
    for (const [pick, first, trace] of [
      ["curve", "w1", "w1_trace"],
      ["inward", "w3", "w3_trace"]
    ]) {
      let state = N4.initialState("explore");
      state.cursor = { scene:"D1-1", node:"c1" };
      state = N4.choose(state, pick).state;
      if (N4.view(state).nodeId !== first) throw new Error("切線錯項入口漂移:" + pick);
      state = N4.advance(state).state;
      if (N4.view(state).nodeId !== trace)
        throw new Error("切線錯紙被即時擦除:" + pick);
    }

    function d42(picks) {
      const state = N4.initialState("explore");
      state.cursor = { scene:"D4-2", node:"n16" };
      for (const pick of picks)
        state.eventLog.push({ t:"choice", at:"D1-1/c1", pick });
      return N4.view(N4.advance(state).state).nodeId;
    }
    if (d42(["curve", "inward", "tangent"]) !== "d42_recall_curve" ||
        d42(["inward", "tangent"]) !== "d42_recall_inward" ||
        d42(["tangent"]) !== "d42_recall_tangent" || d42([]) !== "n17")
      throw new Error("D4-2 未維持 curve＞inward＞tangent＞legacy略過 的事件優先序");

    const d42Scene = scenes4.scenes.find((scene) => scene.id === "D4-2");
    const d42Nodes = new Map(d42Scene.nodes.map((node) => [node.id, node]));
    if (d42Nodes.get("e1")?.next !== "n17")
      throw new Error("D4-2 新主線完成 K5 後仍重播署名爭議");
    for (const id of ["n13", "n14", "n15", "n16",
      "d42_recall_curve", "d42_recall_inward", "d42_recall_tangent"])
      if (d42Nodes.get(id)?.legacyOnly !== true)
        throw new Error("D4-2 舊游標節點未保留為 legacyOnly：" + id);
    const legacyCursor = N4.initialState("explore");
    legacyCursor.cursor = { scene:"D4-2", node:"n13" };
    if (N4.view(legacyCursor).nodeId !== "n13" ||
        !N4.view(legacyCursor).text.includes("虎克的兩封信"))
      throw new Error("D4-2 功能上線前停在舊署名段的游標不能原地續讀");

    const archive = N4.initialState("explore");
    archive.cursor = { scene:"DE-1", node:"ok1" };
    const closed = N4.advance(archive).state;
    if (N4.view(closed).nodeId !== "de1_close" ||
        !N4.view(closed).text.includes("曾真正出現的紙"))
      throw new Error("封存句沒有守住直答者不被寫成走過錯路");
  }
});

tests.push({
  name: "CH5-CR-003|尺度斷言進晚宴、縮句與沉默皆前進且辯論狀態不漂移",
  fn: () => {
    const N5 = Narrative._factory(scenes5, Engine5, debate5);
    for (const [pick, value, response, commit] of [
      ["overclaim-vital-force", "vis-viva-open", "scale_bad_world", "v_commit"],
      ["momentum-wrong", "momentum-false-open", "scale_bad_momentum", "m_commit"]
    ]) {
      let state = N5.initialState("explore");
      const rep = state.rep;
      state.cursor = { scene:"E2-3", node:"c_scale" };
      state = N5.choose(state, pick).state;
      if (state.flags.ch5ScaleClaim !== value || state.rep !== rep ||
          N5.view(state).nodeId !== response)
        throw new Error("尺度斷言未留名、誤計分或錯失原反應:" + pick);
      state = N5.advance(state).state;
      if (N5.view(state).nodeId !== commit ||
          N5.view(N5.advance(state).state).nodeId !== "n1a")
        throw new Error("尺度斷言仍被退回重選:" + pick);
    }

    function atX4(value) {
      let state = N5.initialState("explore");
      state.flags.ch5ScaleClaim = value;
      state.cursor = { scene:"E3-1", node:"init" };
      for (let index = 0; index < 4; index += 1) state = N5.advance(state).state;
      if (N5.view(state).nodeId !== "x4") throw new Error("無法建立晚宴先手拍");
      return state;
    }

    let silent = atX4("vis-viva-open");
    const debateBefore = JSON.stringify(silent.debate);
    silent = N5.advance(silent).state;
    if (N5.view(silent).nodeId !== "duel_v1") throw new Error("活力越界沒有遭對手先手反擊");
    silent = N5.advance(silent).state;
    silent = N5.choose(silent, "hold").state;
    if (N5.view(silent).nodeId !== "duel_v2_v") throw new Error("堅持路沒有遭同批資料再壓");
    silent = N5.advance(silent).state;
    silent = N5.choose(silent, "silent").state;
    if (silent.flags.ch5ScaleClaim !== "vis-viva-silent" ||
        N5.view(silent).nodeId !== "e_silent")
      throw new Error("沉默被當成撤回或形成死路");
    silent = N5.advance(silent).state;
    if (N5.view(silent).nodeId !== "debate" || JSON.stringify(silent.debate) !== debateBefore)
      throw new Error("沉默路未進辯論，或先手戲偷偷改動辯論引擎狀態");

    let shrunk = atX4("momentum-false-open");
    shrunk = N5.advance(shrunk).state;
    if (N5.view(shrunk).nodeId !== "duel_v1m") throw new Error("動量否定沒有進專屬先手拍");
    shrunk = N5.advance(shrunk).state;
    shrunk = N5.choose(shrunk, "shrink").state;
    if (shrunk.flags.ch5ScaleClaim !== "momentum-false-shrunk" ||
        N5.view(shrunk).nodeId !== "e_shrunk")
      throw new Error("玩家縮句沒有保留原立場來源");

    const direct = atX4("measurement-only");
    if (N5.view(N5.advance(direct).state).nodeId !== "debate")
      throw new Error("原本縮限正確者被捏造晚宴失敗戲");
    const legacy = atX4(undefined);
    if (N5.view(N5.advance(legacy).state).nodeId !== "debate")
      throw new Error("legacy 缺旗標者被捏造晚宴失敗戲");

    for (const [value, expected] of [
      ["momentum-false-shrunk", "e32_trace"],
      ["vis-viva-silent", "e32_trace_silent"],
      ["measurement-only", "x1"]
    ]) {
      const state = N5.initialState("explore");
      state.flags.ch5ScaleClaim = value;
      state.cursor = { scene:"E3-2", node:"n1" };
      if (N5.view(N5.advance(state).state).nodeId !== expected)
        throw new Error("E3-2 沒有按處理方式讀回原斷言:" + value);
    }
  }
});

tests.push({
  name: "CH6-CR-002|越界宣言合法取得T2、S8追債與共同頁撤回墊紙",
  fn: () => {
    const N6 = Narrative._factory(scenes6, Engine6, {});
    const San = require("../src/sanitize.js");
    function dryReady() {
      let lab = Engine6.initialState();
      for (const source of Engine6.SOURCES) {
        const rule = Engine6.SOURCE_RULES[source];
        lab = Engine6.setSourceLedger(lab, {
          source, position:rule.position, consequence:rule.consequence
        }).state;
      }
      lab = Engine6.sealModels(lab, { caloric:"finite-sources", motion:"continued-motion" }).state;
      lab = Engine6.runChipComparison(lab).state;
      lab = Engine6.judgeChipComparison(lab, { concept:"chips-not-lower-capacity" }).state;
      for (const condition of ["rotation-only", "pressure-only", "contact-motion"])
        lab = Engine6.runFrictionCondition(lab, { condition }).state;
      lab = Engine6.judgeFrictionConditions(lab, { concept:"contact-and-motion" }).state;
      return Engine6.runDryStrip(lab).state;
    }

    let overreach = N6.initialState("explore");
    overreach.lab = dryReady();
    overreach.cursor = { scene:"H1-3", node:"c1" };
    overreach = N6.choose(overreach, "heat-is-motion").state;
    if (overreach.rep !== 2 || overreach.flags.ch6DryOverreachTried !== 1 ||
        N6.view(overreach).nodeId !== "w3")
      throw new Error("首次越界沒有維持既有署名資格代價");
    for (const nodeId of ["h13_commit", "h13_sign", "t2_overreach"]) {
      overreach = N6.advance(overreach).state;
      if (N6.view(overreach).nodeId !== nodeId)
        throw new Error("越界承諾拍接縫漂移，預期:" + nodeId);
    }
    overreach = N6.advance(overreach).state;
    if (N6.view(overreach).nodeId !== "n6" || !overreach.evidence.T2 ||
        !overreach.lab.evidence.t2 || !overreach.lab.dryBench.judged)
      throw new Error("越界路只演台詞，沒有合法完成乾式判讀與 T2");
    if (overreach.eventLog.filter((event) => event.t === "rep").length !== 1)
      throw new Error("越界路重複扣信譽");
    const air = N6.labAction(overreach, "sealAirPrediction", { prediction:"slower-when-sealed" });
    if (air.error) throw new Error("越界路取得 T2 後仍被 air prediction 擋住:" + air.error);
    const clean = San.sanitizeImport6(JSON.parse(N6.serialize(overreach)), scenes6, Engine6);
    if (!clean.ok) throw new Error("越界路形成 sanitizer 死存檔:" + clean.reason);

    let repeat = N6.initialState("explore");
    repeat.lab = dryReady();
    repeat.rep = 2;
    repeat.flags.ch6DryOverreachTried = 1;
    repeat.cursor = { scene:"H1-3", node:"c1" };
    repeat = N6.choose(repeat, "heat-is-motion-again").state;
    if (repeat.rep !== 2) throw new Error("重複同一越界點擊被當成新破裂事實再扣分");

    function h31(flag, keep) {
      let state = N6.initialState("explore");
      if (flag) state.flags.ch6DryOverreachTried = 1;
      state.cursor = { scene:"H3-1", node:"n9" };
      state = N6.advance(state).state;
      if (!flag) return N6.view(state).nodeId;
      if (N6.view(state).nodeId !== "h31_recall") throw new Error("S8 沒有追到第一天勝負宣言");
      state = N6.advance(state).state;
      if (!keep) return N6.choose(state, "unresolved").state;
      state = N6.choose(state, "keep").state;
      if (N6.view(state).nodeId !== "h31_press" ||
          !N6.view(state).text.includes("不能把缺口寫成勝利"))
        throw new Error("堅持路沒有被數據壓回主張邊界");
      state = N6.advance(state).state;
      return N6.choose(state, "unresolved").state;
    }
    const immediate = h31(true, false);
    const pressed = h31(true, true);
    if (N6.view(immediate).nodeId !== "a1" || immediate.flags.ch6LatentUnresolved !== 1 ||
        N6.view(pressed).nodeId !== "a1" || pressed.flags.ch6LatentUnresolved !== 1)
      throw new Error("S8 兩條修復路沒有親手匯入機制未決");
    if (h31(false, false) !== "c1")
      throw new Error("未曾越界的玩家被捏造勝負宣言");

    const end = N6.initialState("explore");
    end.cursor = { scene:"HE-1", node:"n1" };
    const veil = N6.advance(end).state;
    if (N6.view(veil).nodeId !== "he1_veil" ||
        !N6.view(veil).text.includes("墊著的紙不簽名"))
      throw new Error("共同頁沒有讓撤回紙承托最後簽名");
  }
});
