/* scenes3/histfacts3 JSON → file:// 可執行載體。規範來源永遠是 JSON。 */
...
function buildMirror(base, globalName, description) {
  ...
  const output = `/* data/${base}.js — ${description}（file:// 相容）。規範鏡像:${base}.json。
   ⚠ 本檔為生成物；請改 ${base}.json 後執行 node tools/build-ch3-data.mjs。 */
(function (root) {
 "use strict";
 var data = ${body};
...
export function buildCh3Data() {
  buildMirror("scenes3", "scenes3", "第三章場景執行載體");
  buildMirror("histfacts3", "histfacts3", "第三章史實頁執行載體");
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  buildCh3Data();
  console.log("scenes3.js / histfacts3.js 已由 JSON 重建");
}
