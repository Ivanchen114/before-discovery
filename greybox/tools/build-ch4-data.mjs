/* scenes4/histfacts4 JSON → file:// 可執行載體。規範來源永遠是 JSON。 */
...
function buildMirror(base, globalName, description) {
  ...
  const output = `/* data/${base}.js — ${description}（file:// 相容）。規範鏡像:${base}.json。
   ⚠ 本檔為生成物；請改 ${base}.json 後執行 node tools/build-ch4-data.mjs。 */
(function (root) {
 "use strict";
 var data = ${body};
...
export function buildCh4Data() {
  buildMirror("scenes4", "scenes4", "第四章場景執行載體");
  buildMirror("histfacts4", "histfacts4", "第四章史實頁執行載體");
}

if (process.argv[1] === fileURLToPath(import.meta.url)) {
  buildCh4Data();
  console.log("scenes4.js / histfacts4.js 已由 JSON 重建");
}
